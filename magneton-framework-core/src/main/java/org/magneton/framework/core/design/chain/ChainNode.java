...
public abstract class ChainNode<T extends ChainContext> {
	...
	protected <D> D getData(String key) {
		return this.getChainContext().getData(key);
	}

	protected <D> boolean setData(String key, D data) {
		return this.getChainContext().setData(key, data);
	}

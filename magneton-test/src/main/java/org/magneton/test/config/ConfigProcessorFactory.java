...
public class ConfigProcessorFactory implements AfterAutowrited {
  ...
  @TestAutowired private List<ConfigProcessor> configProcessors;
  ...
  public static ConfigProcessor of(InjectType injectType) {
    ConfigProcessorFactory configProcessorFactory =
        ChaosContext.getComponent(ConfigProcessorFactory.class);
    Verify.verifyNotNull(configProcessorFactory, "inject type factory found");

    ConfigProcessor injectTypeProcessor =
        configProcessorFactory.cacheConfigProcessors.get(injectType);
    Verify.verifyNotNull(
        injectTypeProcessor, "inject type %s not config processor found", injectType);
    return new ConfigPostProcessorAdapter(
            injectTypeProcessor, configProcessorFactory.configPostProcessors)

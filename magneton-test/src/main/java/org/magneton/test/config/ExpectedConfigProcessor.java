...
public class ExpectedConfigProcessor implements ConfigProcessor {
    ...
    if (minBigDecimal.compareTo(maxBigDecimal) == 0) {
      ...
      d =
          ThreadLocalRandom.current()
              .nextDouble(maxBigDecimal.doubleValue(), minBigDecimal.doubleValue());
    } else {
      d =
          ThreadLocalRandom.current()
              .nextDouble(minBigDecimal.doubleValue(), maxBigDecimal.doubleValue());
    }
  ...
  public BigInteger nextBigInteger(Config config, Definition definition) {
    ...
    if (minBigInterger.compareTo(maxBigInteger) == 0) {
      ...
      return BigInteger.valueOf(
          ThreadLocalRandom.current()
              .nextLong(maxBigInteger.longValue(), minBigInterger.longValue()));
    }
    return BigInteger.valueOf(
        ThreadLocalRandom.current()
            .nextLong(minBigInterger.longValue(), maxBigInteger.longValue()));
  }
  ...
  public String nextString(Config config, Definition definition) {
    ...
    GenericGenerator genericGenerator = stringMode.getGenericGenerator();
    String str = null;
    if (genericGenerator != null) {
      str = genericGenerator.generate();
    }

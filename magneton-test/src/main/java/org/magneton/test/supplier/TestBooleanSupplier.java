...
public class TestBooleanSupplier {
   ...
   *
   * <p>注意：如果要判断的对象 {@code obj} 为 {@code null}， 则判断一定为 {@code true}
   *
   * @param obj 要判断的对象
   * @return Boolean Supplier.
   */
   ...
   * 判断两个对象的引用或字段数据是否相同
   *
   * <p>注意：时间类型{@link java.util.Date} 或 {@link java.sql.Date}默认允许存在一定的时间差异， 祥见{@link
   * ValueEqualsBooleanSupplier#doFeieldValueNotEqualsProcess(Class, Object, Object)}
   *
   * @param element1 对象1
   * @param element2 对象2
   * @param <E> 相同对象

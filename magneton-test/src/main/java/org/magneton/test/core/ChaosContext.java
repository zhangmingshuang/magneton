...
public class ChaosContext {
  ...
  public static void init(Class<? extends ChaosApplication> chaosApplication) {
    ...
    try {
      ...
      TestComponentScan componentScan =
          AnnotationUtil.findAnnotations(chaosApplication, TestComponentScan.class);
    ...
    for (Field field : fields) {
      ...
      if (Collection.class.isAssignableFrom(fieldClass)) {
        ...
        if (autowired.required()) {
          Verify.verify(
              !list.isEmpty(), "无法找到%s的对象注入到%s#%s中", generics.get(0), className, field.getName());
        }
        SortUtil.sort(list);
        field.set(obj, list);
      } else {
  ...
  private static void loadAndInitialize(Set<String> scanPackages) throws IOException {
    ...
    for (ClassInfo classInfo : allClassInfos) {
      ...
      TestComponentScan componentScan =
          AnnotationUtil.findAnnotations(clazz, TestComponentScan.class);
      if (componentScan != null

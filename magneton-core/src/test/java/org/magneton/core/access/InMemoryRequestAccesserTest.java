...
class InMemoryRequestAccesserTest {
  ...
  static void beforeAll() {
    requestAccesser =
        (RequestAccesser)
            AccesserBuilder.of(new InMemoryRequestAccesser<>())
                .lockSize(lockSize)
                .lockErrorCount(lockErrorCount)
                .lockTime(lockTime)
                .lockErrorCount(lockErrorCount)
                .errorRecordTime(errorRecordTime)
                .errorRecordSize(errorRecordSize)
                .build();
  }
  ...
  private void recordError(String key) {
    for (int i = 0; i < lockErrorCount; i++) {
      ...
      Assertions.assertEquals(
          remainingErrorCount, lockErrorCount - i - 1, "remaining errors not match.");
    }
  }
}

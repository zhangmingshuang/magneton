...
final class TrieParser {
	...
	/**
	 * Parses a serialized trie representation of a map of reversed public suffixes into
	 * an immutable map of public suffixes.
	 */
	 ...
	 * @param stack The prefixes that precede the characters represented by this node.
	 * Each entry of the stack is in reverse order.
	 * @param encoded The serialized trie.
	 * @param start An index in the encoded serialized trie to begin reading characters
	 * from.
	 * @param builder A map builder to which all entries will be added.
	 * @return The number of characters consumed from {@code encoded}.
	 */
	private static int doParseTrieToBuilder(Deque<CharSequence> stack, CharSequence encoded, int start,
		...
		if (c == '!' || c == '?' || c == ':' || c == ',') {
			// '!' represents an interior node that represents a REGISTRY entry in the
			// map.

...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor, int concurrencyLevel,
		...
		Preconditions.checkState(initialCapacity >= 0, "Initial capacity must not be negative");
		Preconditions.checkState(loadFactor > 0f, "Load factor must be positive");
		Preconditions.checkState(concurrencyLevel > 0, "Concurrency level must be positive");
		Preconditions.checkNotNull(referenceType, "Reference type must not be null");
		...
		int resizeThreshold = (int) (initialSize * this.getLoadFactor());
	...
	public boolean containsKey(@Nullable Object key) {
		Reference<K, V> ref = this.getReference(key, Restructure.WHEN_NECESSARY);
		Entry<K, V> entry = (ref != null ? ref.get() : null);
		return (entry != null && Objects.equal(entry.getKey(), key));
	}
	...
	private V put(@Nullable K key, @Nullable V value, boolean overwriteExisting) {
		return this.doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
			@Override
			@Nullable
			protected V execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry,
				...
				Preconditions.checkState(entries != null, "No entries segment");
				entries.add(value);
				return null;
			}
		});
	...
	public V remove(Object key) {
		return this.doTask(key, new Task<V>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
			@Override
			@Nullable
			protected V execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
				if (entry != null) {
	...
	public boolean remove(Object key, Object value) {
		Boolean result = this.doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
				if (entry != null && Objects.equal(entry.getValue(), value)) {
					if (ref != null) {
						ref.release();
					}
					return true;
	...
	public boolean replace(K key, V oldValue, V newValue) {
		Boolean result = this.doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
				if (entry != null && Objects.equal(entry.getValue(), oldValue)) {
	...
	public V replace(K key, V value) {
		return this.doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
			@Override
			@Nullable
			protected V execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
				if (entry != null) {
	...
	protected final class Segment extends ReentrantLock {
		...
		private void restructure(boolean allowResize, @Nullable Reference<K, V> ref) {
			...
			try {
				...
				Reference<K, V>[] restructured = (resizing ? this.createReferenceArray(restructureSize)
						: this.references);
				...
				if (resizing) {
					...
					this.resizeThreshold = (int) (this.references.length
							* ConcurrentReferenceHashMap.this.getLoadFactor());
				}
	...
	protected static final class Entry<K, V> implements Map.Entry<K, V> {
		...
		public final boolean equals(@Nullable Object other) {
			...
			return (Objects.equal(this.getKey(), otherEntry.getKey())
	...
	private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains(@Nullable Object o) {
			if (o instanceof Map.Entry<?, ?>) {
				...
				if (otherEntry != null) {
					return Objects.equal(otherEntry.getValue(), otherEntry.getValue());
				}
			}
			return false;
		}
	...
	private class EntryIterator implements Iterator<Map.Entry<K, V>> {
		...
		@Override
		public void remove() {
			Preconditions.checkState(this.last != null, "No element to remove");

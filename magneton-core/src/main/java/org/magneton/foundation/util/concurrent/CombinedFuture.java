...
final class CombinedFuture<V extends Object> extends AggregateFuture<@Nullable Object, V> {
	...
	CombinedFuture(ImmutableCollection<? extends ListenableFuture<?>> futures, boolean allMustSucceed,
		...
		task = new AsyncCallableInterruptibleTask(callable, listenerExecutor);
		init();
	}
	...
	CombinedFuture(ImmutableCollection<? extends ListenableFuture<?>> futures, boolean allMustSucceed,
		...
		super(futures, allMustSucceed, false);
		task = new CallableInterruptibleTask(callable, listenerExecutor);
		init();
	}
	...
	void releaseResources(ReleaseResourcesReason reason) {
		...
		/*
		 * If the output future is done, then it won't need to interrupt the task later,
		 * so it can clear its reference to it.
		 *
		 * If the output future is *not* done, then the task field will be cleared after
		 * the task runs or after the output future is done, whichever comes first.
		 */
	...
	private abstract class CombinedFutureInterruptibleTask<T> extends InterruptibleTask<T> {
		...
		final void afterRanInterruptiblySuccess(@ParametricNullness T result) {
			/*
			 * The future no longer needs to interrupt this task, so it no longer needs a
			 * reference to it.
			 *
			 * TODO(cpovirk): It might be nice for our InterruptibleTask subclasses to
			 * null out their `callable` fields automatically. That would make it less
			 * important for us to null out the reference to `task` here (though it's
			 * still nice to do so in case our reference to the executor keeps it alive).
			 * Ideally, nulling out `callable` would be the responsibility of
			 * InterruptibleTask itself so that its other subclasses also benefit.
			 * (Handling `callable` in InterruptibleTask itself might also eliminate some
			 * of the existing boilerplate for, e.g., pendingToString().)
			 */
			task = null;
		...
		final void afterRanInterruptiblyFailure(Throwable error) {
			...
			if (error instanceof ExecutionException) {
				/*
				 * Cast to ExecutionException to satisfy our nullness checker, which
				 * (unsoundly but *usually* safely) assumes that getCause() returns
				 * non-null on an ExecutionException.
				 */
				setException(((ExecutionException) error).getCause());
			}
	...
	private final class AsyncCallableInterruptibleTask extends CombinedFutureInterruptibleTask<ListenableFuture<V>> {
		...
		ListenableFuture<V> runInterruptibly() throws Exception {
			...
			return checkNotNull(result, "AsyncCallable.call returned null instead of a Future. "

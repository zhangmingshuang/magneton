...
public interface TimeLimiter {
	 ...
	 * delegation is also performed for calls to {@link Object#equals},
	 ...
	 * exception is propagated to the caller exactly as-is. If, on the other hand, the
	 * time limit is reached, the proxy will attempt to abort the call to the target, and
	 * will throw an {@link UncheckedTimeoutException} to the caller.
	 *
	 * <p>
	 * It is important to note that the primary purpose of the proxy object is to return
	 * control to the caller when the timeout elapses; aborting the target method call is
	 * of secondary concern. The particular nature and strength of the guarantees made by
	 * the proxy is implementation-dependent. However, it is important that each of the
	 * methods on the target object behaves appropriately when its thread is interrupted.
	 *
	 * <p>
	 * For example, to return the value of {@code target.someMethod()}, but substitute
	 * {@code
	 ...
	 * @param timeoutDuration with timeoutUnit, the maximum length of time that callers
	 * are willing to wait on each method call to the proxy
	 * @param timeoutUnit with timeoutDuration, the maximum length of time that callers
	 * are willing to wait on each method call to the proxy
	 * @return a time-limiting proxy
	 * @throws IllegalArgumentException if {@code interfaceType} is a regular class, enum,
	 * or annotation type, rather than an interface
	 */
	 ...
	 * @param target the object to proxy
	 * @param interfaceType the interface you wish the returned proxy to implement
	 * @param timeout the maximum length of time that callers are willing to wait on each
	...
	/**
	 * Invokes a specified Callable, timing out after the specified time limit. If the
	 * target method call finishes before the limit is reached, the return value or a
	 * wrapped exception is propagated. If, on the other hand, the time limit is reached,
	 * we attempt to abort the call to the target, and throw a {@link TimeoutException} to
	 * the caller.
	 ...
	 * @throws UncheckedExecutionException if {@code callable} throws a
	 ...
	 *
	 * <p>
	 * The difference with {@link #callWithTimeout(Callable, long, TimeUnit)} is that this
	 * method will ignore interrupts on the current thread.
	 * @param callable the Callable to execute
	 * @param timeoutDuration with timeoutUnit, the maximum length of time to wait
	 * @param timeoutUnit with timeoutDuration, the maximum length of time to wait
	 * @return the result returned by the Callable
	 ...
	 *
	 * <p>
	 * The difference with {@link #callWithTimeout(Callable, Duration)} is that this
	 * method will ignore interrupts on the current thread.
	 * @param callable the Callable to execute
	 * @param timeout the maximum length of time to wait
	 * @return the result returned by the Callable
	...
	default <T> T callUninterruptiblyWithTimeout(Callable<T> callable, Duration timeout)
			throws TimeoutException, ExecutionException {
		return callUninterruptiblyWithTimeout(callable, Internal.toNanosSaturated(timeout), TimeUnit.NANOSECONDS);
	}

	/**
	 * Invokes a specified Runnable, timing out after the specified time limit. If the
	 * target method run finishes before the limit is reached, this method returns or a
	 * wrapped exception is propagated. If, on the other hand, the time limit is reached,
	 * we attempt to abort the run, and throw a {@link TimeoutException} to the caller.
	 * @param runnable the Runnable to execute
	 * @param timeoutDuration with timeoutUnit, the maximum length of time to wait
	 * @param timeoutUnit with timeoutDuration, the maximum length of time to wait
	 * @throws TimeoutException if the time limit is reached
	 * @throws InterruptedException if the current thread was interrupted during execution
	 * @throws UncheckedExecutionException if {@code runnable} throws a
	 * {@code RuntimeException}
	 ...
	 *
	 * <p>
	 * The difference with {@link #runWithTimeout(Runnable, long, TimeUnit)} is that this
	 * method will ignore interrupts on the current thread.
	 * @param runnable the Runnable to execute
	 * @param timeoutDuration with timeoutUnit, the maximum length of time to wait
	 * @param timeoutUnit with timeoutDuration, the maximum length of time to wait
	 ...
	 *
	 * <p>
	 * The difference with {@link #runWithTimeout(Runnable, Duration)} is that this method

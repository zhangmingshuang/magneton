...
import org.magneton.foundation.util.concurrent.Service.State;
...
import static org.magneton.foundation.util.concurrent.Service.State.FAILED;
import static org.magneton.foundation.util.concurrent.Service.State.NEW;
import static org.magneton.foundation.util.concurrent.Service.State.RUNNING;
import static org.magneton.foundation.util.concurrent.Service.State.STARTING;
import static org.magneton.foundation.util.concurrent.Service.State.STOPPING;
import static org.magneton.foundation.util.concurrent.Service.State.TERMINATED;

/**
 * A manager for monitoring and controlling a set of
 * {@linkplain Service services}. This class provides
 ...
 * {@linkplain Service services}. Additionally, users
...
public final class ServiceManager implements ServiceManagerBridge {
	...
	/**
	 * Initiates service {@linkplain Service#startAsync
	...
	/**
	 * Initiates service {@linkplain Service#stopAsync
	...
	public ImmutableMap<Service, Duration> startupDurations() {
		return ImmutableMap.copyOf(
				org.magneton.core.collect.Maps.<Service, Long, Duration>transformValues(startupTimes(), Duration::ofMillis));
	}
	 ...
	 * each particular {@linkplain Service service}
	 * starts, or terminates), should attach
	 * {@linkplain Service.Listener service listeners}
	...
	private static final class ServiceManagerState {
		...
		@GuardedBy("monitor")
		final SetMultimap<State, Service> servicesByState = MultimapBuilder
		...
		@GuardedBy("monitor")
		final Map<Service, Stopwatch> startupTimers = org.magneton.core.collect.Maps
				.newIdentityHashMap();
		...
		void markReady() {
			...
			try {
				if (!transitioned) {
					...
					List<Service> servicesInBadStates = org.magneton.core.collect.Lists
							.newArrayList();
		...
		ImmutableSetMultimap<State, Service> servicesByState() {
			ImmutableSetMultimap.Builder<State, Service> builder = ImmutableSetMultimap
					.builder();
			monitor.enter();
			try {
	...
	/**
	 * A {@link Service} that wraps another service and
	 * times how long it takes for it to start and also calls the
	 * {@link ServiceManagerState#transitionService(Service, State, State)},
	...
	/**
	 * A {@link Service} instance that does nothing.

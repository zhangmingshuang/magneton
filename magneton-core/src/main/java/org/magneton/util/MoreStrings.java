...
public final class MoreStrings {
  ...
  /**
   * Returns the given {@code template} string with each occurrence of {@code "%s0","%s1" or "%sx"}
   * replaced with the corresponding argument value from {@code args}; or if the placeholder and
   * argument count do not match, keep the placeholder of that string.
   *
   * <pre>eg: {@code
   ...
   * }</pre>
   *
   * @param template a string containing zero or more {@code "%s0"} placeholder sequences.
   ...
   * @return {@code true} if the given strings contains one or more {@code null} or empty, {@code

...
public final class MoreObjects {
	 ...
	 * {@code Iterables.find(iterable,
	 * Predicates.notNull())}. For varargs, use
	 * {@code Iterables.find(Arrays.asList(a, b, c, ...),
	 * Predicates.notNull())}, static importing as necessary.
	 *
	 * <p>
	 * <b>Note:</b> if {@code first} is represented as an
	 * {@link org.magneton.core.base.Optional}, this can be accomplished with
	 * {@link org.magneton.core.base.Optional#or(Object) first.or(second)}. That approach
	 * also allows for lazy evaluation of the fallback instance, using
	 * {@link org.magneton.core.base.Optional#or(Supplier) first.or(supplier)}.
	 *
	 * <p>
	 * <b>Java 9 users:</b> use
	 * {@code java.util.Objects.requireNonNullElse(first, second)} instead.
	 * @return {@code first} if it is non-null; otherwise {@code second} if it is non-null
	 * @throws NullPointerException if both {@code first} and {@code second} are null
	 * @since 18.0 (since 3.0 as {@code Objects.firstNonNull()}).
	 */
	/*
	 * We annotate firstNonNull in a way that protects against NullPointerException at the
	 * cost of forbidding some reasonable calls.
	 *
	 * The more permissive signature would be to accept (@CheckForNull T
	 * first, @CheckForNull T second), since it's OK for `second` to be null as long as
	 * `first` is not also null. But we expect for that flexibility to be useful
	 * relatively rarely: The more common use case is to supply a clearly non-null
	 * default, like `firstNonNull(someString, "")`. And users who really know that
	 * `first` is guaranteed non-null when `second` is null can write the logic out
	 * longhand, including a requireNonNull call, which calls attention to the fact that
	 * the static analyzer can't prove that the operation is safe.
	 *
	 * This matches the signature we currently have for requireNonNullElse in our own
	 * checker. (And that in turn matches that method's signature under the Checker
	 * Framework.) As always, we could consider the more flexible signature if we judge it
	 * worth the risks. If we do, we would likely update both methods so that they
	 * continue to match.
	 */
	 ...
	 * @param self the object to generate the string for (typically {@code this}), used
	 ...
	 * {@link #toStringHelper(Object)}, but using the simple name of {@code clazz} instead
	...
	/**
	 * Creates an instance of {@link ToStringHelper} in the same manner as
	 * {@link #toStringHelper(Object)}, but using {@code className} instead of using an
	...
	public static final class ToStringHelper {
		...
		private static boolean isEmpty(Object value) {
			...
			else if (value instanceof org.magneton.core.base.Optional) {
		...
		/**
		 * Adds a name/value pair to the formatted output in {@code name=value} format. If
		 * {@code value} is {@code null}, the string {@code "null"} is used, unless
		 ...
		 *
		 * <p>
		 * It is strongly encouraged to use {@link #add(String, Object)} instead and give
		 ...
		 *
		 * <p>
		 * It is strongly encouraged to use {@link #add(String, boolean)} instead and give
		 * value a readable name.
		 *
		 ...
		 *
		 * <p>
		 * It is strongly encouraged to use {@link #add(String, double)} instead and give
		 * value a readable name.
		 *
		 * @since 18.0 (since 11.0 as {@code Objects.ToStringHelper.addValue()}).
		 */
		 ...
		 * properties cannot be removed, so this only allows limited reuse of the helper
		 * instance. The helper allows duplication of properties (multiple name/value
		 * pairs with the same name can be added).
		 */
		@Override
		...
		/**
		 * Holder object for values that cannot be null or empty (will be printed
		 * unconditionally). This helps to shortcut most calls to isEmpty(), which is
		 * important because the check for emptiness is relatively expensive. Use a
		 * subtype so this also doesn't need any extra storage.
		 */

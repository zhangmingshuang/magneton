...
/**
 * Static convenience methods that serve the same purpose as Java language <a href=
 * "https://docs.oracle.com/javase/8/docs/technotes/guides/language/assert.html">assertions</a>,
 * except that they are always enabled. These methods should be used instead of Java
 ...
 *
 * <p>
 * <b>Note:</b> In some cases the differences explained below can be subtle. When it's
 * unclear which approach to use, <b>don't worry</b> too much about it; just pick
 * something that seems reasonable and it will be fine.
 *
 * <ul>
 * <li>If checking whether the <i>caller</i> has violated your method or constructor's
 * contract (such as by passing an invalid argument), use the utilities of the
 * {@link Preconditions} class instead.
 * <li>If checking an <i>impossible</i> condition (which <i>cannot</i> happen unless your
 * own class or its <i>trusted</i> dependencies is badly broken), this is what ordinary
 * Java assertions are for. Note that assertions are not enabled by default; they are
 * essentially considered "compiled comments."
 * <li>An explicit {@code if/throw} (as illustrated below) is always acceptable; we still
 * recommend using our {@link VerifyException} exception type. Throwing a plain
 * {@link RuntimeException} is frowned upon.
 * <li>Use of {@link java.util.Objects#requireNonNull(Object)} is generally discouraged,
 * since {@link #verifyNotNull(Object)} and {@link Preconditions#checkNotNull(Object)}
 ...
 * verification succeeds and the message ends up unneeded. Performance-sensitive
 ...
 *
 * <p>
 * As with {@link Preconditions}, {@code Verify} uses {@link Strings#lenientFormat} to
 * format error message template strings. This only supports the {@code "%s"} specifier,
 * not the full range of {@link java.util.Formatter} specifiers. However, note that if the
 * number of arguments does not match the number of occurrences of {@code "%s"} in the
 * format string, {@code Verify} will still behave as expected, and will still include all
 ...
 * failures explained</a> in the Guava User Guide for advice on when this class should be
 * used.
 *
 * @since 17.0
 */
...
public final class Verify {
	...
	/**
	 * Ensures that {@code expression} is {@code true}, throwing a {@code VerifyException}
	 ...
	 * @see Preconditions#checkState Preconditions.checkState()
	 */
	public static void verify(boolean expression, String errorMessageTemplate,
	 ...
	 *
	 * <p>
	 * See {@link #verify(boolean, String, Object...)} for details.
	 *
	 ...
	 *
	 * (verifyNotNull has many fewer "problem" callers, so we could try to be stricter. On
	 * the other hand, verifyNotNull arguably has more reason to accept nullable arguments
	 * in the first place....)
	 */
	...
	/**
	 * Ensures that {@code reference} is non-null, throwing a {@code VerifyException} with
	 ...
	 */
	@CanIgnoreReturnValue
	public static <T> T verifyNotNull(@CheckForNull T reference, String errorMessageTemplate,
			@CheckForNull @Nullable Object... errorMessageArgs) {
		if (reference == null) {
			throw new VerifyException(Strings.lenientFormat(errorMessageTemplate, errorMessageArgs));
		}

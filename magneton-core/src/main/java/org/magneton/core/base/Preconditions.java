 ...
 * invoked correctly (that is, whether its <i>preconditions</i> were met).
 *
 * <p>
 * If the precondition is not met, the {@code Preconditions} method throws an unchecked
 * exception of a specified type, which helps the method in which the exception was thrown
 * communicate that its caller has made a mistake. This allows constructs such as
 *
 * <pre>{@code
 ...
 *
 * <p>
 * Avoid passing message arguments that are expensive to compute; your code will always
 * compute them, even though they usually won't be needed. If you have such arguments, use
 * the conventional if/throw idiom instead.
 *
 * <p>
 * Depending on your message arguments, memory may be allocated for boxing and varargs
 * array creation. However, the methods of this class have a large number of overloads
 * that prevent such allocations in many common cases.
 *
 * <p>
 * The message string is not formatted unless the exception will be thrown, so the cost of
 * the string formatting itself should not be a concern.
 *
 * <p>
 * As with any performance concerns, you should consider profiling your code (in a
 * production environment if possible) before spending a lot of effort on tweaking a
 * particular element.
 *
 ...
 *
 * <p>
 * It is of course possible to use the methods of this class to check for invalid
 * conditions which are <i>not the caller's fault</i>. Doing so is <b>not recommended</b>
 * because it is misleading to future readers of the code and of stack traces. See
 * <a href="https://github.com/google/guava/wiki/ConditionalFailuresExplained">Conditional
 * failures explained</a> in the Guava User Guide for more advice. Notably,
 * {@link org.magneton.core.base.Verify} offers assertions similar to those in this class
 * for non-precondition checks.
 *
 ...
 * {@link #checkNotNull(Object)} or
 * {@link org.magneton.core.base.Verify#verifyNotNull(Object)} is appropriate to the
 * situation. (The same goes for the message-accepting overloads.)
 *
 * <h3>Only {@code %s} is supported</h3>
 *
 * <p>
 * {@code Preconditions} uses {@link Strings#lenientFormat} to format error message
 * template strings. This only supports the {@code "%s"} specifier, not the full range of
 * {@link java.util.Formatter} specifiers. However, note that if the number of arguments
 * does not match the number of occurrences of {@code "%s"} in the format string,
 * {@code Preconditions} will still behave as expected, and will still include all
 * argument values in the error message; the message will simply not be formatted exactly
 * as intended.
 *
...
public final class Preconditions {
	...
	/**
	 * Ensures the truth of an expression involving one or more parameters to the calling
	 * method.
	 ...
	 * method.
	 * @param expression a boolean expression
	 * @param errorMessageTemplate a template for the exception message should the check
	 * fail. The message is formed by replacing each {@code %s} placeholder in the
	 * template with an argument. These are matched by position - the first {@code %s}
	 * gets {@code
	 ...
	 */
	public static void checkArgument(boolean b, String errorMessageTemplate, @CheckForNull Object p1,
			@CheckForNull Object p2, @CheckForNull Object p3) {
		if (!b) {
	 ...
	 */
	public static void checkArgument(boolean b, String errorMessageTemplate, @CheckForNull Object p1,
			@CheckForNull Object p2, @CheckForNull Object p3, @CheckForNull Object p4) {
		if (!b) {
	...
	/**
	 * Ensures the truth of an expression involving the state of the calling instance, but
	 * not involving any parameters to the calling method.
	 * @param expression a boolean expression
	 * @throws IllegalStateException if {@code expression} is false
	 * @see org.magneton.core.base.Verify#verify Verify.verify()
	 */
	...
	public static void checkState(boolean expression,
			/*
			 * TODO(cpovirk): Consider removing @CheckForNull here, as we've done with the
			 * other methods' errorMessageTemplate parameters: It it unlikely that callers
			 * intend for their string template to be null (though we do handle that case
			 * gracefully at runtime). I've left this one as it is because one of our
			 * users has defined a wrapper API around Preconditions, declaring a
			 * checkState method that accepts a possibly null template. So we'd need to
			 * update that user first.
			 */
			@CheckForNull String errorMessageTemplate, @CheckForNull @Nullable Object... errorMessageArgs) {
		if (!expression) {
	...
	/*
	 * Preconditions.checkNotNull is *intended* for performing eager null checks on
	 * parameters that a nullness checker can already "prove" are non-null. That means
	 * that the first parameter to checkNotNull *should* be annotated to require it to be
	 * non-null.
	 *
	 * However, for a variety of reasons, Google developers have written a ton of code
	 * over the past decade that assumes that they can use checkNotNull for
	 * non-precondition checks. I had hoped to take a principled stand on this, but the
	 * amount of such code is simply overwhelming. To avoid creating a lot of compile
	 * errors that users would not find to be informative, we're giving in and allowing
	 * callers to pass arguments that a nullness checker believes could be null.
	 *
	 * We still encourage people to use requireNonNull over checkNotNull for
	 * non-precondition checks.
	 */
	 ...
	 * @see org.magneton.core.base.Verify#verifyNotNull Verify.verifyNotNull()
	 */
	 ...
	 *
	 * The alternative natural refactorings into void or Exception-returning methods are
	 * much slower. This is a big deal - we're talking factors of 2-8 in microbenchmarks,
	 * not just 10-20%. (This is a hotspot optimizer bug, which should be fixed, but
	 * that's a separate, big project).
	 *
	 * The coding pattern above is heavily used in java.util, e.g. in ArrayList. There is
	 * a RangeCheckMicroBenchmark in the JDK that was used to test this.
	 *
	 * But the methods in this class want to throw different exceptions, depending on the
	 * args, so it appears that this pattern is not directly applicable. But we can use
	 * the ridiculous, devious trick of throwing an exception in the middle of the
	 * construction of another exception. Hotspot is fine with that.
	 */

	/**
	 * Ensures that {@code index} specifies a valid <i>element</i> in an array, list or
	 * string of size {@code size}. An element index may range from zero, inclusive, to
	 * {@code size}, exclusive.
	 * @param index a user-supplied index identifying an element of an array, list or
	 * string
	...
	/**
	 * Ensures that {@code index} specifies a valid <i>position</i> in an array, list or
	 * string of size {@code size}. A position index may range from zero to {@code size},
	 * inclusive.
	 * @param index a user-supplied index identifying a position in an array, list or
	 * string
	 * @param size the size of that array, list or string
	 * @return the value of {@code index}
	 * @throws IndexOutOfBoundsException if {@code index} is negative or is greater than
	 * {@code size}
	 * @throws IllegalArgumentException if {@code size} is negative
	 */
	@CanIgnoreReturnValue
	...
	/**
	 * Ensures that {@code start} and {@code end} specify valid <i>positions</i> in an
	 * array, list or string of size {@code size}, and are in order. A position index may
	 * range from zero to {@code
	 * size}, inclusive.
	 * @param start a user-supplied index identifying a starting position in an array,
	 * list or string
	 * @param end a user-supplied index identifying an ending position in an array, list
	 * or string
	 * @param size the size of that array, list or string
	 * @throws IndexOutOfBoundsException if either index is negative or is greater than
	 * {@code size}, or if {@code end} is less than {@code start}
	 * @throws IllegalArgumentException if {@code size} is negative
	 */

 ...
 * {@link Predicate} does for any {@link Object}. Also offers basic text processing
 * methods based on this function. Implementations are strongly encouraged to be
 ...
 *
 * <p>
 * <b>Warning:</b> This class deals only with {@code char} values, that is,
 * <a href="http://www.unicode.org/glossary/#BMP_character">BMP characters</a>. It does
 * not understand
 * <a href="http://www.unicode.org/glossary/#supplementary_code_point">supplementary
 * Unicode code points</a> in the range {@code 0x10000} to {@code 0x10FFFF} which includes
 * the majority of assigned characters, including important CJK characters and emoji.
 *
 * <p>
 * Supplementary characters are <a href=
 * "https://docs.oracle.com/javase/8/docs/api/java/lang/Character.html#supplementary">encoded
 * into a {@code String} using surrogate pairs</a>, and a {@code CharMatcher} treats these
 * just as two separate characters. {@link #countIn} counts each supplementary character
 * as 2 {@code char}s.
 *
 * <p>
 * For up-to-date Unicode character properties (digit, letter, etc.) and support for
 * supplementary code points, use ICU4J UCharacter and UnicodeSet (freeze() after
 * building). For basic text processing based on UnicodeSet use the ICU4J
 * UnicodeSetSpanner.
 *
 * <p>
 * Example usages:
 ...
 * "https://github.com/google/guava/wiki/StringsExplained#charmatcher">{@code CharMatcher}
 * </a>.
 *
...
public abstract class CharMatcher implements Predicate<Character> {
	...
	/*
	 * N777777777NO N7777777777777N M777777777777777N $N877777777D77777M N
	 * M77777777ONND777M MN777777777NN D777 N7ZN777777777NN ~M7778 N777777777777MMNN88777N
	 * N777777777777MNZZZ7777O DZN7777O77777777777777 N7OONND7777777D77777N
	 * 8$M++++?N???$77777$ M7++++N+M77777777N N77O777777777777$ M DNNM$$$$777777N D
	 * N$N:=N$777N7777M NZ 77Z::::N777777777 ODZZZ 77N::::::N77777777M NNZZZ$
	 * $777:::::::77777777MN ZM8ZZZZZ 777M::::::Z7777777Z77 N++ZZZZNN
	 * 7777M:::::M7777777$777M $++IZZZZM M777$:::::N777777$M7777M +++++ZZZDN
	 * NN$::::::7777$$M777777N N+++ZZZZNZ N::::::N:7$O:77777777 N++++ZZZZN
	 * M::::::::::::N77777777+ +?+++++ZZZM 8::::::::::::D77777777M O+++++ZZ
	 * ::::::::::::M777777777N O+?D M:::::::::::M77777777778 77= D=::::::::::N7777777777N
	 * 777 INN===::::::=77777777777N I777N ?777N========N7777777777787M N7777
	 * 77777$D======N77777777777N777N? N777777
	 * I77777$$$N7===M$$77777777$77777777$MMZ77777777N
	 * $$$$$$$$$$$NIZN$$$$$$$$$M$$7777777777777777ON M$$$$$$$$M
	 * M$$$$$$$$N=N$$$$7777777$$$ND O77Z$$$$$$$ M$$$$$$$$MNI==$DNNNNM=~N 7 :N MNN$$$$M$
	 * $$$777$8 8D8I NMM.:7O 777777778 7777777MN M NO .7: M : M 8
	 */
	 ...
	 *
	 * <p>
	 * <b>Note:</b> as the Unicode definition evolves, we will modify this matcher to keep
	 * it up to date.
	 *
	...
	/**
	 * Determines whether a character is a BMP digit according to <a href=
	 ...
	 * ASCII digits, you can use {@code inRange('0',
	 * '9')}.
	 * @deprecated Many digits are supplementary characters; see the class documentation.
	 ...
	 * {@linkplain Character#isLetter(char) Java's definition}. If you only care to match
	 * letters of the Latin alphabet, you can use
	 ...
	 *
	 * <p>
	 * <b>Note:</b> as the reference file evolves, we will modify this matcher to keep it
	 * up to date.
	 *
	 * <p>
	 * See also <a href="http://www.unicode.org/reports/tr11/">UAX #11 East Asian
	 * Width</a>.
	...
	/**
	 * Returns a {@code char} matcher that matches any BMP character present in the given
	 * character sequence. Returns a bogus matcher if the sequence contains supplementary
	 * characters.
	 */
	...
	/**
	 * Returns a {@code char} matcher that matches any BMP character not present in the
	 * given character sequence. Returns a bogus matcher if the sequence contains
	...
	/**
	 * Returns a {@code char} matcher that matches any character in a given BMP range
	 * (both endpoints are inclusive). For example, to match any lowercase letter of the
	 * English alphabet, use {@code
	...
	/**
	 * Returns a matcher with identical behavior to the given {@link Character}-based
	 * predicate, but which operates on primitive {@code char} instances instead.
	 */
	...
	/**
	 * Helper method for {@link #precomputedInternal} that doesn't test if the negation is
	 * cheaper.
	 */
	...
	/**
	 * Returns the Java Unicode escape sequence for the given {@code char}, in the form
	 * "\u12AB" where "12AB" is the four hexadecimal digits representing the 16-bit code
	 * unit.
	 */
	...
	/**
	 * Returns a {@code char} matcher functionally equivalent to this one, but which may
	 * be faster to query than the original; your mileage may vary. Precomputation takes
	 * time and is likely to be worthwhile only if the precomputed matcher is queried many
	 * thousands of times.
	 *
	 * <p>
	 * This method has no effect (returns {@code this}) when called in GWT: it's unclear
	 * whether a precomputed matcher is faster, but it certainly consumes more memory,
	 * which doesn't seem like a worthwhile tradeoff in a browser.
	 */
	...
	/**
	 * This is the actual implementation of {@link #precomputed}, but we bounce calls
	 * through a method on {@link Platform} so that we can have different behavior in GWT.
	 *
	 * <p>
	 * This implementation tries to be smart in a number of ways. It recognizes cases
	 * where the negation is cheaper to precompute than the matcher itself; it tries to
	 * build small hash tables for matchers that only match a few characters, and so on.
	 * In the worst-case scenario, it constructs an eight-kilobyte bit array and queries
	 * that. In many situations this produces a matcher which is faster to query than the
	 * original.
	 */
	CharMatcher precomputedInternal() {
		...
		if (totalCharacters * 2 <= DISTINCT_CHARS) {
			...
			// TODO(lowasser): is it worth it to worry about the last character of large
			// matchers?
	...
	/**
	 * Returns {@code true} if a character sequence contains at least one matching BMP
	 ...
	 *
	 * <p>
	 * Counts 2 per supplementary character, such as for
	 * {@link #whitespace}().{@link #negate}().
	 */
	...
	/**
	 * Returns a string containing all non-matching characters of a character sequence, in
	 * order. For example:
	 *
	 * <pre>{@code
	...
	/**
	 * Returns a string containing all matching BMP characters of a character sequence, in
	 * order. For example:
	 *
	 * <pre>{@code
	 ...
	 * matching character, then iterates the remainder of the sequence calling
	 ...
	 * matching character in {@code sequence}
	...
	/**
	 * Returns a string copy of the input character sequence, with each matching BMP
	 * character replaced by a given replacement sequence. For example:
	 *
	 * <pre>{@code
	 ...
	 *
	 * <p>
	 * <b>Note:</b> If the replacement is a fixed string with only one character, you are
	 * better off calling {@link #replaceFrom(CharSequence, char)} directly.
	 * @param sequence the character sequence to replace matching characters in
	 * @param replacement the characters to append to the result string in place of each
	 * matching character in {@code sequence}
	 * @return the new string
	 */
	...
	/**
	 * Returns a substring of the input character sequence that omits all matching BMP
	 * characters from the beginning and from the end of the string. For example:
	 *
	 * <pre>{@code
	...
	/**
	 * Returns a substring of the input character sequence that omits all matching BMP
	 * characters from the beginning of the string. For example:
	 *
	 * <pre>{@code
	...
	/**
	 * Returns a substring of the input character sequence that omits all matching BMP
	 * characters from the end of the string. For example:
	 *
	 * <pre>{@code
	 ...
	 * consecutive matching BMP characters replaced by a single replacement character. For
	 * example:
	 *
	 * <pre>{@code
	...
	/**
	 * Collapses groups of matching characters exactly as {@link #collapseFrom} does,
	 * except that groups of matching BMP characters at the start or end of the sequence
	...
	static final class Whitespace extends NamedFastMatcher {
		...
		// TABLE is a precomputed hashset of whitespace characters. MULTIPLIER serves as a
		// hash function
		// whose key property is that it maps 25 characters into the 32-slot table without
		// collision.
		// Basically this is an opportunistic fast implementation as opposed to "good
		// code". For most
	...
	private static final class Digit extends RangesMatcher {
		...
		private static final String ZEROES = "0\u0660\u06f0\u07c0\u0966\u09e6\u0a66\u0ae6\u0b66\u0be6\u0c66\u0ce6\u0d66\u0de6"
				+ "\u0e50\u0ed0\u0f20\u1040\u1090\u17e0\u1810\u1946\u19d0\u1a80\u1a90\u1b50\u1bb0"
				+ "\u1c40\u1c50\ua620\ua8d0\ua900\ua9d0\ua9f0\uaa50\uabf0\uff10";
	...
	private static final class ForPredicate extends CharMatcher {
		...
		// intentional; deprecation is for callers
		// primarily
		@Override
		public boolean apply(Character character) {

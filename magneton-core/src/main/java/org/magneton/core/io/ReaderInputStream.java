...
/**
 * An {@link InputStream} that converts characters from a {@link Reader} into bytes using
 * an arbitrary Charset.
 *
 * <p>
 * This is an alternative to copying the data to an {@code OutputStream} via a
 * {@code Writer}, which is necessarily blocking. By implementing an {@code InputStream}
 * it allows consumers to "pull" as much data as they can handle, which is more convenient
 * when dealing with flow controlled, async APIs.
 *
...
final class ReaderInputStream extends InputStream {
	...
	/**
	 * charBuffer holds characters that have been read from the Reader but not encoded
	 * yet. The buffer is perpetually "flipped" (unencoded characters between position and
	 * limit).
	 */
	private CharBuffer charBuffer;

	/**
	 * byteBuffer holds encoded characters that have not yet been sent to the caller of
	 * the input stream. When encoding it is "unflipped" (encoded bytes between 0 and
	 * position) and when draining it is flipped (undrained bytes between position and
	 * limit).
	 */
	...
	/**
	 * Creates a new input stream that will encode the characters from {@code reader} into
	 * bytes using the given character set. Malformed input and unmappable characters will
	 * be replaced.
	...
	/**
	 * Creates a new input stream that will encode the characters from {@code reader} into
	 * bytes using the given character set encoder.
	 * @param reader input source
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		// The rest of this method implements the process described by the CharsetEncoder
		// javadoc.
	...
	private void readMoreChars() throws IOException {
		...
		//
		// In case 2 we shift the existing chars to the left, and in case 3 we create a
		// bigger
	...
	/**
	 * Flips the buffer output buffer so we can start reading bytes from it. If we are
	 * starting to drain because there was overflow, and there aren't actually any
	 * characters to drain, then the overflow must be due to a small output buffer.
	 */
	...
	/**
	 * Copy as much of the byte buffer into the output array as possible, returning the

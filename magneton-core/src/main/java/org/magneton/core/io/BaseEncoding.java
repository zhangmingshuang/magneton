...
/**
 * A binary encoding scheme for reversibly translating between byte sequences and
 * printable ASCII strings. This class includes several constants for encoding schemes
 * specified by <a href="http://tools.ietf.org/html/rfc4648">RFC 4648</a>. For example,
 * the expression:
 *
 * <pre>{@code
 ...
 * RFC 4648. Decoding rejects characters in the wrong case, though padding is optional. To
 * modify encoding and decoding behavior, use configuration methods to obtain a new
 * encoding with modified behavior:
 *
 * <pre>{@code
 ...
 *
 * <p>
 * Warning: BaseEncoding instances are immutable. Invoking a configuration method has no
 * effect on the receiving instance; you must store and use the new encoding instance it
 * returns, instead.
 *
 * <pre>{@code
 * // Do NOT do this
 ...
 * <td>1.60
 * <td>=
 * <td>"Numerical" base 32; extended from the traditional hex alphabet. Defaults to upper
 * case.
 * <tr>
 * <td>{@link #base64()}
 * <td>A-Z a-z 0-9 + /
 ...
 *
 * <p>
 * All instances of this class are immutable, so they may be stored safely as static
 * constants.
 *
...
public abstract class BaseEncoding {
	...
	// TODO(lowasser): document the extent of leniency, probably after adding
	// ignore(CharMatcher)
	 ...
	 * <a href="http://tools.ietf.org/html/rfc4648#section-4">RFC 4648 section 4</a>, Base
	 * 64 Encoding. (This is the same as the base 64 encoding from
	 * <a href="http://tools.ietf.org/html/rfc3548#section-3">RFC 3548</a>.)
	 *
	 * <p>
	 * The character {@code '='} is used for padding, but can be
	 * {@linkplain #omitPadding() omitted} or {@linkplain #withPadChar(char) replaced}.
	 *
	 * <p>
	 * No line feeds are added by default, as per
	 * <a href="http://tools.ietf.org/html/rfc4648#section-3.1">RFC 4648 section 3.1</a>,
	 * Line Feeds in Encoded Data. Line feeds may be added using
	 ...
	 * <a href="http://tools.ietf.org/html/rfc4648#section-5">RFC 4648 section 5</a>, Base
	 * 64 Encoding with URL and Filename Safe Alphabet, also sometimes referred to as the
	 * "web safe Base64." (This is the same as the base 64 encoding with URL and filename
	 * safe alphabet from <a href="http://tools.ietf.org/html/rfc3548#section-4">RFC
	 * 3548</a>.)
	 *
	 * <p>
	 * The character {@code '='} is used for padding, but can be
	 ...
	 * <a href="http://tools.ietf.org/html/rfc4648#section-6">RFC 4648 section 6</a>, Base
	 * 32 Encoding. (This is the same as the base 32 encoding from
	 ...
	 * <a href="http://tools.ietf.org/html/rfc4648#section-7">RFC 4648 section 7</a>, Base
	 * 32 Encoding with Extended Hex Alphabet. There is no corresponding encoding in RFC
	 * 3548.
	 *
	 * <p>
	 * The character {@code '='} is used for padding, but can be
	 ...
	 * <a href="http://tools.ietf.org/html/rfc4648#section-8">RFC 4648 section 8</a>, Base
	 * 16 Encoding. (This is the same as the base 16 encoding from
	...
	/**
	 * Returns an {@code OutputStream} that encodes bytes using this encoding into the
	 * specified {@code Writer}. When the returned {@code OutputStream} is closed, so is
	 * the backing {@code
	 * Writer}.
	 */
	...
	public final org.magneton.core.io.ByteSink encodingSink(CharSink encodedSink) {
	...
	/**
	 * Determines whether the specified character sequence is a valid encoded string
	 ...
	 * Decodes the specified character sequence, and returns the resulting {@code byte[]}.
	 * This is the inverse operation to {@link #encode(byte[])}.
	 * @throws DecodingException if the input is not a valid encoded string according to
	 * this encoding.
	 */
	 ...
	 * {@code
	 * Reader}. The returned stream throws a {@link DecodingException} upon
	...
	public final org.magneton.core.io.ByteSource decodingSource(CharSource encodedSource) {
	 ...
	 * <a href="http://tools.ietf.org/html/rfc4648#section-3.2">RFC 4648 section 3.2</a>,
	 ...
	 * @throws IllegalArgumentException if this padding character is already used in the
	 * alphabet or a separator
	 */
	 ...
	 * @throws IllegalArgumentException if any alphabet or padding characters appear in
	 * the separator string, or if {@code n <= 0}
	 ...
	 * decodes with uppercase letters. Padding and separator characters remain in their
	 * original case.
	 * @throws IllegalStateException if the alphabet used by this encoding contains mixed
	...
	/**
	 * Returns an encoding that behaves equivalently to this encoding, but encodes and
	 * decodes with lowercase letters. Padding and separator characters remain in their
	 * original case.
	 * @throws IllegalStateException if the alphabet used by this encoding contains mixed
	 * upper- and lower-case characters
	 */
	...
	private static final class Alphabet {
		...
		Alphabet(String name, char[] chars) {
			...
			try {
				bitsPerChar = IntMath.log2(chars.length, UNNECESSARY);
			}
			...
			byte[] decodabet = new byte[org.magneton.core.base.Ascii.MAX + 1];
		...
		private boolean hasLowerCase() {
			for (char c : chars) {
				if (org.magneton.core.base.Ascii.isLowerCase(c)) {
					return true;
				}
			}
			return false;
		...
		private boolean hasUpperCase() {
			for (char c : chars) {
				if (org.magneton.core.base.Ascii.isUpperCase(c)) {
					return true;
				}
			}
			return false;
		...
		Alphabet upperCase() {
			...
			for (int i = 0; i < chars.length; i++) {
				upperCased[i] = org.magneton.core.base.Ascii.toUpperCase(chars[i]);
			}
	...
	static class StandardBaseEncoding extends BaseEncoding {
		...
		public InputStream decodingStream(Reader reader) {
			...
			return new InputStream() {
				...
				public int read(byte[] buf, int off, int len) throws IOException {
					// Overriding this to work around the fact that InputStream's default
					// implementation of
					// this method will silently swallow exceptions thrown by the
					// single-byte read() method
					// (other than on the first call to it), which in this case can cause
					// invalid encoded
		...
		public BaseEncoding withSeparator(String separator, int afterEveryChars) {
			for (int i = 0; i < separator.length(); i++) {
				Preconditions.checkArgument(!alphabet.matches(separator.charAt(i)),
						"Separator (%s) cannot contain alphabet characters", separator);
			}
			if (paddingChar != null) {
				Preconditions.checkArgument(separator.indexOf(paddingChar.charValue()) < 0,
		...
		public boolean equals(@CheckForNull Object other) {
			if (other instanceof StandardBaseEncoding) {
				...
				return alphabet.equals(that.alphabet)
						&& org.magneton.core.base.Objects.equal(paddingChar, that.paddingChar);
			}
			return false;
		}

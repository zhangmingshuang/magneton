...
final class CharSequenceReader extends Reader {
	...
	/*
	 * To avoid the need to call requireNonNull so much, we could consider more clever
	 * approaches, such as:
	 *
	 * - Make checkOpen return the non-null `seq`. Then callers can assign that to a local
	 * variable or even back to `this.seq`. However, that may suggest that we're defending
	 * against concurrent mutation, which is not an actual risk because we use
	 * `synchronized`. - Make `remaining` require a non-null `seq` argument. But this is a
	 * bit weird because the method, while it would avoid the instance field `seq` would
	 * still access the instance field `pos`.
	 */
	...
	public synchronized long skip(long n) throws IOException {
		...
		checkOpen();
		int charsToSkip = (int) Math.min(remaining(), n); // safe because remaining is an
															// int
		pos += charsToSkip;
		return charsToSkip;
	}

...
public final class CharStreams {
	...
	public static long copy(Readable from, Appendable to) throws IOException {
		// The most common case is that from is a Reader (like InputStreamReader or
		// StringReader) so
	...
	// TODO(lukes): consider allowing callers to pass in a buffer to use, some callers
	// would be able
	// to reuse buffers, others would be able to size them more appropriately than the
	// constant
	// defaults

	/**
	 * Copies all characters between the {@link Reader} and {@link StringBuilder} objects.
	 * Does not close or flush the reader.
	 *
	 * <p>
	 * This is identical to {@link #copy(Readable, Appendable)} but optimized for these
	 * specific types. CharBuffer has poor performance when being written into or read out
	 * of so round tripping all the bytes through the buffer takes a long time. With these
	 * specialized types we can just use a char array.
	 * @param from the object to read from
	 * @param to the object to write to
	 * @return the number of characters copied
	 * @throws IOException if an I/O error occurs
	...
	/**
	 * Copies all characters between the {@link Reader} and {@link Writer} objects. Does
	 * not close or flush the reader or writer.
	 *
	 * <p>
	 * This is identical to {@link #copy(Readable, Appendable)} but optimized for these
	 * specific types. CharBuffer has poor performance when being written into or read out
	...
	/**
	 * Reads all characters from a {@link Readable} object into a {@link String}. Does not
	 * close the {@code Readable}.
	 ...
	 *
	 * <p>
	 * Does not close the {@code Readable}. If reading files or resources you should use
	 * the {@link Files#readLines} and {@link Resources#readLines} methods.
	 * @param r the object to read from
	...
	/**
	 * Streams lines from a {@link Readable} object, stopping when the processor returns
	 * {@code false} or all lines have been read and returning the result produced by the
	 * processor. Does not close {@code readable}. Note that this method may not fully
	 * consume the contents of {@code readable} if the processor stops processing early.
	 * @throws IOException if an I/O error occurs
	 * @since 14.0
	 */
	...
	/**
	 * Reads and discards data from the given {@code Readable} until the end of the stream
	 * is reached. Returns the total number of chars read. Does not close the stream.
	 *
	 * @since 20.0
	 */
	...
	/**
	 * Discards {@code n} characters of data from the reader. This method will block until
	 * the full amount has been skipped. Does not close the reader.
	 ...
	 * Closing the writer will close the target if it is {@link Closeable}, and flushing
	 * the writer will flush the target if it is {@link java.io.Flushable}.
	 * @param target the object to which output will be sent
	 * @return a new Writer object, unless target is a Writer, in which case the target is
	 * returned
	 */
	...
	public static Writer asWriter(Appendable target) {
		...
		return new org.magneton.core.io.AppendableWriter(target);
	}

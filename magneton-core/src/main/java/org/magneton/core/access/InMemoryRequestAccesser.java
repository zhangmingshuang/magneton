...
public class InMemoryRequestAccesser<T> extends AbstractAccesser implements RequestAccesser<T> {
  ...
  protected void initialiazation() {
    this.locked =
        CacheBuilder.newBuilder()
            ...
            .maximumSize(super.accessConfig.getLockSize())
            .build();
    this.errorRcords =
        CacheBuilder.newBuilder()
            .expireAfterAccess(super.accessConfig.getErrorRecordTime(), TimeUnit.MILLISECONDS)
            .maximumSize(super.accessConfig.getErrorRecordSize())
            .build();
  }
  ...
  public long ttl(String key) {
    @Nullable Long ttl = this.locked.getIfPresent(Preconditions.checkNotNull(key));

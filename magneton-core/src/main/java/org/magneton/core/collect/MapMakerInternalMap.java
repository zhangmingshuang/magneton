...
class MapMakerInternalMap<K, V, E extends MapMakerInternalMap.InternalEntry<K, V, E>, S extends MapMakerInternalMap.Segment<K, V, E, S>>
	 ...
	 * the constructors with arguments. MUST be a power of two no greater than
	 * {@code 1<<30} to ensure that entries are indexable using ints.
	 */
	static final int MAXIMUM_CAPACITY = org.magneton.core.primitives.Ints.MAX_POWER_OF_TWO;
	...
	/**
	 * A singleton {@link WeakValueReference} used to denote an unset value in a entry
	 * with weak values.
	 */
	...
	final org.magneton.core.base.Equivalence<Object> keyEquivalence;
	...
	private MapMakerInternalMap(MapMaker builder, InternalEntryHelper<K, V, E, S> entryHelper) {
		...
		for (int i = 0; i < segments.length; ++i) {
			segments[i] = createSegment(segmentSize, MapMaker.UNSET_INT);
		}
	}

	/*
	 * Note: the following classes have a lot of duplicate code. It sucks, but it saves a
	 * lot of memory. If only Java had mixins!
	 */

	/**
	 * Returns a fresh {@link MapMakerInternalMap} as specified by the given
	 * {@code builder}.
	 */
	...
	/**
	 * Returns a fresh {@link MapMakerInternalMap} with {@link MapMaker.Dummy} values but
	 * otherwise as specified by the given {@code builder}. The returned
	 * {@link MapMakerInternalMap} will be optimized to saved memory. Since
	 * {@link MapMaker.Dummy} is a singleton, we don't need to store any values at all.
	 * Because of this optimization, {@code build.getValueStrength()} must be
	 * {@link Strength#STRONG}.
	 *
	 * <p>
	 * This method is intended to only be used by the internal implementation of
	 * {@link Interners}, since a map of dummy values is the exact use case there.
	 */
	...
	enum Strength {
		...
		STRONG {
			@Override
			org.magneton.core.base.Equivalence<Object> defaultEquivalence() {
				return org.magneton.core.base.Equivalence.equals();
			}
		},
		...
		WEAK {
			@Override
			org.magneton.core.base.Equivalence<Object> defaultEquivalence() {
				return org.magneton.core.base.Equivalence.identity();
			}
		};
		 ...
		 */
		abstract org.magneton.core.base.Equivalence<Object> defaultEquivalence();
	...
	/**
	 * A helper object for operating on {@link InternalEntry} instances in a type-safe and
	 * efficient manner.
	 *
	 * <p>
	 * For each of the four combinations of strong/weak key and strong/weak value, there
	 * are corresponding {@link InternalEntry}, {@link Segment}, and
	 * {@link InternalEntryHelper} implementations.
	 *
	...
	interface InternalEntryHelper<K, V, E extends InternalEntry<K, V, E>, S extends Segment<K, V, E, S>> {
		...
		/**
		 * Sets the value of the given {@code entry} in the given {@code segment} to be
		 * the given {@code
		 * value}
		 */
	...
	interface WeakValueEntry<K, V, E extends InternalEntry<K, V, E>> extends InternalEntry<K, V, E> {
		...
		/**
		 * Clears the weak value reference held by the entry. Should be used when the
		 * entry's value is overwritten.
		 */
		void clearValue();
	...
	interface WeakValueReference<K, V, E extends InternalEntry<K, V, E>> {
		...
		/**
		 * Returns the current value being referenced, or {@code null} if there is none
		 * (e.g. because either it got collected, or {@link #clear} was called, or it
		...
		/**
		 * Unsets the referenced value. Subsequent calls to {@link #get} will return
		 * {@code null}.
		 */
		void clear();

		/**
		 * Returns a freshly created {@link WeakValueReference} for the given
		 * {@code entry} (and on the given {@code queue} with the same value as this
		 * {@link WeakValueReference}.
		 */
	...
	static final class StrongKeyStrongValueEntry<K, V>
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for strong keys and
		 * strong values.
		 */
	...
	static final class StrongKeyWeakValueEntry<K, V> extends AbstractStrongKeyEntry<K, V, StrongKeyWeakValueEntry<K, V>>
		...
		private volatile WeakValueReference<K, V, StrongKeyWeakValueEntry<K, V>> valueReference = unsetWeakValueReference();
		...
		void setValue(V value, ReferenceQueue<V> queueForValues) {
			WeakValueReference<K, V, StrongKeyWeakValueEntry<K, V>> previous = valueReference;
			valueReference = new WeakValueReferenceImpl<>(queueForValues, value, this);
			previous.clear();
		}
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for strong keys and weak
		 * values.
		 */
	...
	/**
	 * Concrete implementation of {@link InternalEntry} for strong keys and
	 * {@link MapMaker.Dummy} values.
	 */
	static final class StrongKeyDummyValueEntry<K>
		...
		StrongKeyDummyValueEntry<K> copy(StrongKeyDummyValueEntry<K> newNext) {
			return new StrongKeyDummyValueEntry<K>(key, hash, newNext);
		}
		 ...
		 * {@link MapMaker.Dummy} values.
		 */
		static final class Helper<K> implements
			...
			public StrongKeyDummyValueEntry<K> copy(StrongKeyDummyValueSegment<K> segment,
					StrongKeyDummyValueEntry<K> entry, @Nullable StrongKeyDummyValueEntry<K> newNext) {
				return entry.copy(newNext);
			}
	...
	/**
	 * Concrete implementation of {@link InternalEntry} for weak keys and
	 * {@link MapMaker.Dummy} values.
	 */
	static final class WeakKeyDummyValueEntry<K>
		...
		WeakKeyDummyValueEntry<K> copy(ReferenceQueue<K> queueForKeys, WeakKeyDummyValueEntry<K> newNext) {
			return new WeakKeyDummyValueEntry<K>(queueForKeys, getKey(), hash, newNext);
		}

		/**
		 * Concrete implementation of {@link InternalEntryHelper} for weak keys and
		 * {@link MapMaker.Dummy} values.
		 */
		static final class Helper<K> implements
	...
	static final class WeakKeyStrongValueEntry<K, V> extends AbstractWeakKeyEntry<K, V, WeakKeyStrongValueEntry<K, V>>
		...
		WeakKeyStrongValueEntry<K, V> copy(ReferenceQueue<K> queueForKeys, WeakKeyStrongValueEntry<K, V> newNext) {
			WeakKeyStrongValueEntry<K, V> newEntry = new WeakKeyStrongValueEntry<>(queueForKeys, getKey(), hash,
					newNext);
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for weak keys and strong
		 * values.
		 */
		static final class Helper<K, V>
	...
	static final class WeakKeyWeakValueEntry<K, V> extends AbstractWeakKeyEntry<K, V, WeakKeyWeakValueEntry<K, V>>
		...
		private volatile WeakValueReference<K, V, WeakKeyWeakValueEntry<K, V>> valueReference = unsetWeakValueReference();
		...
		void setValue(V value, ReferenceQueue<V> queueForValues) {
			WeakValueReference<K, V, WeakKeyWeakValueEntry<K, V>> previous = valueReference;
			valueReference = new WeakValueReferenceImpl<>(queueForValues, value, this);
			previous.clear();
		}
		...
		/**
		 * Concrete implementation of {@link InternalEntryHelper} for weak keys and weak
		 * values.
		 */
		static final class Helper<K, V>
	...
	/**
	 * A dummy implementation of {@link InternalEntry}, solely for use in the type
	 * signature of {@link #UNSET_WEAK_VALUE_REFERENCE} below.
	 */
	...
	abstract static class Segment<K, V, E extends InternalEntry<K, V, E>, S extends Segment<K, V, E, S>>
		...
		/**
		 * The number of live elements in this segment's region. This does not include
		 * unset elements which are awaiting cleanup.
		 */
		volatile int count;
		...
		/**
		 * Returns {@code this} up-casted to the specific {@link Segment} implementation
		 * type {@code S}.
		 *
		 * <p>
		 * This method exists so that the {@link Segment} code can be generic in terms of
		 * {@code S}, the type of the concrete implementation.
		 */
		abstract S self();
		...
		/**
		 * Unsafely sets the weak value reference inside the given {@code entry} to be the
		 * given {@code
		 * valueReference}
		 */
		...
		/**
		 * Unsafely sets the given index of this segment's internal hash table to be the
		 * given entry.
		 */
		...
		/**
		 * Unsafely removes the given entry from the given chain in this segment's hash
		 * table.
		 */
		...
		/**
		 * Removes an entry from within a table. All entries following the removed node
		 * can stay, but all preceding ones need to be cloned.
		 *
		 * <p>
		 * This method does not decrement count for the removed entry, but does decrement
		 * count for all partially collected entries which are skipped. As such callers
		 * which are modifying count must re-read it after calling removeFromChain.
		...
		/**
		 * Clears a value that has not yet been set, and thus does not require count to be
		 * modified.
		 */
		@CanIgnoreReturnValue
		 ...
		 * writes, or from the cleanupExecutor. If cleanup is not observed after a
		 * sufficient number of reads, try cleaning up from the read thread.
		 */
	...
	static final class StrongKeyWeakValueSegment<K, V>
		...
		public void setWeakValueReferenceForTesting(InternalEntry<K, V, ?> e,
			...
			WeakValueReference<K, V, StrongKeyWeakValueEntry<K, V>> newValueReference = (WeakValueReference<K, V, StrongKeyWeakValueEntry<K, V>>) valueReference;
	...
	/**
	 * Concrete implementation of {@link Segment} for strong keys and
	 * {@link MapMaker.Dummy} values.
	 */
	...
	static final class WeakKeyWeakValueSegment<K, V>
		...
		public void setWeakValueReferenceForTesting(InternalEntry<K, V, ?> e,
			...
			WeakValueReference<K, V, WeakKeyWeakValueEntry<K, V>> newValueReference = (WeakValueReference<K, V, WeakKeyWeakValueEntry<K, V>>) valueReference;
	...
	/**
	 * Concrete implementation of {@link Segment} for weak keys and {@link MapMaker.Dummy}
	 * values.
	 */
	...
	static final class CleanupMapTask implements Runnable {
		...
		public CleanupMapTask(MapMakerInternalMap<?, ?, ?, ?> map) {
			mapReference = new WeakReference<MapMakerInternalMap<?, ?, ?, ?>>(map);
		}
	...
	/**
	 * The actual object that gets serialized. Unfortunately, readResolve() doesn't get
	 * called when a circular dependency is present, so the proxy must be able to behave
	 * as the map itself.
	 */
	abstract static class AbstractSerializationProxy<K, V> extends ForwardingConcurrentMap<K, V>
		...
		final org.magneton.core.base.Equivalence<Object> valueEquivalence;
		...
		AbstractSerializationProxy(Strength keyStrength, Strength valueStrength,
				org.magneton.core.base.Equivalence<Object> keyEquivalence,
				org.magneton.core.base.Equivalence<Object> valueEquivalence, int concurrencyLevel,
	...
	private static final class SerializationProxy<K, V> extends AbstractSerializationProxy<K, V> {
		...
		SerializationProxy(Strength keyStrength, Strength valueStrength,
				org.magneton.core.base.Equivalence<Object> keyEquivalence, Equivalence<Object> valueEquivalence,
				int concurrencyLevel, ConcurrentMap<K, V> delegate) {
	...
	abstract class HashIterator<T> implements Iterator<T> {
		...
		/**
		 * Finds the next entry in the current chain. Returns {@code true} if an entry was
		 * found.
		 */
		boolean nextInChain() {
			if (nextEntry != null) {
		...
		/**
		 * Finds the next entry in the current table. Returns {@code true} if an entry was
		 * found.
		 */
		boolean nextInTable() {
		...
		/**
		 * Advances to the given entry. Returns {@code true} if the entry was valid,
	 ...
	 * the underlying map.
	 */
	final class WriteThroughEntry extends org.magneton.core.collect.AbstractMapEntry<K, V> {

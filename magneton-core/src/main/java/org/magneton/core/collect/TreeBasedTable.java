...
/**
 * Implementation of {@code Table} whose row keys and column keys are ordered by their
 * natural ordering or by supplied comparators. When constructing a
 * {@code TreeBasedTable}, you may provide comparators for the row keys and the column
 * keys, or you may use natural ordering for both.
 *
 * <p>
 * The {@link #rowKeySet} method returns a {@link SortedSet} and the {@link #rowMap}
 * method returns a {@link SortedMap}, instead of the {@link Set} and {@link Map}
 * specified by the {@link Table} interface.
 *
 * <p>
 ...
 * row(rowKey)} and {@code rowMap().get(rowKey)} are {@link SortedMap} instances, instead
 * of the {@link Map} specified in the {@link Table} interface.
 *
 * <p>
...
public class TreeBasedTable<R, C, V> extends StandardRowSortedTable<R, C, V> {
	...
	/**
	 * Creates an empty {@code TreeBasedTable} that uses the natural orderings of both row
	 * and column keys.
	 *
	 * <p>
	 * The method signature specifies {@code R extends Comparable} with a raw
	 * {@link Comparable}, instead of {@code R extends Comparable<? super R>}, and the
	 * same for {@code C}. That's necessary to support classes defined without generics.
	 */
	...
	public Comparator<? super R> rowComparator() {
		/*
		 * requireNonNull is safe because the factories require non-null Comparators,
		 * which they pass on to the backing collections.
		 */
	 ...
	 * {@link Ordering#natural()} is returned.
	 * @deprecated Store the {@link Comparator} alongside the {@link Table}. Or, if you
	 * know that the {@link Table} contains at least one value, you can retrieve the
	 * {@link Comparator} with:
	 ...
	 *
	 * <p>
	 * Because a {@code TreeBasedTable} has unique sorted values for a given row, this
	 * method returns a {@link SortedMap}, instead of the {@link Map} specified in the
	...
	Iterator<C> createColumnKeyIterator() {
		...
		Iterator<C> merged = Iterators.mergeSorted(
				Iterables.transform(backingMap.values(), (Map<C, V> input) -> input.keySet().iterator()), comparator);
	...
	private class TreeRow extends Row implements SortedMap<C, V> {
		...
		public SortedMap<C, V> subMap(C fromKey, C toKey) {
			Preconditions.checkArgument(rangeContains(Preconditions.checkNotNull(fromKey))
					&& rangeContains(Preconditions.checkNotNull(toKey)));
		...
		// If the row was previously empty, we check if there's a new row here every time
		// we're queried.

...
/**
 * An assortment of mainly legacy static utility methods that operate on or return objects
 ...
 *
 * <p>
 * <b>Java 8 users:</b> several common uses for this class are now more comprehensively
 * addressed by the new {@link Stream} library. Read the method documentation below for
 * comparisons. This class is not being deprecated, but we gently encourage you to migrate
 * to streams.
 *
 * <p>
 * <i>Performance notes:</i> Unless otherwise noted, all of the iterables produced in this
 * class are <i>lazy</i>, which means that their iterators only advance the backing
 * iteration when absolutely necessary.
 *
 * <p>
...
public final class Iterables {
	 ...
	 * {@code
	 * Objects.equals(o, element)} would return {@code true}. Otherwise returns
	 * {@code false}, even in cases where {@link Collection#contains} might throw
	 * {@link NullPointerException} or {@link ClassCastException}.
	 */
	// <? extends Object> instead of <?> because of Kotlin b/189937072,
	// discussed in Joiner.
	public static boolean contains(Iterable<? extends Object> iterable, @CheckForNull Object element) {
		if (iterable instanceof Collection) {
			Collection<?> collection = (Collection<?>) iterable;
			return org.magneton.core.collect.Collections2.safeContains(collection, element);
		}
	 ...
	 *
	 * <p>
	 * This method calls {@link Collection#removeAll} if {@code iterable} is a collection,
	 * and {@link Iterators#removeAll} otherwise.
	 ...
	 *
	 * <p>
	 * This method calls {@link Collection#retainAll} if {@code iterable} is a collection,
	 * and {@link Iterators#retainAll} otherwise.
	 * @param removeFrom the iterable to (potentially) remove elements from
	 ...
	 *
	 * <p>
	 * Removals may or may not happen immediately as each element is tested against the
	 * predicate. The behavior of this method is not specified if {@code predicate} is
	 * dependent on {@code
	 * removeFrom}.
	 *
	 * <p>
	 * <b>Java 8 users:</b> if {@code removeFrom} is a {@link Collection}, use {@code
	 ...
	 */
	@CanIgnoreReturnValue
	public static <T> boolean removeIf(Iterable<T> removeFrom, org.magneton.core.base.Predicate<? super T> predicate) {
	...
	/**
	 * Removes and returns the first matching element, or returns {@code null} if there is
	 * none.
	 */
	@CheckForNull
	static <T> T removeFirstMatching(Iterable<T> removeFrom, org.magneton.core.base.Predicate<? super T> predicate) {
	...
	/**
	 * Determines whether two iterables contain equal elements in the same order. More
	 ...
	 * Arrays}{@code
	 * .toString(Iterables.toArray(iterable))}). Note that for <i>most</i> implementations
	 * of {@link Collection}, {@code collection.toString()} also gives the same result,
	 * but that behavior is not generally guaranteed.
	 */
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> the {@code Stream} equivalent to this method is {@code
	...
	/**
	 * Returns the single element contained in {@code iterable}, or {@code defaultValue}
	 ...
	 * @return a newly-allocated array into which all the elements of the iterable have
	 * been copied
	 */
	/*
	 * If we could express Class<@Nonnull T>, we could generalize the type parameter to <T
	 * extends
	 *
	 * @Nullable Object>, and then we could accept an Iterable<? extends T> and return a
	 * plain T[] instead of a @Nullable T[].
	 */
	...
	/**
	 * Converts an iterable into a collection. If the iterable is already a collection, it
	 * is returned. Otherwise, an {@link java.util.ArrayList} is created with the contents
	 * of the iterable in the same iteration order.
	 */
	...
	/**
	 * Returns the number of elements in the specified iterable that equal the specified
	 * object. This implementation avoids a full iteration when the iterable is a
	 * {@link Multiset} or {@link Set}.
	 *
	 * <p>
	 * <b>Java 8 users:</b> In most cases, the {@code Stream} equivalent of this method is
	 * {@code
	 * stream.filter(element::equals).count()}. If {@code element} might be null, use
	 * {@code
	 ...
	 *
	 * <p>
	 * To cycle over the iterable {@code n} times, use the following: {@code
	 ...
	 *
	 * <p>
	 * After {@code remove} is invoked on a generated iterator, the removed element will
	 * no longer appear in either that iterator or any other iterator created from the
	 * same source iterable. That is, this method behaves exactly as
	 * {@code Iterables.cycle(Lists.newArrayList(elements))}. The iterator's
	 * {@code hasNext} method returns {@code true} until all of the original elements have
	 * been removed.
	 *
	 * <p>
	 * <b>Warning:</b> Typical uses of the resulting iterator may produce an infinite
	 ...
	 *
	 * <p>
	 * To cycle over the elements {@code n} times, use the following: {@code
	 * Iterables.concat(Collections.nCopies(n, Arrays.asList(elements)))}
	 *
	 * <p>
	 * <b>Java 8 users:</b> If passing a single element {@code e}, the {@code Stream}
	 * equivalent of this method is {@code Stream.generate(() -> e)}. Otherwise, put the
	 * elements in a collection and use
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> The {@code Stream} equivalent of this method is
	 * {@code Stream.concat(a,
	 * b)}.
	 */
	...
	/**
	 * Combines multiple iterables into a single iterable. The returned iterable has an
	 * iterator that traverses the elements of each iterable in {@code inputs}. The input
	 ...
	 * {@code [[a, b, c], [d, e]]} -- an outer iterable containing two inner lists of
	...
	/**
	 * Divides an iterable into unmodifiable sublists of the given size, padding the final
	 * iterable with null values if necessary. For example, partitioning an iterable
	 * containing {@code [a, b,
	 * c, d, e]} with a partition size of 3 yields {@code [[a, b, c], [d, e, null]]} -- an
	 * outer iterable containing two inner lists of three elements each, all in the
	 * original order.
	 *
	 * <p>
	 * Iterators returned by the returned iterable do not support the
	 ...
	 * @return an iterable of unmodifiable lists containing the elements of
	 * {@code iterable} divided into partitions (the final iterable may have trailing null
	 * elements)
	 * @throws IllegalArgumentException if {@code size} is nonpositive
	 */
	 ...
	 * predicate {@code retainIfTrue}. The returned iterable's iterator does not support
	 * {@code remove()}.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Stream#filter}.
	 */
	public static <T> Iterable<T> filter(Iterable<T> unfiltered,
		...
		return new FluentIterable<T>() {
			...
			public void forEach(Consumer<? super T> action) {
				...
				unfiltered.forEach((@ParametricNullness T a) -> {
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Stream#anyMatch}.
	 */
	public static <T> boolean any(Iterable<T> iterable, org.magneton.core.base.Predicate<? super T> predicate) {
	 ...
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Stream#allMatch}.
	 */
	public static <T> boolean all(Iterable<T> iterable, org.magneton.core.base.Predicate<? super T> predicate) {
		return Iterators.all(iterable.iterator(), predicate);
	}

	/**
	 * Returns the first element in {@code iterable} that satisfies the given predicate;
	 * use this method only when such an element is known to exist. If it is possible that
	 * <i>no</i> element will match, use {@link #tryFind} or
	 * {@link #find(Iterable, org.magneton.core.base.Predicate, Object)} instead.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code stream.filter(predicate).findFirst().get()}
	 * @throws NoSuchElementException if no element in {@code iterable} matches the given
	 * predicate
	 */
	@ParametricNullness
	public static <T> T find(Iterable<T> iterable, org.magneton.core.base.Predicate<? super T> predicate) {
	 ...
	 * Returns the first element in {@code iterable} that satisfies the given predicate,
	 * or {@code
	 * defaultValue} if none found. Note that this can usually be handled more naturally
	 * using {@code
	...
	// (That would be unsound for an input Iterable<@Nullable Foo>. So, if we wanted to
	// use
	// @PolyNull, we would have to restrict this method to non-null <T>. But it has users
	// who pass
	// iterables with null elements.)
	//
	// - @JointlyNullable means "@Nullable or no annotation"
	@CheckForNull
	public static <T> T find(Iterable<? extends T> iterable, org.magneton.core.base.Predicate<? super T> predicate,
	 ...
	 * {@code iterable} that satisfies the given predicate, if such an element exists.
	 *
	 * <p>
	 * <b>Warning:</b> avoid using a {@code predicate} that matches {@code null}. If
	 * {@code null} is matched in {@code iterable}, a NullPointerException will be thrown.
	 *
	 * <p>
	 ...
	 * @since 11.0
	 */
	public static <T> Optional<T> tryFind(Iterable<T> iterable, org.magneton.core.base.Predicate<? super T> predicate) {
	 ...
	 * predicate.apply(Iterables.get(iterable, i))} returns {@code true}, or {@code -1} if
	 ...
	 * {@code fromIterable}'s iterator does. After a successful {@code remove()} call,
	 ...
	 */
	public static <F extends Object, T extends Object> Iterable<T> transform(Iterable<F> fromIterable,
			org.magneton.core.base.Function<? super F, ? extends T> function) {
	 ...
	 * or equal to the size of {@code iterable}
	 */
	@ParametricNullness
	...
	/**
	 * Returns the element at the specified position in an iterable or a default value
	 * otherwise.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@code
	 * stream.skip(position).findFirst().orElse(defaultValue)} (returns the default value
	 * if the index is out of bounds)
	 * @param position position of the element to return
	 * @param defaultValue the default value to return if {@code position} is greater than
	 * or equal to the size of the iterable
	 * @return the element at the specified position in {@code iterable} or
	 * {@code defaultValue} if {@code iterable} contains fewer than {@code position + 1}
	 * elements.
	 ...
	 * {@link NoSuchElementException} to be thrown), it is recommended that {@code
	 * iterable.iterator().next()} is used instead.
	 *
	 * <p>
	 * To get the only element in a single-element {@code Iterable}, consider using
	 * {@link #getOnlyElement(Iterable)} or {@link #getOnlyElement(Iterable, Object)}
	 * instead.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@code stream.findFirst().orElse(defaultValue)}
	...
	/**
	 * Returns the last element of {@code iterable}. If {@code iterable} is a {@link List}
	 * with {@link RandomAccess} support, then this operation is guaranteed to be
	 * {@code O(1)}.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Streams#findLast
	 * Streams.findLast(stream).get()}
	 ...
	 * iterable is empty. If {@code iterable} is a {@link List} with {@link RandomAccess}
	 * support, then this operation is guaranteed to be {@code O(1)}.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b>
	 * {@code Streams.findLast(stream).orElse(defaultValue)}
	 ...
	 * elements. If {@code iterable} contains fewer than {@code numberToSkip} elements,
	 * the returned iterable skips all of its elements.
	 *
	 * <p>
	 * Modifications to the underlying {@link Iterable} before a call to
	 * {@code iterator()} are reflected in the returned iterator. That is, the iterator
	 * skips the first {@code numberToSkip} elements that exist when the {@code Iterator}
	 * is created, not when {@code skip()} is called.
	 *
	 * <p>
	 * The returned iterable's iterator supports {@code remove()} if the iterator of the
	 * underlying iterable supports it. Note that it is <i>not</i> possible to delete the
	 * last skipped element by immediately calling {@code remove()} on that iterator, as
	 * the {@code Iterator} contract states that a call to {@code remove()} before a call
	 * to {@code next()} will throw an {@link IllegalStateException}.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Stream#skip}
	 *
	 * @since 3.0
	 */
	 ...
	 * If {@code
	 * iterable} contains fewer than {@code limitSize} elements, the returned view
	 * contains all of its elements. The returned iterable's iterator supports
	 * {@code remove()} if {@code iterable}'s iterator does.
	 *
	 * <p>
	 * <b>{@code Stream} equivalent:</b> {@link Stream#limit}
	...
	/**
	 * Returns a view of the supplied iterable that wraps each generated {@link Iterator}
	 ...
	 * undefined. Calling {@link Iterator#hasNext()} on a generated iterator from the
	 * returned iterable may cause an item to be immediately dequeued for return on a
	 * subsequent call to {@link Iterator#next()}.
	 * @param iterable the iterable to wrap
	 * @return a view of the supplied iterable that wraps each generated iterator through
	 * {@link Iterators#consumingIterator(Iterator)}; for queues, an iterable that
	 * generates iterators that return and consume the queue's elements in queue order
	 ...
	 *
	 * <p>
	 * There is no precise {@link Iterator} equivalent to this method, since one can only
	 * ask an iterator whether it has any elements <i>remaining</i> (which one does using
	 ...
	 *
	 * <p>
	 * Callers must ensure that the source {@code iterables} are in non-descending order
	 * as this method does not sort its input.
	 *
	 * <p>
	 * For any equivalent elements across all {@code iterables}, it is undefined which
	...
	static <T> org.magneton.core.base.Function<Iterable<? extends T>, Iterator<? extends T>> toIterator() {
	...
	private static final class UnmodifiableIterable<T> extends FluentIterable<T> {
		...
		// safe upcast, assuming no one has a crazy
		// Spliterator subclass
		@Override
		public Spliterator<T> spliterator() {

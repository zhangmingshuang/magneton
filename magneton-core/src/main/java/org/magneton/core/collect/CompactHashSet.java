 ...
 *
 * <p>
 * {@code contains(x)}, {@code add(x)} and {@code remove(x)}, are all (expected and
 ...
 *
 * <p>
 * Unlike {@code java.util.HashSet}, iteration is only proportional to the actual
 * {@code size()}, which is optimal, and <i>not</i> the size of the internal hashtable,
 * which could be much larger than {@code size()}. Furthermore, this structure only
 * depends on a fixed number of arrays; {@code
 * add(x)} operations <i>do not</i> create objects for the garbage collector to deal with,
 * and for every element added, the garbage collector will have to traverse {@code 1.5}
 * references on average, in the marking phase, not {@code 5.0} as in
 * {@code java.util.HashSet}.
 *
 * <p>
 * If there are no removals, then {@link #iterator iteration} order is the same as
 * insertion order. Any removal invalidates any ordering guarantees.
 *
 * <p>
 * This class should not be assumed to be universally superior to
 * {@code java.util.HashSet}. Generally speaking, this class reduces object allocation and
...
class CompactHashSet<E extends Object> extends AbstractSet<E> implements Serializable {
	 ...
	 * j.u.LinkedHashSet based implementation. Experimentally determined.
	 */
	private static final int MAX_HASH_BUCKET_LENGTH = 9;

	/**
	 * The elements contained in the set, in the range of [0, size()). The elements in
	 * [size(), elements.length) are all {@code null}.
	 */
	@VisibleForTesting
	@CheckForNull
	 ...
	 * </ul>
	 * <li>another java.util.Set delegate implementation. In most modern JDKs, normal
	 ...
	 * int are the part of the smeared hash of the element not covered by the hashtable
	 * mask, whereas the low bits are the "next" pointer (pointing to the next entry in
	 * the bucket chain), which will always be less than or equal to the hashtable mask.
	 *
	 * <pre>
	 * hash  = aaaaaaaa
	 * mask  = 00000fff
	...
	// See CompactHashMap for a detailed description of how the following fields work.
	// That
	// description talks about `keys`, `values`, and `entries`; here the `keys` and
	// `values` arrays
	...
	/**
	 * Creates a <i>mutable</i> {@code CompactHashSet} instance containing the elements of
	 * the given collection in unspecified order.
	 * @param collection the elements that the set should contain
	 * @return a new {@code CompactHashSet} containing those elements (minus duplicates)
	 */
	public static <E extends Object> CompactHashSet<E> create(Collection<? extends E> collection) {
	 ...
	 * @return a new, empty {@code CompactHashSet} with enough capacity to hold
	 * {@code expectedSize} elements without resizing
	 * @throws IllegalArgumentException if {@code expectedSize} is negative
	 */
	public static <E extends Object> CompactHashSet<E> createWithExpectedSize(int expectedSize) {
		...
		if (next == CompactHashing.UNSET) { // uninitialized bucket
			...
			do {
				...
				if (CompactHashing.getHashPrefix(entry, mask) == hashPrefix
						&& org.magneton.core.base.Objects.equal(object, elements[entryIndex])) {
					return false;
				}
	...
	void resizeEntries(int newCapacity) {
		...
		elements = Arrays.copyOf(requireElements(), newCapacity);
	}
	...
	public boolean remove(@CheckForNull Object object) {
		...
		int mask = hashTableMask();
		int index = CompactHashing.remove(object, /* value= */ null, mask, requireTable(), requireEntries(),
				requireElements(), /* values= */ null);
		if (index == -1) {
			return false;
		}
	...
	public Spliterator<E> spliterator() {
		...
		Set<E> delegate = delegateOrNull();
		return (delegate != null) ? delegate.spliterator()
				: Spliterators.spliterator(requireElements(), 0, size, Spliterator.DISTINCT | Spliterator.ORDERED);
	}
	...
	public <T> T[] toArray(T[] a) {
		...
		Set<E> delegate = delegateOrNull();
		return (delegate != null) ? delegate.toArray(a) : ObjectArrays.toArrayImpl(requireElements(), 0, size, a);
	}
	...
	/*
	 * For discussion of the safety of the following methods, see the comments near the
	 * end of CompactHashMap.
	 */

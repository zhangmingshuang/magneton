 ...
 * natural order or an explicit {@link Comparator}. In all cases, this implementation uses
 * {@link Comparable#compareTo} or {@link Comparator#compare} instead of
 * {@link Object#equals} to determine equivalence of instances.
 *
 * <p>
 * <b>Warning:</b> The comparison must be <i>consistent with equals</i> as explained by
 * the {@link Comparable} class specification. Otherwise, the resulting multiset will
 * violate the {@link java.util.Collection} contract, which is specified in terms of
 * {@link Object#equals}.
 *
 * <p>
...
public final class TreeMultiset<E extends Object> extends AbstractSortedMultiset<E> implements Serializable {
	...
	/**
	 * Creates a new, empty multiset, sorted according to the elements' natural order. All
	 * elements inserted into the multiset must implement the {@code Comparable}
	 ...
	 * elements {@code e1} and {@code e2} in the multiset. If the user attempts to add an
	 * element to the multiset that violates this constraint (for example, the user
	 * attempts to add a string element to a set whose elements are integers), the {@code
	 ...
	 * specific {@code <E extends Comparable<? super E>>}, to support classes defined
	 * without generics.
	 */
	...
	/**
	 * Creates a new, empty multiset, sorted according to the specified comparator. All
	 * elements inserted into the multiset must be <i>mutually comparable</i> by the
	 ...
	 * multiset. If the user attempts to add an element to the multiset that violates this
	 * constraint, the {@code add(Object)} call will throw a {@code
	 * ClassCastException}.
	 * @param comparator the comparator that will be used to sort this multiset. A null
	 * value indicates that the elements' <i>natural ordering</i> should be used.
	 */
	public static <E extends Object> TreeMultiset<E> create(@CheckForNull Comparator<? super E> comparator) {
		return (comparator == null) ? new TreeMultiset<E>((Comparator) Ordering.natural())
				: new TreeMultiset<E>(comparator);
	}

	/**
	 * Creates an empty multiset containing the given initial elements, sorted according
	...
	private long aggregateBelowRange(Aggregate aggr, @CheckForNull AvlNode<E> node) {
		...
		int cmp = comparator().compare(NullnessCasts.uncheckedCastNullableTToT(range.getLowerEndpoint()),
				node.getElement());
		if (cmp < 0) {
	...
	private long aggregateAboveRange(Aggregate aggr, @CheckForNull AvlNode<E> node) {
		...
		int cmp = comparator().compare(NullnessCasts.uncheckedCastNullableTToT(range.getUpperEndpoint()),
				node.getElement());
		if (cmp > 0) {
	...
	@Override
	public int size() {
		return org.magneton.core.primitives.Ints.saturatedCast(aggregateForEntries(Aggregate.SIZE));
	}
	...
	public void clear() {
		if (!range.hasLowerBound() && !range.hasUpperBound()) {
			// We can do this in O(n) rather than removing one by one, which could force
			// rebalancing.
			for (AvlNode<E> current = header.succ(); current != header;) {
				AvlNode<E> next = current.succ();

				current.elemCount = 0;
				// Also clear these fields so that one deleted Entry doesn't retain all
				// elements.
				current.left = null;
				current.right = null;
				current.pred = null;
	...
	private Multiset.Entry<E> wrapEntry(AvlNode<E> baseEntry) {
		return new org.magneton.core.collect.Multisets.AbstractEntry<E>() {
			@Override
			@ParametricNullness
			public E getElement() {
	...
	private AvlNode<E> firstNode() {
		...
		if (range.hasLowerBound()) {
			...
			E endpoint = NullnessCasts.uncheckedCastNullableTToT(range.getLowerEndpoint());
			node = root.ceiling(comparator(), endpoint);
			if (node == null) {
				return null;
			}
			if (range.getLowerBoundType() == org.magneton.core.collect.BoundType.OPEN
	...
	private AvlNode<E> lastNode() {
		...
		if (range.hasUpperBound()) {
			...
			E endpoint = NullnessCasts.uncheckedCastNullableTToT(range.getUpperEndpoint());
			node = root.floor(comparator(), endpoint);
			if (node == null) {
				return null;
			}
			if (range.getUpperBoundType() == org.magneton.core.collect.BoundType.OPEN
					&& comparator().compare(endpoint, node.getElement()) == 0) {
				node = node.pred();
			}
		}
	...
	Iterator<E> elementIterator() {
		return org.magneton.core.collect.Multisets.elementIterator(entryIterator());
	}
	...
	Iterator<Multiset.Entry<E>> entryIterator() {
		return new Iterator<Multiset.Entry<E>>() {
			...
			public Multiset.Entry<E> next() {
				...
				// requireNonNull is safe because current is only nulled out after
				// iteration is complete.
				Multiset.Entry<E> result = wrapEntry(requireNonNull(current));
				prevEntry = result;
			...
			@Override
			public void remove() {
				Preconditions.checkState(prevEntry != null, "no calls to next() since the last call to remove()");
	...
	Iterator<Multiset.Entry<E>> descendingEntryIterator() {
		return new Iterator<Multiset.Entry<E>>() {
			...
			public Multiset.Entry<E> next() {
				...
				Multiset.Entry<E> result = wrapEntry(current);
				prevEntry = result;
	...
	public void forEachEntry(ObjIntConsumer<? super E> action) {
		...
		for (AvlNode<E> node = firstNode(); node != header && node != null
				&& !range.tooHigh(node.getElement()); node = node.succ()) {
	...
	public Iterator<E> iterator() {
		return org.magneton.core.collect.Multisets.iteratorImpl(this);
	}
	...
	/**
	 * @serialData the comparator, the number of distinct elements, the first element, its
	 ...
	 * method that calls the comparator to compare the two keys. If that change is made,
	 * AbstractMultiset.equals() can simply check whether two multisets have equal entry
	 * sets.
	 */
	...
	private static final class AvlNode<E extends Object> {
		...
		/*
		 * For "normal" nodes, the type of this field is `E`, not `@Nullable E` (though
		 * note that E is a type that can include null, as in a TreeMultiset<@Nullable
		 * String>).
		 *
		 * For the header node, though, this field contains `null`, regardless of the type
		 * of the multiset.
		 *
		 * Most code that operates on an AvlNode never operates on the header node. Such
		 * code can access the elem field without a null check by calling getElement().
		 */
		@CheckForNull
		...
		/*
		 * pred and succ are nullable after construction, but we always call successor()
		 * to initialize them immediately thereafter.
		 *
		 * They may be subsequently nulled out by TreeMultiset.clear(). I think that the
		 * only place that we can reference a node whose fields have been cleared is
		 * inside the iterator (and presumably only under concurrent modification).
		 *
		 * To access these fields when you know that they are not null, call the pred()
		 * and succ() methods, which perform null checks before returning the fields.
		 */
		@CheckForNull
		...
		// For discussion of pred() and succ(), see the comment on the pred and succ
		// fields.
		...
		@CheckForNull
		AvlNode<E> setCount(Comparator<? super E> comparator, @ParametricNullness E e, int expectedCount, int newCount,
				int[] result) {
			int cmp = comparator.compare(e, getElement());
			if (cmp < 0) {
				AvlNode<E> initLeft = left;
		...
		private AvlNode<E> rebalance() {
			...
			case -2:
				// requireNonNull is safe because right must exist in order to get a
				// negative factor.
				...
				return rotateLeft();
			case 2:
				// requireNonNull is safe because left must exist in order to get a
				// positive factor.
				requireNonNull(left);
		...
		private AvlNode<E> rotateLeft() {
			...
			right = newTop.left;
			newTop.left = this;
			newTop.totalCount = totalCount;
			newTop.distinctElements = distinctElements;
			recompute();
		...
		private AvlNode<E> ceiling(Comparator<? super E> comparator, @ParametricNullness E e) {
			int cmp = comparator.compare(e, getElement());
			if (cmp < 0) {
				return (left == null) ? this
						: org.magneton.core.base.MoreObjects.firstNonNull(left.ceiling(comparator, e), this);
			}
			else if (cmp == 0) {
				return this;
			}
		...
		E getElement() {
			...
			return NullnessCasts.uncheckedCastNullableTToT(elem);
		}

...
final class Platform {
	...
	private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(Platform.class.getName());

	private Platform() {
	}

	/** Returns the platform preferred implementation of a map based on a hash table. */
	static <K extends Object, V extends Object> Map<K, V> newHashMapWithExpectedSize(int expectedSize) {
		return org.magneton.core.collect.Maps.newHashMapWithExpectedSize(expectedSize);
	}

	/**
	 * Returns the platform preferred implementation of an insertion ordered map based on
	 * a hash table.
	 */
	static <K extends Object, V extends Object> Map<K, V> newLinkedHashMapWithExpectedSize(int expectedSize) {
	...
	/**
	 * Returns the platform preferred implementation of an insertion ordered set based on
	 * a hash table.
	 */
	...
	/**
	 * Returns the platform preferred map implementation that preserves insertion order
	 * when used only for insertions.
	 */
	...
	/**
	 * Returns the platform preferred set implementation that preserves insertion order
	 * when used only for insertions.
	 */
	 ...
	 */
	/*
	 * The new array contains nulls, even if the old array did not. If we wanted to be
	 * accurate, we would declare a return type of `@Nullable T[]`. However, we've decided
	 * not to think too hard about arrays for now, as they're a mess. (We previously
	 * discussed this in the review of ObjectArrays, which is the main caller of this
	 * method.)
	 */
	...
	/*
	 * Arrays are a mess from a nullness perspective, and Class instances for object-array
	 * types are even worse. For now, we just suppress and move on with our lives.
	 *
	 ...
	 * otherwise (i.e., in GWT). This is sometimes acceptable, when only server-side code
	 * could generate enough volume that reclamation becomes important.
	 */
	static org.magneton.core.collect.MapMaker tryWeakKeys(MapMaker mapMaker) {
	...
	static void checkGwtRpcEnabled() {
		...
		if (!Boolean.parseBoolean(System.getProperty(propertyName, "false"))) {
			throw new UnsupportedOperationException(Strings.lenientFormat(
					...
					propertyName, "https://stackoverflow.com/q/5189914/28465",

...
public final class Streams {
	...
	/**
	 * If a value is present in {@code optional}, returns a stream containing only that
	 * element, otherwise returns an empty stream.
	 */
	...
	private static void closeAll(BaseStream<?, ?>[] toClose) {
		for (BaseStream<?, ?> stream : toClose) {
			// TODO(b/80534298): Catch exceptions, rethrowing later with extras as
	...
	/**
	 * Returns a {@link Stream} containing the elements of the first stream, followed by
	 * the elements of the second stream, and so on.
	 *
	 * <p>
	 * This is equivalent to {@code Stream.of(streams).flatMap(stream -> stream)}, but the
	...
	public static <T> Stream<T> concat(Stream<? extends T>... streams) {
		...
		return StreamSupport
				.stream(CollectSpliterators.flatMap(splitrsBuilder.build().spliterator(),
						splitr -> (Spliterator<T>) splitr, characteristics, estimatedSize), isParallel)
				.onClose(() -> closeAll(streams));
	}

	/**
	 * Returns an {@link IntStream} containing the elements of the first stream, followed
	 * by the elements of the second stream, and so on.
	 *
	 * <p>
	 * This is equivalent to {@code Stream.of(streams).flatMapToInt(stream -> stream)},
	...
	public static IntStream concat(IntStream... streams) {
		...
		return StreamSupport.intStream(CollectSpliterators.flatMapToInt(splitrsBuilder.build().spliterator(),
				splitr -> splitr, characteristics, estimatedSize), isParallel).onClose(() -> closeAll(streams));
	}

	/**
	 * Returns a {@link LongStream} containing the elements of the first stream, followed
	 * by the elements of the second stream, and so on.
	 *
	 * <p>
	 * This is equivalent to {@code Stream.of(streams).flatMapToLong(stream -> stream)},
	 * but the returned stream may perform better.
	 *
	...
	public static LongStream concat(LongStream... streams) {
		...
		return StreamSupport.longStream(CollectSpliterators.flatMapToLong(splitrsBuilder.build().spliterator(),
				splitr -> splitr, characteristics, estimatedSize), isParallel).onClose(() -> closeAll(streams));
	}

	/**
	 * Returns a {@link DoubleStream} containing the elements of the first stream,
	 * followed by the elements of the second stream, and so on.
	 *
	 * <p>
	 * This is equivalent to {@code Stream.of(streams).flatMapToDouble(stream -> stream)},
	 * but the returned stream may perform better.
	 *
	...
	public static DoubleStream concat(DoubleStream... streams) {
		...
		return StreamSupport.doubleStream(CollectSpliterators.flatMapToDouble(splitrsBuilder.build().spliterator(),
				splitr -> splitr, characteristics, estimatedSize), isParallel).onClose(() -> closeAll(streams));
	}

	/**
	 * Returns a stream in which each element is the result of passing the corresponding
	 ...
	 *
	 * <p>
	 * Note that if you are calling {@link Stream#forEach} on the resulting stream, you
	 * might want to consider using {@link #forEachPair} instead of this method.
	 *
	 * <p>
	...
	public static <A, B, R> Stream<R> zip(Stream<A> streamA, Stream<B> streamB,
		...
		boolean isParallel = streamA.isParallel() || streamB.isParallel(); // same as
																			// Stream.concat
		...
		return StreamSupport.stream(
				new AbstractSpliterator<R>(min(splitrA.estimateSize(), splitrB.estimateSize()), characteristics) {
					@Override
	...
	/**
	 * Invokes {@code consumer} once for each pair of <i>corresponding</i> elements in
	 * {@code streamA} and {@code streamB}. If one stream is longer than the other, the
	 * extra elements are silently ignored. Elements passed to the consumer are guaranteed
	 * to come from the same position in their respective source streams. For example:
	 *
	 * <pre>{@code
	 * Streams.forEachPair(
	 *   Stream.of("foo1", "foo2", "foo3"),
	 ...
	 * correspondence between elements will be made, but the order in which those pairs of
	 * elements are passed to the consumer is <i>not</i> defined.
	 *
	 * <p>
	 * Note that many usages of this method can be replaced with simpler calls to
	 * {@link #zip}. This method behaves equivalently to {@linkplain #zip zipping} the
	 * stream elements into temporary pair objects and then using {@link Stream#forEach}
	 * on that stream.
	 *
	 * @since 22.0
	 */
	public static <A, B> void forEachPair(Stream<A> streamA, Stream<B> streamB,
	 ...
	 * elements of {@code stream} and their indices in the stream. For example,
	 *
	 * <pre>{@code
	 * mapWithIndex(
	 *     Stream.of("a", "b", "c"),
	 ...
	 * <a href="http://gee.cs.oswego.edu/dl/html/StreamParallelGuidance.html">efficiently
	 * splittable</a> if and only if {@code stream} was efficiently splittable and its
	 * underlying spliterator reported {@link Spliterator#SUBSIZED}. This is generally the
	 * case if the underlying stream comes from a data structure supporting efficient
	 * indexed random access, typically an array or list.
	 *
	 * <p>
	 * The order of the resulting stream is defined if and only if the order of the
	 * original stream was defined.
	 */
			...
			public boolean tryAdvance(Consumer<? super R> action) {
				if (fromSpliterator.tryAdvance(this)) {
					try {
						...
						action.accept(function.apply(NullnessCasts.uncheckedCastNullableTToT(holder), index++));
						return true;
					}
					finally {
						holder = null;
	...
	/**
	 * Returns a stream consisting of the results of applying the given function to the
	 * elements of {@code stream} and their indexes in the stream. For example,
	 *
	 * <pre>{@code
	 * mapWithIndex(
	 ...
	 * {@link #mapWithIndex(Stream, FunctionWithIndex)}.
	 *
	 * @since 21.0
	 */
	...
	private abstract static class MapWithIndexSpliterator<F extends Spliterator<?>, R, S extends MapWithIndexSpliterator<F, R, S>>
		...
		public S trySplit() {
			...
			index += split.getExactSizeIfKnown();
			return result;
		}
	 ...
	 * <p>
	 * This interface is only intended for use by callers of
	 * {@link #mapWithIndex(IntStream, IntFunctionWithIndex)}.
	 *
	 * @since 21.0
	 */
	 ...
	 * <p>
	 * This interface is only intended for use by callers of
	 * {@link #mapWithIndex(LongStream, LongFunctionWithIndex)}.
	 *
	 * @since 21.0
	 */
	 ...
	 * {@link java.util.Optional#empty} if the stream is empty.
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform significantly
	 * better. This method's runtime will be between O(log n) and O(n), performing better
	 * on
	 * <a href="http://gee.cs.oswego.edu/dl/html/StreamParallelGuidance.html">efficiently
	 * splittable</a> streams.
	 *
	 * <p>
	 * If the stream has nondeterministic order, this has equivalent semantics to
	 * {@link Stream#findAny} (which you might as well use).
	 *
	 * @see Stream#findFirst()
	 * @throws NullPointerException if the last element of the stream is null
	 */
	/*
	 * By declaring <T> instead of <T >, we declare this method as requiring a stream
	 * whose elements are non-null. However, the method goes out of its way to still
	 * handle nulls in the stream. This means that the method can safely be used with a
	 * stream that contains nulls as long as the *last* element is *not* null.
	 *
	...
	/**
	 * Returns the last element of the specified stream, or {@link OptionalInt#empty} if
	 * the stream is empty.
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform significantly
	...
	/**
	 * Returns the last element of the specified stream, or {@link OptionalLong#empty} if
	 * the stream is empty.
	 *
	 * <p>
	 * Equivalent to {@code stream.reduce((a, b) -> b)}, but may perform significantly
	...
	/**
	 * Returns the last element of the specified stream, or {@link OptionalDouble#empty}

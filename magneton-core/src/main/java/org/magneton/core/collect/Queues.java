...
/**
 * Static utility methods pertaining to {@link Queue} and {@link Deque} instances. Also
 * see this class's counterparts {@link Lists}, {@link Sets}, and {@link Maps}.
 *
...
public final class Queues {
	...
	/**
	 * Creates an empty {@code ArrayBlockingQueue} with the given (fixed) capacity and
	 * nonfair access policy.
	 */
	...
	/**
	 * Creates an {@code ArrayDeque} containing the elements of the specified iterable, in
	...
	public static <E> ArrayDeque<E> newArrayDeque(Iterable<? extends E> elements) {
		...
		org.magneton.core.collect.Iterables.addAll(deque, elements);
		return deque;
	}
	...
	/**
	 * Creates a {@code ConcurrentLinkedQueue} containing the elements of the specified
	...
	public static <E> ConcurrentLinkedQueue<E> newConcurrentLinkedQueue(Iterable<? extends E> elements) {
		...
		org.magneton.core.collect.Iterables.addAll(queue, elements);
		return queue;
	}
	 ...
	 * containing the elements of the specified iterable, in the order they are returned
	 * by the iterable's iterator.
	 *
	 * @since 12.0
	 */
	 ...
	 *
	 * @since 11.0 (but the bound of {@code E} was changed from {@code Object} to
	 * {@code Comparable} in 15.0)
	 */
	 ...
	 * {@code PriorityQueue}, this priority queue will be ordered according to the same
	 * ordering.
	 *
	 * @since 11.0 (but the bound of {@code E} was changed from {@code Object} to
	 * {@code Comparable} in 15.0)
	...
	/**
	 * Creates an empty {@code PriorityQueue} with the ordering given by its elements'
	 * natural ordering.
	 *
	 * @since 11.0 (but the bound of {@code E} was changed from {@code Object} to
	 * {@code Comparable} in 15.0)
	...
	/**
	 * Drains the queue as {@link BlockingQueue#drainTo(Collection, int)}, but if the
	 * requested {@code
	...
	public static <E> int drain(BlockingQueue<E> q, Collection<? super E> buffer, int numElements, long timeout,
		...
		while (added < numElements) {
			...
			// there are multiple
			// elements already available (e.g. LinkedBlockingQueue#drainTo locks only
			// once)
			...
			if (added < numElements) { // not enough elements immediately available; will
				...
				if (e == null) {
					break; // we already waited enough, and there are no more elements in
							// sight
				}
				buffer.add(e);
				added++;
	...
	/**
	 * Drains the queue as {@linkplain #drain(BlockingQueue, Collection, int, Duration)},
	 * but with a different behavior in case it is interrupted while waiting. In that
	 * case, the operation will continue as usual, and in the end the thread's
	...
	/**
	 * Drains the queue as
	 * {@linkplain #drain(BlockingQueue, Collection, int, long, TimeUnit)}, but with a
	 * different behavior in case it is interrupted while waiting. In that case, the
	 * operation will continue as usual, and in the end the thread's interruption status
	 * will be set (no {@code
	 * InterruptedException} is thrown).
	 * @param q the blocking queue to be drained
	 * @param buffer where to add the transferred elements
	...
	public static <E> int drainUninterruptibly(BlockingQueue<E> q, Collection<? super E> buffer, int numElements,
		...
		try {
			while (added < numElements) {
				// we could rely solely on #poll, but #drainTo might be more efficient
				// when there are
				// multiple elements already available (e.g. LinkedBlockingQueue#drainTo
				// locks only once)
				added += q.drainTo(buffer, numElements - added);
				if (added < numElements) { // not enough elements immediately available;
											// will have to poll
					E e; // written exactly once, by a successful (uninterrupted)
							// invocation of #poll
					while (true) {
						try {
	...
	/**
	 * Returns a synchronized (thread-safe) queue backed by the specified queue. In order
	 * to guarantee serial access, it is critical that <b>all</b> access to the backing
	 * queue is accomplished through the returned queue.
	 *
	 * <p>
	 * It is imperative that the user manually synchronize on the returned queue when
	 * accessing the queue's iterator:
	 *
	 * <pre>{@code
	...
	public static <E extends Object> Queue<E> synchronizedQueue(Queue<E> queue) {
		return org.magneton.core.collect.Synchronized.queue(queue, null);
	}

	/**
	 * Returns a synchronized (thread-safe) deque backed by the specified deque. In order
	 * to guarantee serial access, it is critical that <b>all</b> access to the backing
	 * deque is accomplished through the returned deque.
	 *
	 * <p>
	 * It is imperative that the user manually synchronize on the returned deque when
	 * accessing any of the deque's iterators:
	 *
	 * <pre>{@code
	...
	public static <E extends Object> Deque<E> synchronizedDeque(Deque<E> deque) {
		return org.magneton.core.collect.Synchronized.deque(deque, null);
	}

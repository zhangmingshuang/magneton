...
/**
 * Implementation of {@code Multimap} that does not allow duplicate key-value entries and
 * that returns collections whose iterators follow the ordering in which the data was
 * added to the multimap.
 *
 * <p>
 * The collections returned by {@code keySet}, {@code keys}, and {@code asMap} iterate
 * through the keys in the order they were first added to the multimap. Similarly,
 * {@code get}, {@code
 * removeAll}, and {@code replaceValues} return collections that iterate through the
 * values in the order they were added. The collections generated by {@code entries} and
 * {@code values} iterate across the key-value mappings in the order they were added to
 * the multimap.
 *
 * <p>
 * The iteration ordering of the collections generated by {@code keySet}, {@code keys},
 * and {@code asMap} has a few subtleties. As long as the set of keys remains unchanged,
 * adding or removing mappings does not affect the key iteration order. However, if you
 * remove all values associated with a key and then add the key back to the multimap, that
 * key will come last in the key iteration order.
 *
 * <p>
 ...
 *
 * <p>
 * <b>Warning:</b> Do not modify either a key <i>or a value</i> of a
 * {@code LinkedHashMultimap} in a way that affects its {@link Object#equals} behavior.
...
public final class LinkedHashMultimap<K extends Object, V extends Object>
	...
	/**
	 * Constructs an empty {@code LinkedHashMultimap} with enough capacity to hold the
	 ...
	 */
	public static <K extends Object, V extends Object> LinkedHashMultimap<K, V> create(int expectedKeys,
			int expectedValuesPerKey) {
		return new LinkedHashMultimap<>(Maps.capacity(expectedKeys), Maps.capacity(expectedValuesPerKey));
	}

	/**
	 * Constructs a {@code LinkedHashMultimap} with the same mappings as the specified
	 * multimap. If a key-value mapping appears multiple times in the input multimap, it
	 * only appears once in the constructed multimap. The new multimap has the same
	 * {@link Multimap#entries()} iteration order as the input multimap, except for
	 * excluding duplicate mappings.
	...
	private static <K extends Object, V extends Object> void succeedsInValueSet(ValueSetLink<K, V> pred,
			ValueSetLink<K, V> succ) {
		pred.setSuccessorInValueSet(succ);
		succ.setPredecessorInValueSet(pred);
	}

	private static <K extends Object, V extends Object> void succeedsInMultimap(ValueEntry<K, V> pred,
			ValueEntry<K, V> succ) {
		pred.setSuccessorInMultimap(succ);
		succ.setPredecessorInMultimap(pred);
	}

	private static <K extends Object, V extends Object> void deleteFromValueSet(ValueSetLink<K, V> entry) {
		succeedsInValueSet(entry.getPredecessorInValueSet(), entry.getSuccessorInValueSet());
	}

	private static <K extends Object, V extends Object> void deleteFromMultimap(ValueEntry<K, V> entry) {
	 ...
	 *
	 * <p>
	 * If {@code values} is not empty and the multimap already contains a mapping for
	 * {@code key}, the {@code keySet()} ordering is unchanged. However, the provided
	 * values always come last in the {@link #entries()} and {@link #values()} iteration
	 * orderings.
	 */
	...
	/**
	 * Returns a set of all key-value pairs. Changes to the returned set will update the
	 * underlying multimap, and vice versa. The entries set does not support the
	 * {@code add} or {@code addAll} operations.
	 *
	 * <p>
	 * The iterator generated by the returned set traverses the entries in the order they
	 ...
	 *
	 * <p>
	 * The iterator generated by the returned set traverses the keys in the order they
	 * were first added to the multimap.
	 *
	 * <p>
	...
	/**
	 * @serialData the expected values per key, the number of distinct keys, the number of
	 * entries, and the entries in order
	 */
	...
	private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
		...
		for (int i = 0; i < entries; i++) {
			...
			/*
			 * requireNonNull is safe for a properly serialized multimap: We've already
			 * inserted a collection for each key that we expect.
			 */
	...
	/**
	 * LinkedHashMultimap entries are in no less than three coexisting linked lists: a
	 * bucket in the hash table for a {@code Set<V>} associated with a key, the linked
	...
	static final class ValueEntry<K extends Object, V extends Object>
		...
		/*
		 * The *InValueSet and *InMultimap fields below are null after construction, but
		 * we almost always call succeedsIn*() to initialize them immediately thereafter.
		 *
		 * The exception is the *InValueSet fields of multimapHeaderEntry, which are never
		 * set. (That works out fine as long as we continue to be careful not to try
		 * delete them or iterate past them.)
		 *
		 * We could consider "lying" and omitting @CheckNotNull from all these fields.
		 * Normally, I'm not a fan of that: What if we someday implement (presumably to be
		 * enabled during tests only) bytecode rewriting that checks for any null value
		 * that passes through an API with a known-non-null type? But that particular
		 * problem might not arise here, since we're not actually reading from the fields
		 * in any case in which they might be null (as proven by the requireNonNull checks
		 * below). Plus, we're *already* lying here, since newHeader passes a null key and
		 * value, which we pass to the superconstructor, even though the key and value
		 * type for a given entry might not include null. The right fix for the header
		 * problems is probably to define a separate MultimapLink interface with a
		 * separate "header" implementation, which hopefully could avoid implementing
		 * Entry or ValueSetLink at all. (But note that that approach requires us to
		 * define extra classes -- unfortunate under Android.) *Then* we could consider
		 * lying about the fields below on the grounds that we always initialize them just
		 * after the constructor -- an example of the kind of lying that our hypotheticaly
		 * bytecode rewriter would already have to deal with, thanks to DI frameworks that
		 * perform field and method injection, frameworks like Android that define
		 * post-construct hooks like Activity.onCreate, etc.
		 */
		...
		ValueEntry(@ParametricNullness K key, @ParametricNullness V value, int smearedValueHash,
		...
		// see the comment on the class fields, especially
		// about newHeader
	...
	final class ValueSet extends Sets.ImprovedAbstractSet<V> implements ValueSetLink<K, V> {
		...
		/*
		 * We currently use a fixed load factor of 1.0, a bit higher than normal to reduce
		 * memory consumption.
		 */

...
/**
 * A {@link NavigableSet} whose contents will never change, with many other important
 ...
 * {@code a.equals(b)}. If this advice is not followed, the resulting collection will not
...
public abstract class ImmutableSortedSet<E> extends ImmutableSortedSetFauxverideShim<E>
		implements NavigableSet<E>, org.magneton.core.collect.SortedIterable<E> {

	static final int SPLITERATOR_CHARACTERISTICS = org.magneton.core.collect.ImmutableSet.SPLITERATOR_CHARACTERISTICS
			| Spliterator.SORTED;
	 ...
	 *
	 * <p>
	 * If the elements contain duplicates (according to the comparator), only the first
	...
	public static <E> Collector<E, ?, ImmutableSortedSet<E>> toImmutableSortedSet(Comparator<? super E> comparator) {
		return org.magneton.core.collect.CollectCollectors.toImmutableSortedSet(comparator);
	}
	...
	static <E> RegularImmutableSortedSet<E> emptySet(Comparator<? super E> comparator) {
		if (Ordering.natural().equals(comparator)) {
			...
			return new RegularImmutableSortedSet<E>(org.magneton.core.collect.ImmutableList.<E>of(), comparator);
		}
	}
	...
	public static <E extends Comparable<? super E>> ImmutableSortedSet<E> of(E element) {
		return new RegularImmutableSortedSet<E>(org.magneton.core.collect.ImmutableList.of(element),
				Ordering.natural());
	}
	 ...
	 * Returns an immutable sorted set containing the given elements sorted by their
	 * natural ordering. When multiple elements are equivalent according to
	 * {@code compareTo()}, only the first one specified is included. To create a copy of
	 * a {@code SortedSet} that preserves the comparator, call {@link #copyOfSorted}
	 * instead. This method iterates over {@code elements} at most once.
	 *
	 * <p>
	 * Note that if {@code s} is a {@code Set<String>}, then
	 * {@code ImmutableSortedSet.copyOf(s)} returns an {@code ImmutableSortedSet<String>}
	 * containing each of the strings in {@code s}, while {@code ImmutableSortedSet.of(s)}
	 * returns an {@code ImmutableSortedSet<Set<String>>} containing one element (the
	 * given set itself).
	 *
	 * <p>
	 ...
	 * {@code compareTo()}, only the first one specified is included.
	 * @throws NullPointerException if {@code comparator} or any of {@code elements} is
	 * null
	 */
	 ...
	 * Returns an immutable sorted set containing the given elements sorted by the given
	 * {@code
	 * Comparator}. When multiple elements are equivalent according to {@code compare()},
	 * only the first one specified is included. This method iterates over
	 * {@code elements} at most once.
	 *
	 * <p>
	 * Despite the method name, this method attempts to avoid actually copying the data
	 * when it is safe to do so. The exact circumstances under which a copy will or will
	...
	public static <E> ImmutableSortedSet<E> copyOf(Comparator<? super E> comparator, Iterable<? extends E> elements) {
		...
		boolean hasSameComparator = org.magneton.core.collect.SortedIterables.hasSameComparator(comparator, elements);
		...
		E[] array = (E[]) org.magneton.core.collect.Iterables.toArray(elements);
	...
	/**
	 * Returns an immutable sorted set containing the elements of a sorted set, sorted by
	 * the same {@code Comparator}. That behavior differs from {@link #copyOf(Iterable)},
	...
	public static <E> ImmutableSortedSet<E> copyOfSorted(SortedSet<E> sortedSet) {
		Comparator<? super E> comparator = org.magneton.core.collect.SortedIterables.comparator(sortedSet);
		org.magneton.core.collect.ImmutableList<E> list = org.magneton.core.collect.ImmutableList.copyOf(sortedSet);
		if (list.isEmpty()) {
	 ...
	 * {@code k} positions of {@code contents}, and {@code contents[i] ==
	 * null} for {@code k <= i < n}.
	 *
	 * <p>
	...
	static <E> ImmutableSortedSet<E> construct(Comparator<? super E> comparator, int n, E... contents) {
		...
		return new RegularImmutableSortedSet<E>(
				org.magneton.core.collect.ImmutableList.<E>asImmutableList(contents, uniques), comparator);
	}

	/**
	 * Returns a builder that creates immutable sorted sets with an explicit comparator.
	 * If the comparator has a more general type than the set being generated, such as
	 * creating a {@code
	 * SortedSet<Integer>} with a {@code Comparator<Number>}, use the {@link Builder}
	 * constructor instead.
	...
	/**
	 * Returns a builder that creates immutable sorted sets whose elements are ordered by
	 * the reverse of their natural ordering.
	 */
	...
	/**
	 * Returns a builder that creates immutable sorted sets whose elements are ordered by
	 * their natural ordering. The sorted sets use {@link Ordering#natural()} as the
	 * comparator. This method provides more type-safety than {@link #builder}, as it can
	 * be called only for classes that implement {@link Comparable}.
	 */
	...
	static int unsafeCompare(Comparator<?> comparator, Object a, @CheckForNull Object b) {
		...
		// compare a and b, we should get a CCE or NPE on the subsequent line. Only
		// methods
	 ...
	 * {@link Ordering#natural()} when the natural ordering of the elements is used. Note
	 * that its behavior is not consistent with {@link SortedSet#comparator()}, which
	 * returns {@code null} to indicate natural ordering.
	 */
	@Override
	 ...
	 *
	 * <p>
	 * The {@link SortedSet#headSet} documentation states that a subset of a subset throws
	 * an {@link IllegalArgumentException} if passed a {@code toElement} greater than an
	 * earlier {@code
	 * toElement}. However, this method doesn't throw an exception in that situation, but
	 * instead keeps the original {@code toElement}.
	 */
	@Override
	 ...
	 *
	 * <p>
	 * The {@link SortedSet#subSet} documentation states that a subset of a subset throws
	 * an {@link IllegalArgumentException} if passed a {@code fromElement} smaller than an
	 * earlier {@code
	 * fromElement}. However, this method doesn't throw an exception in that situation,
	 * but instead keeps the original {@code fromElement}. Similarly, this method keeps
	 * the original {@code
	 * toElement}, instead of throwing an exception, if passed a {@code toElement} greater
	 * than an earlier {@code toElement}.
	 */
	@Override
	 ...
	 *
	 * <p>
	 * The {@link SortedSet#tailSet} documentation states that a subset of a subset throws
	 * an {@link IllegalArgumentException} if passed a {@code fromElement} smaller than an
	 * earlier {@code
	 * fromElement}. However, this method doesn't throw an exception in that situation,
	 * but instead keeps the original {@code fromElement}.
	 */
	@Override
	...
	/*
	 * These methods perform most headSet, subSet, and tailSet logic, besides parameter
	 * validation.
	 */
	...
	@CheckForNull
	public E lower(E e) {
		return org.magneton.core.collect.Iterators.getNext(headSet(e, false).descendingIterator(), null);
	}
	...
	public E ceiling(E e) {
		return org.magneton.core.collect.Iterables.getFirst(tailSet(e, true), null);
	}
	...
	// but we push down that implementation because ProGuard can't eliminate it even when
	// it's always
	// overridden.
	...
	public Spliterator<E> spliterator() {
		return new Spliterators.AbstractSpliterator<E>(size(), SPLITERATOR_CHARACTERISTICS | Spliterator.SIZED) {
	 ...
	 * {@code public static final} sets ("constant sets"), with a given comparator.
	 * Example:
	 *
	 * <pre>{@code
	 ...
	 * to build multiple sets in series. Each set is a superset of the set created before
	 * it.
	 *
	 * @since 2.0
	 */
	public static final class Builder<E> extends org.magneton.core.collect.ImmutableSet.Builder<E> {
		...
		public Builder(Comparator<? super E> comparator) {
			...
			elements = (E[]) new Object[ImmutableCollection.Builder.DEFAULT_INITIAL_CAPACITY];
			n = 0;
		}
		 ...
		 * {@code ImmutableSortedSet} already contains {@code element}, then {@code add}
		...
		public Builder<E> add(E element) {
			...
			if (n == elements.length) {
				sortAndDedup();
				/*
				 * Sorting operations can only be allowed to occur once every O(n)
				 * operations to keep amortized O(n log n) performance. Therefore, ensure
				 * there are at least O(n) *unused* spaces in the builder array.
				 */
		...
		/**
		 * Returns a newly-created {@code ImmutableSortedSet} based on the contents of the
		 * {@code
		...
		public ImmutableSortedSet<E> build() {
			...
			if (n == 0) {
				...
				forceCopy = true;
				return new RegularImmutableSortedSet<E>(ImmutableList.asImmutableList(elements, n), comparator);
			}
		}
	...
	/*
	 * This class is used to serialize all ImmutableSortedSet instances, regardless of
	 * implementation type. It captures their "logical contents" only. This is necessary
	 * to ensure that the existence of a particular implementation type is an

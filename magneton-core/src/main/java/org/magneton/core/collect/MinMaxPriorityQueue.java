...
/**
 * A double-ended priority queue, which provides constant-time access to both its least
 * element and its greatest element, as determined by the queue's specified comparator. If
 * no comparator is given at creation time, the natural order of elements is used. If no
 * maximum size is given at creation time, the queue is unbounded.
 *
 * <p>
 * Usage example:
 *
 ...
 *
 * <p>
 * As a {@link Queue} it functions exactly as a {@link PriorityQueue}: its head element --
 * the implicit target of the methods {@link #peek()}, {@link #poll()} and
 * {@link #remove()} -- is defined as the <i>least</i> element in the queue according to
 * the queue's comparator. But unlike a regular priority queue, the methods
 * {@link #peekLast}, {@link #pollLast} and {@link #removeLast} are also provided, to act
 * on the <i>greatest</i> element in the queue instead.
 *
 * <p>
 * A min-max priority queue can be configured with a maximum size. If so, each time the
 * size of the queue exceeds that value, the queue automatically removes its greatest
 * element according to its comparator (which might be the element that was just added).
 * This is different from conventional bounded queues, which either block or reject new
 * elements when full.
 *
 * <p>
 * This implementation is based on the
 * <a href="http://portal.acm.org/citation.cfm?id=6621">min-max heap</a> developed by
 * Atkinson, et al. Unlike many other double-ended priority queues, it stores elements in
 * a single array, as compact as the traditional heap data structure used in
 * {@link PriorityQueue}.
 *
 * <p>
 ...
 *
 * <ul>
 * <li>If you only access one end of the queue, and do use a maximum size, this class will
 * perform significantly worse than a {@code PriorityQueue} with manual eviction above the
 * maximum size. In many cases {@link Ordering#leastOf} may work for your use case with
 * significantly improved (and asymptotically superior) performance.
 * <li>The retrieval operations {@link #peek}, {@link #peekFirst}, {@link #peekLast},
 * {@link #element}, and {@link #size} are constant-time.
 * <li>The enqueuing and dequeuing operations ({@link #offer}, {@link #add}, and all the
 * forms of {@link #poll} and {@link #remove()}) run in {@code O(log n) time}.
 * <li>The {@link #remove(Object)} and {@link #contains} operations require linear
 * ({@code O(n)}) time.
 * <li>If you only access one end of the queue, and don't use a maximum size, this class
 * is functionally equivalent to {@link PriorityQueue}, but significantly slower.
 * </ul>
 *
...
public final class MinMaxPriorityQueue<E> extends AbstractQueue<E> {
	...
	/**
	 * Creates a new min-max priority queue using natural order, no maximum size, and
	 * initially containing the given elements.
	 */
	...
	/**
	 * Creates and returns a new builder, configured to build {@code MinMaxPriorityQueue}
	 * instances that use {@code comparator} to determine the least and greatest elements.
	 */
	/*
	 * TODO(cpovirk): Change to Comparator<? super B> to permit Comparator<@Nullable ...>
	 * and Comparator<SupertypeOfB>? What we have here matches the immutable collections,
	 * but those also expose a public Builder constructor that accepts "? super." So maybe
	...
	/**
	 * Creates and returns a new builder, configured to build {@code MinMaxPriorityQueue}
	 * instances sized appropriately to hold {@code expectedSize} elements.
	 */
	...
	/**
	 * Creates and returns a new builder, configured to build {@code MinMaxPriorityQueue}
	 * instances that are limited to {@code maximumSize} elements. Each time a queue grows
	 * beyond this bound, it immediately removes its greatest element (according to its
	 * comparator), which might be the element that was just added.
	 */
	 ...
	 * adding {@code
	 * element} the queue will automatically evict its greatest element (according to its
	 * comparator), which may be {@code element} itself.
	...
	/**
	 * Removes and returns the least element of this queue, or returns {@code null} if the
	 * queue is empty.
	 */
	@CanIgnoreReturnValue
	@CheckForNull
	...
	/**
	 * Removes and returns the greatest element of this queue, or returns {@code null} if
	 * the queue is empty.
	 */
	@CanIgnoreReturnValue
	@CheckForNull
	 ...
	 *
	 * <p>
	 * Normally this method leaves the elements at up to {@code index - 1}, inclusive,
	 * untouched. Under these circumstances, it returns {@code null}.
	 *
	 * <p>
	 * Occasionally, in order to maintain the heap invariant, it must swap a later element
	 * of the list with one before {@code index}. Under these circumstances it returns a
	 * pair of elements as a {@link MoveDesc}. The first one is the element that was
	 * previously at the end of the heap and is now at some position before {@code index}.
	 * The second element is the one that was swapped down to replace the element at
	...
	MoveDesc<E> removeAt(int index) {
		...
		if (lastElementAt == index) {
			...
			// 'lastElementAt'
			// is now at the end of queue. If that's the element we wanted to remove in
			// the first place,
	 ...
	 *
	 * <p>
	 * The iterator is <i>fail-fast</i>: If the MinMaxPriorityQueue is modified at any
	 ...
	 * {@code ConcurrentModificationException} on a best-effort basis. Therefore, it would
	 * be wrong to write a program that depended on this exception for its correctness:
	...
	/**
	 * Returns the comparator used to order the elements in this queue. Obeys the general
	 * contract of {@link PriorityQueue#comparator}, but returns {@link Ordering#natural}
	 ...
	 * constructing one directly, use {@link MinMaxPriorityQueue#orderedBy(Comparator)},
	 * {@link MinMaxPriorityQueue#expectedSize(int)} or
	 * {@link MinMaxPriorityQueue#maximumSize(int)}.
	 *
	 * @param <B> the upper bound on the eventual type that can be produced by this
	 * builder (for example, a {@code Builder<Number>} can produce a {@code Queue<Number>}
	 * or {@code
	...
	public static final class Builder<B> {
		...
		/*
		 * TODO(kevinb): when the dust settles, see if we still need this or can just
		 * default to DEFAULT_CAPACITY.
		 */
		...
		/**
		 * Configures this builder to build {@code MinMaxPriorityQueue} instances that are
		 * limited to {@code maximumSize} elements. Each time a queue grows beyond this
		 * bound, it immediately removes its greatest element (according to its
		 * comparator), which might be the element that was just added.
		 */
		@CanIgnoreReturnValue
		...
		/**
		 * Builds a new min-max priority queue using the previously specified options, and
		...
		/**
		 * Builds a new min-max priority queue using the previously specified options, and
		 * having the given initial elements.
		 */
	...
	/**
	 * Each instance of MinMaxPriortyQueue encapsulates two instances of Heap: a min-heap
	 * and a max-heap. Conceptually, these might each have their own array for storage,
	 * but for efficiency's sake they are stored interleaved on alternate heap levels in
	 * the same array (MMPQ.queue).
	 */
	@WeakOuter
	private class Heap {
		...
		/**
		 * Tries to move {@code toTrickle} from a min to a max level and bubble up there.
		 * If it moved before {@code removeIndex} this method returns a pair as described
		 * in {@link #removeAt}.
		 */
		@CheckForNull
		...
		/**
		 * Bubbles a value from {@code index} up the levels of this heap, and returns the
		 * index the element ended up at.
		 */
		@CanIgnoreReturnValue
		...
		/**
		 * Swap {@code actualLastElement} with the conceptually correct last element of
		 * the heap. Returns the index that {@code actualLastElement} now resides in.
		 *
		 * <p>
		 ...
		 * invariant if this element becomes the new parent of the uncle. In that case, we
		...
		/**
		 * Crosses an element over to the opposite heap by moving it one level down (or up
		 * if there are no elements below it).
		 *
		 * <p>
		 * Returns the new position of the element.
		 */
		...
		/**
		 * Fills the hole at {@code index} by moving in the least of its grandchildren to
		 * this position, then recursively filling the new hole created.
	...
	private class QueueIterator implements Iterator<E> {
		...
		/**
		 * Returns true if an exact reference (==) was found and removed from the supplied
		 * iterable.
		 */
		...
		/**
		 * Advances nextCursor to the index of the first element after {@code c} that is
		 * not in {@code

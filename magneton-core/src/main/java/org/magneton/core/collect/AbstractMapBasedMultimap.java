...
/**
 * Basic implementation of the {@link Multimap} interface. This class represents a
 * multimap as a map that associates each key with a collection of values. All methods of
 * {@link Multimap} are supported, including those specified as optional in the interface.
 *
 * <p>
 * To implement a multimap, a subclass must define the method {@link #createCollection()},
 * which creates an empty collection of values for a key.
 *
 * <p>
 * The multimap constructor takes a map that has a single entry for each distinct key.
 * When you insert a key-value pair with a key that isn't already in the multimap, {@code
 * AbstractMapBasedMultimap} calls {@link #createCollection()} to create the collection of
 * values for that key. The subclass should not call {@link #createCollection()} directly,
 * and a new instance should be created every time the method is called.
 *
 * <p>
 * For example, the subclass could pass a {@link java.util.TreeMap} during construction,
 * and {@link #createCollection()} could return a {@link java.util.TreeSet}, in which case
 * the multimap's iterators would propagate through the keys and values in sorted order.
 *
 * <p>
 ...
 *
 * <p>
 * The collections created by {@link #createCollection()} may or may not allow duplicates.
 * If the collection, such as a {@link Set}, does not support duplicates, an added
 * key-value pair will replace an existing pair with the same key and value, if such a
 * pair is present. With collections like {@link List} that allow duplicates, the
 * collection will keep the existing key-value pairs while adding a new pair.
 *
 * <p>
 * This class is not threadsafe when any concurrent operations update the multimap, even
 * if the underlying map and {@link #createCollection()} method return threadsafe classes.
 * Concurrent read operations will work correctly. To allow concurrent update operations,
 * wrap your multimap with a call to
 * {@link org.magneton.core.collect.Multimaps#synchronizedMultimap}.
 *
 * <p>
...
abstract class AbstractMapBasedMultimap<K extends Object, V extends Object>
		extends org.magneton.core.collect.AbstractMultimap<K, V> implements Serializable {

	/*
	 * Here's an outline of the overall design.
	 *
	 * The map variable contains the collection of values associated with each key. When a
	 * key-value pair is added to a multimap that didn't previously contain any values for
	 * that key, a new collection generated by createCollection is added to the map. That
	 * same collection instance remains in the map as long as the multimap has any values
	 * for the key. If all values for the key are removed, the key and collection are
	 * removed from the map.
	 *
	 * The get method returns a WrappedCollection, which decorates the collection in the
	 * map (if the key is present) or an empty collection (if the key is not present).
	 * When the collection delegate in the WrappedCollection is empty, the multimap may
	 * contain subsequently added values for that key. To handle that situation, the
	 * WrappedCollection checks whether map contains an entry for the provided key, and if
	 * so replaces the delegate.
	 */
	 ...
	 *
	 * <p>
	 * The returned collection class determines whether duplicate key-value pairs are
	 * allowed.
	 ...
	 * simply calls {@link #createCollection()}, which is the correct behavior for most
	 * implementations. The {@link LinkedHashMultimap} class overrides it.
	...
	/**
	 * Generates a decorated collection that remains consistent with the values in the
	 * multimap for the provided key. Changes to the multimap may alter the returned
	 ...
	 *
	 * <p>
	 * The iterator generated by the returned collection traverses the values for one key,
	 * followed by the values of a second key, and so on.
	 */
	@Override
	...
	Spliterator<V> valueSpliterator() {
		return org.magneton.core.collect.CollectSpliterators.flatMap(map.values().spliterator(),
				Collection::spliterator, Spliterator.SIZED, size());
	}
	 ...
	 *
	 * <p>
	 * Each entry is an immutable snapshot of a key-value mapping in the multimap, taken
	 * at the time the entry is returned by a method call to the collection or its
	 * iterator.
	 */
	@Override
	 ...
	 * {@code entries().iterator()} and {@code values().iterator()}. The default behavior,
	 * which traverses the values for one key, the values for a second key, and so on,
	 * suffices for most {@code AbstractMapBasedMultimap} implementations.
	...
	Spliterator<Entry<K, V>> entrySpliterator() {
		return org.magneton.core.collect.CollectSpliterators.flatMap(map.entrySet().spliterator(),
				keyToValueCollectionEntry -> {
					...
					return org.magneton.core.collect.CollectSpliterators.map(valueCollection.spliterator(),
	...
	/**
	 * Collection decorator that stays in sync with the multimap values for a key. There
	 * are two kinds of wrapped collections: full and subcollections. Both have a delegate
	 * pointing to the underlying collection class.
	 *
	 * <p>
	 * Full collections, identified by a null ancestor field, contain all multimap values
	 * for a given key. Its delegate is a value in {@link AbstractMapBasedMultimap#map}
	 * whenever the delegate is non-empty. The {@code refreshIfEmpty},
	 * {@code removeIfEmpty}, and {@code addToMap} methods ensure that the
	 * {@code WrappedCollection} and map remain consistent.
	 *
	 * <p>
	 * A subcollection, such as a sublist, contains some of the values for a given key.
	 * Its ancestor field points to the full wrapped collection with all values for the
	 * key. The subcollection {@code refreshIfEmpty}, {@code removeIfEmpty}, and
	 * {@code addToMap} methods call the corresponding methods of the full wrapped
	 * collection.
	 */
	@WeakOuter
	class WrappedCollection extends AbstractCollection<V> {
		...
		WrappedCollection(@ParametricNullness K key, Collection<V> delegate, @CheckForNull WrappedCollection ancestor) {
			this.key = key;
	...
	/*
	 * TODO(kevinb): should we copy this javadoc to each concrete class, so that classes
	 * like LinkedHashMultimap that need to say something different are still able to
	 * {@inheritDoc} all the way from Multimap?
	 */
	...
	class WrappedSortedSet extends WrappedCollection implements SortedSet<V> {
		...
		WrappedSortedSet(@ParametricNullness K key, SortedSet<V> delegate, @CheckForNull WrappedCollection ancestor) {
	...
	class WrappedNavigableSet extends WrappedSortedSet implements NavigableSet<V> {
		...
		WrappedNavigableSet(@ParametricNullness K key, NavigableSet<V> delegate,
		...
		@CheckForNull
		public V pollLast() {
			return org.magneton.core.collect.Iterators.pollNext(descendingIterator());
		}
		...
		@Override
		public NavigableSet<V> subSet(@ParametricNullness V fromElement, boolean fromInclusive,
				@ParametricNullness V toElement, boolean toInclusive) {
	...
	class WrappedList extends WrappedCollection implements List<V> {
		...
		WrappedList(@ParametricNullness K key, List<V> delegate, @CheckForNull WrappedCollection ancestor) {
			super(key, delegate, ancestor);
		}
	...
	/**
	 * List decorator that stays in sync with the multimap values for a key and supports
	 * rapid random access.
	 */
	private class RandomAccessWrappedList extends WrappedList implements RandomAccess {

		RandomAccessWrappedList(@ParametricNullness K key, List<V> delegate, @CheckForNull WrappedCollection ancestor) {
			super(key, delegate, ancestor);
		}
	...
	private class KeySet extends Maps.KeySet<K, Collection<V>> {
		...
		public Iterator<K> iterator() {
			...
			return new Iterator<K>() {
				...
				@Override
				public void remove() {
					Preconditions.checkState(entry != null, "no calls to next() since the last call to remove()");
		...
		@Override
		public void clear() {
			org.magneton.core.collect.Iterators.clear(iterator());
		}
	...
	private abstract class Itr<T> implements Iterator<T> {
		...
		Itr() {
			...
			key = null;
			collection = null;
			valueIterator = org.magneton.core.collect.Iterators.emptyModifiableIterator();
		}
		...
		public T next() {
			...
			/*
			 * uncheckedCastNullableTToT is safe: The first call to this method always
			 * enters the !hasNext() case and populates key, after which it's never
			 * cleared.
			 */
		...
		public void remove() {
			...
			/*
			 * requireNonNull is safe because we've already initialized `collection`. If
			 * we hadn't, then valueIterator.remove() would have failed.
			 */
	...
	private class AsMap extends Maps.ViewCachingAbstractMap<K, Collection<V>> {
		...
		/**
		 * Usually the same as map, but smaller for the headMap(), tailMap(), or subMap()
		 * of a SortedAsMap.
		 */
		...
		class AsMapEntries extends Maps.EntrySet<K, Collection<V>> {
			...
			public Spliterator<Entry<K, Collection<V>>> spliterator() {
				return org.magneton.core.collect.CollectSpliterators.map(submap.entrySet().spliterator(),
		...
		class AsMapIterator implements Iterator<Entry<K, Collection<V>>> {
			...
			@Override
			public void remove() {
				Preconditions.checkState(collection != null, "no calls to next() since the last call to remove()");
				delegateIterator.remove();
				totalSize -= collection.size();
				collection.clear();
				collection = null;
	...
	class NavigableAsMap extends SortedAsMap implements NavigableMap<K, Collection<V>> {
		...
		@Override
		public NavigableMap<K, Collection<V>> subMap(@ParametricNullness K fromKey, boolean fromInclusive,

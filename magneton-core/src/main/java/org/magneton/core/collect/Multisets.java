...
/**
 * Provides static utility methods for creating and working with {@link Multiset}
 * instances.
 *
 * <p>
...
public final class Multisets {
	...
	/**
	 * Returns a {@code Collector} that accumulates elements into a multiset created via
	 * the specified {@code Supplier}, whose elements are the result of applying
	 * {@code elementFunction} to the inputs, with counts equal to the result of applying
	 * {@code countFunction} to the inputs. Elements are added in encounter order.
	 *
	 * <p>
	 * If the mapped elements contain duplicates (according to {@link Object#equals}), the
	 * element will be added more than once, with the count summed over all appearances of
	 * the element.
	 *
	 * <p>
	 * Note that {@code stream.collect(toMultiset(function, e -> 1, supplier))} is
	 * equivalent to
	 * {@code stream.map(function).collect(Collectors.toCollection(supplier))}.
	 *
	 * <p>
	 * To collect to an {@link ImmutableMultiset}, use
	 * {@link ImmutableMultiset#toImmutableMultiset}.
	 *
	 * @since 22.0
	 */
	public static <T, E, M extends Multiset<E>> Collector<T, ?, M> toMultiset(Function<? super T, E> elementFunction,
			ToIntFunction<? super T> countFunction, Supplier<M> multisetSupplier) {
		return CollectCollectors.toMultiset(elementFunction, countFunction, multisetSupplier);
	}

	/**
	 * Returns an unmodifiable view of the specified multiset. Query operations on the
	 * returned multiset "read through" to the specified multiset, and attempts to modify
	 * the returned multiset result in an {@link UnsupportedOperationException}.
	 *
	 * <p>
	 ...
	 */
	public static <E extends Object> Multiset<E> unmodifiableMultiset(Multiset<? extends E> multiset) {
	...
	/**
	 * Returns an unmodifiable view of the specified sorted multiset. Query operations on
	 * the returned multiset "read through" to the specified multiset, and attempts to
	 * modify the returned multiset result in an {@link UnsupportedOperationException}.
	 *
	 * <p>
	 * The returned multiset will be serializable if the specified multiset is
	 * serializable.
	 * @param sortedMultiset the sorted multiset for which an unmodifiable view is to be
	 * generated
	 * @return an unmodifiable view of the multiset
	 * @since 11.0
	 */

	public static <E extends Object> SortedMultiset<E> unmodifiableSortedMultiset(SortedMultiset<E> sortedMultiset) {
	...
	/**
	 * Returns an immutable multiset entry with the specified element and count. The entry
	 * will be serializable if {@code e} is.
	 * @param e the element to be associated with the returned entry
	 * @param n the count to be associated with the returned entry
	 * @throws IllegalArgumentException if {@code n} is negative
	 */
	public static <E extends Object> Multiset.Entry<E> immutableEntry(@ParametricNullness E e, int n) {
		return new ImmutableEntry<E>(e, n);
	}

	/**
	 * Returns a view of the elements of {@code unfiltered} that satisfy a predicate. The
	 * returned multiset is a live view of {@code unfiltered}; changes to one affect the
	 * other.
	 *
	 * <p>
	 * The resulting multiset's iterators, and those of its {@code entrySet()} and {@code
	 * elementSet()}, do not support {@code remove()}. However, all other multiset methods
	 * supported by {@code unfiltered} are supported by the returned multiset. When given
	 * an element that doesn't satisfy the predicate, the multiset's {@code add()} and
	 * {@code addAll()} methods throw an {@link IllegalArgumentException}. When methods
	 * such as {@code removeAll()} and {@code
	 ...
	 *
	 * <p>
	 * The returned multiset isn't threadsafe or serializable, even if {@code unfiltered}
	 * is.
	 *
	 * <p>
	 * Many of the filtered multiset's methods, such as {@code size()}, iterate across
	 * every element in the underlying multiset and determine which elements satisfy the
	...
	public static <E extends Object> Multiset<E> filter(Multiset<E> unfiltered,
		...
		if (unfiltered instanceof FilteredMultiset) {
			...
			org.magneton.core.base.Predicate<E> combinedPredicate = Predicates.<E>and(filtered.predicate, predicate);
	...
	/**
	 * Returns the expected number of distinct elements given the specified elements. The
	 * number of distinct elements is only computed if {@code elements} is an instance of
	 * {@code Multiset}; otherwise the default value of 11 is returned.
	 */
	 ...
	 * multiset, the count of each element is the <i>maximum</i> of its counts in the two
	 * backing multisets. The iteration order of the returned multiset matches that of the
	 * element set of {@code multiset1} followed by the members of the element set of
	 * {@code multiset2} that are not contained in {@code
	 ...
	 * different equivalence relations (as {@code HashMultiset} and {@code TreeMultiset}
	 * are).
	 *
	 * @since 14.0
	 */

	public static <E extends Object> Multiset<E> union(Multiset<? extends E> multiset1,
	...
	/**
	 * Returns an unmodifiable view of the intersection of two multisets. In the returned
	 * multiset, the count of each element is the <i>minimum</i> of its counts in the two
	 * backing multisets, with elements that would have a count of 0 not included. The
	 * iteration order of the returned multiset matches that of the element set of
	 ...
	 * @since 2.0
	 */
	public static <E extends Object> Multiset<E> intersection(Multiset<E> multiset1, Multiset<?> multiset2) {
		Preconditions.checkNotNull(multiset1);
		Preconditions.checkNotNull(multiset2);
	...
	/**
	 * Returns an unmodifiable view of the sum of two multisets. In the returned multiset,
	 * the count of each element is the <i>sum</i> of its counts in the two backing
	 * multisets. The iteration order of the returned multiset matches that of the element
	 * set of {@code multiset1} followed by the members of the element set of
	 * {@code multiset2} that are not contained in {@code
	 * multiset1}, with repeated occurrences of the same element appearing consecutively.
	 *
	 * <p>
	...
	public static <E extends Object> Multiset<E> sum(Multiset<? extends E> multiset1, Multiset<? extends E> multiset2) {
		Preconditions.checkNotNull(multiset1);
		Preconditions.checkNotNull(multiset2);
	...
	/**
	 * Returns an unmodifiable view of the difference of two multisets. In the returned
	 * multiset, the count of each element is the result of the <i>zero-truncated
	 * subtraction</i> of its count in the second multiset from its count in the first
	 * multiset, with elements that would have a count of 0 not included. The iteration
	 * order of the returned multiset matches that of the element set of
	 * {@code multiset1}, with repeated occurrences of the same element appearing
	 * consecutively.
	 *
	 * <p>
	...
	public static <E extends Object> Multiset<E> difference(Multiset<E> multiset1, Multiset<?> multiset2) {
		Preconditions.checkNotNull(multiset1);
		Preconditions.checkNotNull(multiset2);
	...
	/**
	 * Returns {@code true} if {@code subMultiset.count(o) <= superMultiset.count(o)} for
	 * all {@code
	 * o}.
	 *
	 * @since 10.0
	...
	/**
	 * Modifies {@code multisetToModify} so that its count for an element {@code e} is at
	 * most {@code
	 * multisetToRetain.count(e)}.
	 *
	 * <p>
	 * To be precise, {@code multisetToModify.count(e)} is set to {@code
	 * Math.min(multisetToModify.count(e), multisetToRetain.count(e))}. This is similar to
	 * {@link #intersection(Multiset, Multiset) intersection}
	 * {@code (multisetToModify, multisetToRetain)}, but mutates {@code multisetToModify}
	 * instead of returning a view.
	 *
	 * <p>
	 * In contrast, {@code multisetToModify.retainAll(multisetToRetain)} keeps all
	 * occurrences of elements that appear at all in {@code multisetToRetain}, and deletes
	 * all occurrences of all other elements.
	 * @return {@code true} if {@code multisetToModify} was changed as a result of this
	 * operation
	 * @since 10.0
	 */
	@CanIgnoreReturnValue
	...
	private static <E extends Object> boolean retainOccurrencesImpl(Multiset<E> multisetToModify,
	...
	/**
	 * For each occurrence of an element {@code e} in {@code occurrencesToRemove}, removes
	 * one occurrence of {@code e} in {@code multisetToModify}.
	 *
	 * <p>
	 ...
	 * removeAll}{@code (occurrencesToRemove)}, which removes all occurrences of elements
	 * that appear in {@code occurrencesToRemove}. However, this operation <i>is</i>
	 * equivalent to, albeit sometimes more efficient than, the following:
	 *
	 * <pre>{@code
	 ...
	 * @return {@code true} if {@code multisetToModify} was changed as a result of this
	 * operation
	 * @since 10.0 (missing in 18.0 when only the overload taking an {@code Iterable} was
	 * present)
	 */
	@CanIgnoreReturnValue
	...
	static boolean equalsImpl(Multiset<?> multiset, @CheckForNull Object object) {
		...
		if (object instanceof Multiset) {
			 ...
			 * fails when a TreeMultiset has a comparator that returns 0 when passed
			 * unequal elements.
			 */
	...
	static <E extends Object> boolean addAllImpl(Multiset<E> self, Collection<? extends E> elements) {
	...
	/**
	 * A specialization of {@code addAllImpl} for when {@code elements} is itself a
	 * Multiset.
	 */
	private static <E extends Object> boolean addAllImpl(Multiset<E> self, Multiset<? extends E> elements) {
		if (elements.isEmpty()) {
			return false;
		}
	...
	static <E extends Object> int setCountImpl(Multiset<E> self, @ParametricNullness E element, int count) {
	...
	static <E extends Object> boolean setCountImpl(Multiset<E> self, @ParametricNullness E element, int oldCount,
			int newCount) {
	...
	static <E extends Object> Iterator<E> elementIterator(Iterator<Multiset.Entry<E>> entryIterator) {
	 ...
	 * order is highest count first, with ties broken by the iteration order of the
	 * original multiset.
	 *
	 * @since 11.0
	 */
	...
	public static <E> ImmutableMultiset<E> copyHighestCountFirst(Multiset<E> multiset) {
		Multiset.Entry<E>[] entries = (Multiset.Entry<E>[]) multiset.entrySet().toArray(new Multiset.Entry[0]);
		Arrays.sort(entries, DecreasingCount.INSTANCE);
		return ImmutableMultiset.copyFromEntries(Arrays.asList(entries));
	}

	static class UnmodifiableMultiset<E extends Object> extends ForwardingMultiset<E> implements Serializable {
	...
	static class ImmutableEntry<E extends Object> extends AbstractEntry<E> implements Serializable {
	...
	private static final class FilteredMultiset<E extends Object> extends ViewMultiset<E> {
		...
		final org.magneton.core.base.Predicate<? super E> predicate;

		FilteredMultiset(Multiset<E> unfiltered, org.magneton.core.base.Predicate<? super E> predicate) {
	...
	abstract static class AbstractEntry<E extends Object> implements Multiset.Entry<E> {
		...
		/**
		 * Indicates whether an object equals this entry, following the behavior specified
		 * in {@link Multiset.Entry#equals}.
		 */
		@Override
	...
	abstract static class EntrySet<E extends Object> extends Sets.ImprovedAbstractSet<Multiset.Entry<E>> {
	...
	private static final class DecreasingCount implements Comparator<Multiset.Entry<?>> {
		...
		public int compare(Multiset.Entry<?> entry1, Multiset.Entry<?> entry2) {
			return entry2.getCount() - entry1.getCount(); // subtracting two nonnegative
															// integers
		}
	...
	/**
	 * An {@link AbstractMultiset} with additional default implementations, some of them
	 * linear-time implementations in terms of {@code elementSet} and {@code entrySet}.
	 */
	private abstract static class ViewMultiset<E extends Object> extends AbstractMultiset<E> {

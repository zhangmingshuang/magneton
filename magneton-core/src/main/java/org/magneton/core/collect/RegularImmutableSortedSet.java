...
/**
 * An immutable sorted set with one or more elements. TODO(jlevy): Consider separate class
 * for a single-element sorted set.
 *
 * @author Jared Levy
...
final class RegularImmutableSortedSet<E> extends org.magneton.core.collect.ImmutableSortedSet<E> {

	static final RegularImmutableSortedSet<Comparable> NATURAL_EMPTY_SET = new RegularImmutableSortedSet<>(
			org.magneton.core.collect.ImmutableList.<Comparable>of(), Ordering.natural());

	private final transient org.magneton.core.collect.ImmutableList<E> elements;

	RegularImmutableSortedSet(org.magneton.core.collect.ImmutableList<E> elements, Comparator<? super E> comparator) {
		super(comparator);
	...
	public boolean containsAll(Collection<?> targets) {
		...
		if (!org.magneton.core.collect.SortedIterables.hasSameComparator(comparator(), targets)
				|| (targets.size() <= 1)) {
			return super.containsAll(targets);
		}

		/*
		 * If targets is a sorted set with the same comparator, containsAll can run in
		 * O(n) time stepping through the two collections.
		 */
	...
	public boolean equals(@CheckForNull Object object) {
		...
		if (org.magneton.core.collect.SortedIterables.hasSameComparator(comparator, that)) {
	...
	int headIndex(E toElement, boolean inclusive) {
		int index = Collections.binarySearch(elements, Preconditions.checkNotNull(toElement), comparator());
		if (index >= 0) {
	...
	int tailIndex(E fromElement, boolean inclusive) {
		int index = Collections.binarySearch(elements, Preconditions.checkNotNull(fromElement), comparator());
		if (index >= 0) {
	...
	ImmutableList<E> createAsList() {
		return (size() <= 1) ? elements : new org.magneton.core.collect.ImmutableSortedAsList<E>(this, elements);
	}

	@Override
	ImmutableSortedSet<E> createDescendingSet() {
		Comparator<? super E> reversedOrder = Collections.reverseOrder(comparator);
		return isEmpty() ? emptySet(reversedOrder)

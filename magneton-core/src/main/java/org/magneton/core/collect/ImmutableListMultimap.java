...
public class ImmutableListMultimap<K, V> extends org.magneton.core.collect.ImmutableMultimap<K, V>
	...
	ImmutableListMultimap(org.magneton.core.collect.ImmutableMap<K, org.magneton.core.collect.ImmutableList<V>> map,
			int size) {
		super(map, size);
	}
	 ...
	 * {@code ImmutableListMultimap} whose keys and values are the result of applying the
	 ...
	 * @since 21.0
	 */
	public static <T extends Object, K, V> Collector<T, ?, ImmutableListMultimap<K, V>> toImmutableListMultimap(
			Function<? super T, ? extends K> keyFunction, Function<? super T, ? extends V> valueFunction) {
		return org.magneton.core.collect.CollectCollectors.toImmutableListMultimap(keyFunction, valueFunction);
	}

	/**
	 * Returns a {@code Collector} accumulating entries into an
	 * {@code ImmutableListMultimap}. Each input element is mapped to a key and a stream
	 * of values, each of which are put into the resulting {@code Multimap}, in the
	 * encounter order of the stream and the encounter order of the streams of values.
	 *
	 * <p>
	 * Example:
	 *
	 ...
	 * @since 21.0
	 */
	public static <T extends Object, K, V> Collector<T, ?, ImmutableListMultimap<K, V>> flatteningToImmutableListMultimap(
			Function<? super T, ? extends K> keyFunction,
			Function<? super T, ? extends Stream<? extends V>> valuesFunction) {
		return org.magneton.core.collect.CollectCollectors.flatteningToImmutableListMultimap(keyFunction,
				valuesFunction);
	}
	...
	/**
	 * Returns an immutable multimap containing the same mappings as {@code multimap}. The
	 * generated multimap's key and value orderings correspond to the iteration ordering
	 * of the {@code
	 * multimap.asMap()} view.
	 *
	 * <p>
	...
	static <K, V> ImmutableListMultimap<K, V> fromMapEntries(
		...
		org.magneton.core.collect.ImmutableMap.Builder<K, org.magneton.core.collect.ImmutableList<V>> builder = new org.magneton.core.collect.ImmutableMap.Builder<>(
				mapEntries.size());
		int size = 0;
		...
		for (Entry<? extends K, ? extends Collection<? extends V>> entry : mapEntries) {
			...
			org.magneton.core.collect.ImmutableList<V> list = (valueComparator == null)
					? org.magneton.core.collect.ImmutableList.copyOf(values)
					: org.magneton.core.collect.ImmutableList.sortedCopyOf(valueComparator, values);
			if (!list.isEmpty()) {
	...
	/**
	 * Returns an immutable list of the values for the given key. If no mappings in the
	 * multimap have the provided key, an empty immutable list is returned. The values are
	 * in the same order as the parameters used to build this multimap.
	 */
	@Override
	public org.magneton.core.collect.ImmutableList<V> get(K key) {
		// This cast is safe as its type is known in constructor.
		org.magneton.core.collect.ImmutableList<V> list = (org.magneton.core.collect.ImmutableList<V>) map.get(key);
		return (list == null) ? org.magneton.core.collect.ImmutableList.<V>of() : list;
	}
	 ...
	 *
	 * <p>
	 * Because an inverse of a list multimap can contain multiple pairs with the same key
	 * and value, this method returns an {@code ImmutableListMultimap} rather than the
	 * {@code
	...
	public final org.magneton.core.collect.ImmutableList<V> removeAll(@CheckForNull Object key) {
	...
	@Override
	@DoNotCall("Always throws UnsupportedOperationException")
	public final org.magneton.core.collect.ImmutableList<V> replaceValues(K key, Iterable<? extends V> values) {
		throw new UnsupportedOperationException();
	}
	...
	private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
		...
		org.magneton.core.collect.ImmutableMap.Builder<Object, org.magneton.core.collect.ImmutableList<Object>> builder = org.magneton.core.collect.ImmutableMap
				.builder();
		int tmpSize = 0;
		...
		for (int i = 0; i < keyCount; i++) {
			...
			org.magneton.core.collect.ImmutableList.Builder<Object> valuesBuilder = org.magneton.core.collect.ImmutableList
					.builder();

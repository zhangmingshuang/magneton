...
/**
 * An implementation of {@link org.magneton.core.collect.RangeSet} backed by a
 * {@link TreeMap}.
 *
...
public class TreeRangeSet<C extends Comparable<?>> extends AbstractRangeSet<C> implements Serializable {
	...
	@CheckForNull
	private transient org.magneton.core.collect.RangeSet<C> complement;
	...
	/**
	 * Returns a {@code TreeRangeSet} initialized with the ranges in the specified range
	 * set.
	 */
	public static <C extends Comparable<?>> TreeRangeSet<C> create(org.magneton.core.collect.RangeSet<C> rangeSet) {
	 ...
	 * An element will be contained in this {@code RangeSet} if and only if it is
	 * contained in at least one {@code Range} in {@code ranges}.
	 *
	 * @since 21.0
	 */
	...
	public Set<Range<C>> asDescendingSetOfRanges() {
		...
		return (result == null) ? asDescendingSetOfRanges = new AsRanges(rangesByLowerBound.descendingMap().values())
				: result;
	}
	...
	public boolean intersects(Range<C> range) {
		...
		if (ceilingEntry != null && ceilingEntry.getValue().isConnected(range)
		...
		return priorEntry != null && priorEntry.getValue().isConnected(range)
	...
	public Range<C> span() {
		...
		if (firstEntry == null || lastEntry == null) {
			/*
			 * Either both are null or neither is: Either the set is empty, or it's not.
			 * But we check both to make the nullness checker happy.
			 */
		...
		if (entryBelowLB != null) {
			...
			if (rangeBelowLB.upperBound.compareTo(lbToAdd) >= 0) {
				...
				if (rangeBelowLB.upperBound.compareTo(ubToAdd) >= 0) {
					...
					/*
					 * TODO(cpovirk): can we just "return;" here? Or, can we remove this
					 * if() entirely? If not, add tests to demonstrate the problem with
					 * each approach
					 */
				}
	...
	@Override
	public org.magneton.core.collect.RangeSet<C> complement() {
		org.magneton.core.collect.RangeSet<C> result = complement;
		return (result == null) ? complement = new Complement() : result;
	}

	@Override
	public org.magneton.core.collect.RangeSet<C> subRangeSet(Range<C> view) {
	...
	static final class RangesByUpperBound<C extends Comparable<?>> extends AbstractNavigableMap<Cut<C>, Range<C>> {
		...
		public NavigableMap<Cut<C>, Range<C>> subMap(Cut<C> fromKey, boolean fromInclusive, Cut<C> toKey,
				boolean toInclusive) {
			return subMap(Range.range(fromKey, org.magneton.core.collect.BoundType.forBoolean(fromInclusive), toKey,
					org.magneton.core.collect.BoundType.forBoolean(toInclusive)));
		}

		@Override
		public NavigableMap<Cut<C>, Range<C>> headMap(Cut<C> toKey, boolean inclusive) {
			return subMap(Range.upTo(toKey, org.magneton.core.collect.BoundType.forBoolean(inclusive)));
		}

		@Override
		public NavigableMap<Cut<C>, Range<C>> tailMap(Cut<C> fromKey, boolean inclusive) {
			return subMap(Range.downTo(fromKey, org.magneton.core.collect.BoundType.forBoolean(inclusive)));
		}
		...
		Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
			/*
			 * We want to start the iteration at the first range where the upper bound is
			 * in upperBoundWindow.
			 */
		...
		Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
			...
			PeekingIterator<Range<C>> backingItr = org.magneton.core.collect.Iterators
					.peekingIterator(candidates.iterator());
		...
		public int size() {
			...
			return org.magneton.core.collect.Iterators.size(entryIterator());
		}
	...
	private static final class ComplementRangesByLowerBound<C extends Comparable<?>>
		 ...
		 * entire "complement ranges by lower bound" map; it's a constraint on the *keys*,
		 * and does not affect the values.
		 */
		...
		private ComplementRangesByLowerBound(NavigableMap<Cut<C>, Range<C>> positiveRangesByLowerBound,
			...
			positiveRangesByUpperBound = new RangesByUpperBound<>(positiveRangesByLowerBound);
		...
		Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
			...
			if (complementLowerBoundWindow.hasLowerBound()) {
				positiveRanges = positiveRangesByUpperBound.tailMap(complementLowerBoundWindow.lowerEndpoint(),
						complementLowerBoundWindow.lowerBoundType() == org.magneton.core.collect.BoundType.CLOSED)
						.values();
			}
			else {
				positiveRanges = positiveRangesByUpperBound.values();
			}
			PeekingIterator<Range<C>> positiveItr = org.magneton.core.collect.Iterators
					.peekingIterator(positiveRanges.iterator());
		...
		Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
			...
			Cut<C> startingPoint = complementLowerBoundWindow.hasUpperBound()
					? complementLowerBoundWindow.upperEndpoint() : Cut.<C>aboveAll();
			boolean inclusive = complementLowerBoundWindow.hasUpperBound()
					&& complementLowerBoundWindow.upperBoundType() == org.magneton.core.collect.BoundType.CLOSED;
			PeekingIterator<Range<C>> positiveItr = org.magneton.core.collect.Iterators.peekingIterator(
	...
	private static final class SubRangeSetRangesByLowerBound<C extends Comparable<?>>
		...
		/**
		 * restriction is the subRangeSet view; ranges are truncated to their intersection
		 * with restriction.
		 */
		...
		private SubRangeSetRangesByLowerBound(Range<Cut<C>> lowerBoundWindow, Range<C> restriction,
			...
			rangesByUpperBound = new RangesByUpperBound<>(rangesByLowerBound);
		}
		...
		Iterator<Entry<Cut<C>, Range<C>>> entryIterator() {
			...
			if (lowerBoundWindow.upperBound.isLessThan(restriction.lowerBound)) {
				...
				// starts at the first range with lower bound above
				// lowerBoundWindow.lowerBound
				completeRangeItr = rangesByLowerBound
						.tailMap(lowerBoundWindow.lowerBound.endpoint(),
								lowerBoundWindow.lowerBoundType() == org.magneton.core.collect.BoundType.CLOSED)
						.values().iterator();
			}
		...
		Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
			...
			Cut<Cut<C>> upperBoundOnLowerBounds = Ordering.natural().min(lowerBoundWindow.upperBound,
					Cut.belowValue(restriction.upperBound));
			Iterator<Range<C>> completeRangeItr = rangesByLowerBound
	...
	private final class SubRangeSet extends TreeRangeSet<C> {
		...
		@Override
		public void add(Range<C> rangeToAdd) {
			Preconditions.checkArgument(restriction.encloses(rangeToAdd), "Cannot add range %s to subRangeSet(%s)",

...
/**
 * A {@link Set} whose contents will never change, with many other important properties
...
public abstract class ImmutableSet<E> extends ImmutableCollection<E> implements Set<E> {
	 ...
	 * ImmutableSet}. Elements appear in the resulting set in the encounter order of the
	 * stream; if the stream contains duplicates (according to
	 * {@link Object#equals(Object)}), only the first duplicate in encounter order will
	...
	/**
	 * Returns the empty immutable set. Preferred over {@link Collections#emptySet} for
	 * code consistency, and because the return type conveys the immutability guarantee.
	 *
	 * <p>
	 * <b>Performance note:</b> the instance returned is a singleton.
	 */
	// fully variant implementation (never actually
	// produces any Es)
	 ...
	 * {@link Collections#singleton} for code consistency, {@code null} rejection, and
	...
	/**
	 * Returns an immutable set containing the given elements, minus duplicates, in the
	 * order each was first specified. That is, if multiple elements are
	 * {@linkplain Object#equals equal}, all except the first are ignored.
	 */
	...
	public static <E> ImmutableSet<E> of(E e1, E e2, E e3, E e4, E e5, E e6, E... others) {
		Preconditions.checkArgument(others.length <= Integer.MAX_VALUE - 6,
	 ...
	 * specified array, which we have no particular reason to believe does or does not
	 * contain duplicates. If {@code k} is the size of the returned {@code ImmutableSet},
	 * then the unique elements of {@code elements} will be in the first {@code k}
	 * positions, and {@code elements[i] == null} for {@code k <= i <
	 * n}.
	 *
	 * <p>
	 * This may modify {@code elements}. Additionally, if {@code n == elements.length} and
	 * {@code
	 * elements} contains no duplicates, {@code elements} may be used without copying in
	 * the returned {@code ImmutableSet}, in which case the caller must not modify it.
	 *
	 * <p>
	...
	private static <E> ImmutableSet<E> constructUnknownDuplication(int n, Object... elements) {
		// Guess the size is "halfway between" all duplicates and no duplicates, on a log
		// scale.
		return construct(n, Math.max(ImmutableCollection.Builder.DEFAULT_INITIAL_CAPACITY,
				org.magneton.core.math.IntMath.sqrt(n, RoundingMode.CEILING)), elements);
	}

	/**
	 * Constructs an {@code ImmutableSet} from the first {@code n} elements of the
	 * specified array. If {@code k} is the size of the returned {@code ImmutableSet},
	 * then the unique elements of {@code
	 * elements} will be in the first {@code k} positions, and {@code elements[i] == null}
	 * for {@code
	 * k <= i < n}.
	 *
	 * <p>
	 ...
	 * {@code
	 * elements} contains no duplicates, {@code elements} may be used without copying in
	 * the returned {@code ImmutableSet}, in which case it may no longer be modified.
	 *
	 * <p>
	 * {@code elements} may contain only values of type {@code E}.
	 * @throws NullPointerException if any of the first {@code n} elements of
	...
	/**
	 * Returns an immutable set containing each of {@code elements}, minus duplicates, in
	 * the order each appears first in the source collection.
	 *
	 * <p>
	 * <b>Performance note:</b> This method will sometimes recognize that the actual copy
	 * operation is unnecessary; for example, {@code copyOf(copyOf(anArrayList))} will
	 * copy the data only once. This reduces the expense of habitually making defensive
	 * copies at API boundaries. However, the precise conditions for skipping the copy
	...
	public static <E> ImmutableSet<E> copyOf(Collection<? extends E> elements) {
		/*
		 * TODO(lowasser): consider checking for ImmutableAsList here TODO(lowasser):
		 * consider checking for Multiset here
		 */
		...
		if (elements instanceof Set) {
			// assume probably no duplicates (though it might be using different equality
			// semantics)
	...
	/**
	 * Returns an immutable set containing each of {@code elements}, minus duplicates, in
	 * the order each appears first in the source iterable. This method iterates over
	 * {@code elements} only once.
	 *
	 * <p>
	 * <b>Performance note:</b> This method will sometimes recognize that the actual copy
	 * operation is unnecessary; for example, {@code copyOf(copyOf(anArrayList))} should
	 * copy the data only once. This reduces the expense of habitually making defensive
	 * copies at API boundaries. However, the precise conditions for skipping the copy
	 * operation are undefined.
	 * @throws NullPointerException if any of {@code elements} is null
	...
	/**
	 * Returns an immutable set containing each of {@code elements}, minus duplicates, in
	 * the order each appears first in the source iterator.
	 * @throws NullPointerException if any of {@code elements} is null
	 */
	...
	/**
	 * Returns an immutable set containing each of {@code elements}, minus duplicates, in
	 * the order each appears first in the source array.
	 * @throws NullPointerException if any of {@code elements} is null
	 * @since 3.0
	 */
	...
	/**
	 * Returns a new builder, expecting the specified number of distinct elements to be
	 * added.
	 *
	 * <p>
	 * If {@code expectedSize} is exactly the number of distinct elements added to the
	 * builder before {@link Builder#build} is called, the builder is likely to perform
	 * better than an unsized {@link #builder()} would have.
	 *
	 * <p>
	 * It is not specified if any performance benefits apply if {@code expectedSize} is
	 * close to, but not exactly, the number of distinct elements added to the builder.
	 *
	 * @since 23.1
	 */
	 ...
	 * addressing with linear probing in its implementation. The returned size is the
	 * smallest power of two that can hold setSize elements with the desired load factor.
	 * Always returns at least setSize + 2.
	 */
	// TODO(cpovirk): Move to Hashing or something, since it's used elsewhere in the
	// Android version.
	...
	public boolean equals(@CheckForNull Object object) {
		...
		if (object instanceof ImmutableSet && isHashCodeFast() && ((ImmutableSet<?>) object).isHashCodeFast()
	 ...
	 * ImmutableEnumSet/ImmutableSortedSet, regardless of implementation type. It captures
	 * their "logical contents" and they are reconstructed using public static factories.
	 * This is necessary to ensure that the existence of a particular implementation type
	 ...
	 *
	 * <p>
	 * Elements appear in the resulting set in the same order they were first added to the
	 * builder.
	 *
	 * <p>
	 * Building does not change the state of the builder, so it is still possible to add
	 * more elements and to build again.
	 *
	 * @since 2.0
	 */
	public static class Builder<E> extends ImmutableCollection.Builder<E> {
		...
		/*
		 * `impl` is null only for instances of the subclass, ImmutableSortedSet.Builder.
		 * That subclass overrides all the methods that access it here. Thus, all the
		 * methods here can safely assume that this field is non-null.
		 */
		@CheckForNull
		...
		void forceJdk() {
			...
			impl = new JdkBackedSetBuilderImpl<E>(impl);
		}
		...
		Builder<E> combine(Builder<E> other) {
			 ...
			 *
			 * (And I don't believe there's any situation in which we call x.combine(y)
			 * when x is a plain ImmutableSet.Builder but y is an
			 * ImmutableSortedSet.Builder (or vice versa). Certainly
			 * ImmutableSortedSet.Builder.combine() is written as if its argument will
			 * never be a plain ImmutableSet.Builder: It casts immediately to
			 * ImmutableSortedSet.Builder.)
			 */
			copyIfNecessary();
			impl = impl.combine(other.impl);
			return this;
		}
	...
	private abstract static class SetBuilderImpl<E> {
		...
		SetBuilderImpl(int expectedCapacity) {
			dedupedElements = (E[]) new Object[expectedCapacity];
			distinct = 0;
		}

		/**
		 * Initializes this SetBuilderImpl with a copy of the deduped elements array from
		 * toCopy.
		 */
		SetBuilderImpl(SetBuilderImpl<E> toCopy) {
			dedupedElements = Arrays.copyOf(toCopy.dedupedElements, toCopy.dedupedElements.length);
			distinct = toCopy.distinct;
		}

		/**
		 * Resizes internal data structures if necessary to store the specified number of
		 * distinct elements.
		 */
		...
		/**
		 * Adds e to the insertion-order array of deduplicated elements. Calls
		 * ensureCapacity.
		 */
		...
		/**
		 * Adds e to this SetBuilderImpl, returning the updated result. Only use the
		 * returned SetBuilderImpl, since we may switch implementations if e.g. hash
		...
		final SetBuilderImpl<E> combine(SetBuilderImpl<E> other) {
			...
			for (int i = 0; i < other.distinct; i++) {
				/*
				 * requireNonNull is safe because we ensure that the first `distinct`
				 * elements have been populated.
				 */
		...
		/**
		 * Creates a new copy of this SetBuilderImpl. Modifications to that SetBuilderImpl
		 * will not affect this SetBuilderImpl or sets constructed from this
		 * SetBuilderImpl via build().
		 */
		abstract SetBuilderImpl<E> copy();

		/**
		 * Call this before build(). Does a final check on the internal data structures,
		 * e.g. shrinking unnecessarily large structures or detecting previously unnoticed
		 * hash flooding.
		 */
	 ...
	 * open-addressed hash table and deduplicating elements as they come, so it only
	 * allocates O(max(distinct, expectedCapacity)) rather than O(calls to add).
	 *
	 * <p>
	 * This implementation attempts to detect hash flooding, and if it's identified, falls
	 * back to JdkBackedSetBuilderImpl.
	 */
	private static final class RegularSetBuilderImpl<E> extends SetBuilderImpl<E> {

		/**
		 * We attempt to detect deliberate hash flooding attempts. If one is detected, we
		 * fall back to a wrapper around j.u.HashSet, which has built in flooding
		 * protection. MAX_RUN_MULTIPLIER was determined experimentally to match our
		 * desired probability of false positives.
		 */
		// NB: yes, this is surprisingly high, but that's what the experiments said was
		// necessary
		// Raising this number slows the worst-case contains behavior, speeds up
		...
		RegularSetBuilderImpl(int expectedCapacity) {
			...
			hashTable = null;
			maxRunBeforeFallback = 0;
			expandTableThreshold = 0;
		}

		RegularSetBuilderImpl(RegularSetBuilderImpl<E> toCopy) {
			super(toCopy);
			hashTable = (toCopy.hashTable == null) ? null : toCopy.hashTable.clone();
			maxRunBeforeFallback = toCopy.maxRunBeforeFallback;
			expandTableThreshold = toCopy.expandTableThreshold;
		...
		static @Nullable Object[] rebuildHashTable(int newTableSize, Object[] elements, int n) {
			...
			for (int i = 0; i < n; i++) {
				// requireNonNull is safe because we ensure that the first n elements have
				// been populated.
		...
		/**
		 * Checks the whole hash table for poor hash distribution. Takes O(n) in the worst
		 * case, O(n / log n) on average.
		 *
		 * <p>
		 * The online hash flooding detecting in RegularSetBuilderImpl.add can detect e.g.
		 * many exactly matching hash codes, which would cause construction to take
		 * O(n^2), but can't detect e.g. hash codes adversarially designed to go into
		 * ascending table locations, which keeps construction O(n) (as desired) but then
		 ...
		 *
		 * <p>
		 * Note that for a RegularImmutableSet with elements with truly random hash codes,
		 * contains operations take expected O(1) time but with high probability take
		 * O(log n) for at least some element.
		 * (https://en.wikipedia.org/wiki/Linear_probing#Analysis)
		 *
		 * <p>
		 * This method may return {@code true} even on truly random input, but {@code
		...
		static boolean hashFloodingDetected(@Nullable Object[] hashTable) {
			...
			// Invariant: all elements at indices in [knownRunStart, knownRunEnd) are
			// nonnull.
			// If knownRunStart == knownRunEnd, this is vacuously true.
			// When knownRunEnd exceeds hashTable.length, it "wraps", detecting runs
			// around the end
			// of the table.
			int knownRunStart = 0;
			int knownRunEnd = 0;
			...
			outerLoop: while (knownRunStart < hashTable.length) {
				if (knownRunStart == knownRunEnd && hashTable[knownRunStart] == null) {
					if (hashTable[(knownRunStart + maxRunBeforeFallback - 1) & mask] == null) {
						// There are only maxRunBeforeFallback - 1 elements between here
						// and there,
						// so even if they were all nonnull, we wouldn't detect a hash
						// flood. Therefore,
						// we can skip them all.
						knownRunStart += maxRunBeforeFallback;
					}
					else {
						knownRunStart++; // the only case in which maxRunEnd doesn't
											// increase by mRBF
						// happens about f * (1-f) for f = DESIRED_LOAD_FACTOR, so around
						// 21% of the time
					}
		...
		/**
		 * If more than this many consecutive positions are filled in a table of the
		 * specified size, report probable hash flooding. ({@link #hashFloodingDetected}
		 * may also report hash flooding if fewer consecutive positions are filled; see
		...
		ImmutableSet<E> build() {
			...
			default:
				/*
				 * The suppression is safe because we ensure that the first `distinct`
				 * elements have been populated.
				 */
				Object[] elements = (distinct == dedupedElements.length) ? dedupedElements
						: Arrays.copyOf(dedupedElements, distinct);
				return new RegularImmutableSet<E>(elements, hashCode, requireNonNull(hashTable), hashTable.length - 1);
			}
		}
	...
	/**
	 * SetBuilderImpl version that uses a JDK HashSet, which has built in hash flooding
	 * protection.
	 */
	private static final class JdkBackedSetBuilderImpl<E> extends SetBuilderImpl<E> {
		...
		ImmutableSet<E> build() {
			...
			case 1:
				...
				return of(requireNonNull(dedupedElements[0]));
			default:
				return new JdkBackedImmutableSet<E>(delegate, ImmutableList.asImmutableList(dedupedElements, distinct));
			}
		}

 ...
 * for a given key. A {@link HashMap} associates each key with an {@link ArrayList} of
 * values.
 *
 * <p>
 * When iterating through the collections supplied by this class, the ordering of values
 * for a given key agrees with the order in which the values were added.
 *
 * <p>
 * This multimap allows duplicate key-value pairs. After adding a new key-value pair equal
 * to an existing key-value pair, the {@code ArrayListMultimap} will contain entries for
 * both the new value and the old value.
 *
 * <p>
 ...
 * Concurrent read operations will work correctly. To allow concurrent update operations,
 * wrap your multimap with a call to {@link Multimaps#synchronizedListMultimap}.
 *
 * <p>
...
public final class ArrayListMultimap<K extends Object, V extends Object>
	...
	private ArrayListMultimap(Multimap<? extends K, ? extends V> multimap) {
		this(multimap.keySet().size(), (multimap instanceof ArrayListMultimap)
				? ((ArrayListMultimap<?, ?>) multimap).expectedValuesPerKey : DEFAULT_VALUES_PER_KEY);
		putAll(multimap);
	}
	...
	/**
	 * Constructs an empty {@code ArrayListMultimap} with enough capacity to hold the
	 * specified numbers of keys and values without resizing.
	 *
	 * <p>
	 * This method will soon be deprecated in favor of {@code
	 ...
	 */
	public static <K extends Object, V extends Object> ArrayListMultimap<K, V> create(int expectedKeys,
			int expectedValuesPerKey) {
		return new ArrayListMultimap<>(expectedKeys, expectedValuesPerKey);
	}

	/**
	 * Constructs an {@code ArrayListMultimap} with the same mappings as the specified
	 * multimap.
	 *
	 * <p>
	 * This method will soon be deprecated in favor of {@code
	...
	/**
	 * Creates a new, empty {@code ArrayList} to hold the collection of values for an
	 * arbitrary key.
	 */
	@Override
	 ...
	 * @deprecated For a {@link ListMultimap} that automatically trims to size, use
	 * {@link ImmutableListMultimap}. If you need a mutable collection, remove the
	 * {@code trimToSize} call, or switch to a {@code HashMap<K, ArrayList<V>>}.
	 */
	@Deprecated
	...
	private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
		...
		int distinctKeys = org.magneton.core.collect.Serialization.readCount(stream);
		Map<K, Collection<V>> map = Maps.newHashMap();
		setMap(map);
		org.magneton.core.collect.Serialization.populateMultimap(this, stream, distinctKeys);
	}

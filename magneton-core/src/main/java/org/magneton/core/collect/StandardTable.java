...
/**
 * {@link Table} implementation backed by a map that associates row keys with column key /
 * value secondary maps. This class provides rapid access to records by the row key alone
 * or by both keys, but not by just the column key.
 *
 * <p>
 * The views returned by {@link #column}, {@link #columnKeySet()}, and
 * {@link #columnMap()} have iterators that don't support {@code remove()}. Otherwise, all
 * optional operations are supported. Null row keys, columns keys, and values are not
 * supported.
 *
 * <p>
 * Lookups by row key are often faster than lookups by column key, because the data is
 * stored in a {@code Map<R, Map<C, V>>}. A method call like
 * {@code column(columnKey).get(rowKey)} still runs quickly, since the row key is
 * provided. However, {@code column(columnKey).size()} takes longer, since an iteration
 * across all row keys occurs.
 *
 * <p>
 * Note that this implementation is not synchronized. If multiple threads access this
 * table concurrently and one of the threads modifies the table, it must be synchronized
 * externally.
 *
 * @author Jared Levy
 */
...
class StandardTable<R, C, V> extends AbstractTable<R, C, V> implements Serializable {
	...
	@org.magneton.core.collect.GwtTransient
	final org.magneton.core.base.Supplier<? extends Map<C, V>> factory;
	...
	private boolean containsMapping(@CheckForNull Object rowKey, @CheckForNull Object columnKey,
			@CheckForNull Object value) {
		return value != null && value.equals(get(rowKey, columnKey));
	}

	/** Remove a row key / column key / value mapping, if present. */
	private boolean removeMapping(@CheckForNull Object rowKey, @CheckForNull Object columnKey,
			@CheckForNull Object value) {
	 ...
	 *
	 * <p>
	 * The set's iterator traverses the mappings for the first row, the mappings for the
	 * second row, and so on.
	 *
	 * <p>
	 * Each cell is an immutable snapshot of a row key / column key / value mapping, taken
	 * at the time the cell is returned by a method call to the set or its iterator.
	 */
	@Override
	...
	Spliterator<Cell<R, C, V>> cellSpliterator() {
		return CollectSpliterators
				.flatMap(backingMap.entrySet().spliterator(),
						(Entry<R, Map<C, V>> rowEntry) -> CollectSpliterators.map(
								rowEntry.getValue().entrySet().spliterator(),
								(Entry<C, V> columnEntry) -> Tables.immutableCell(rowEntry.getKey(),
	 ...
	 *
	 * <p>
	 * The collection's iterator traverses the values for the first row, the values for
	...
	private class CellIterator implements Iterator<Cell<R, C, V>> {
		...
		Iterator<Entry<C, V>> columnIterator = org.magneton.core.collect.Iterators.emptyModifiableIterator();
		...
		public Cell<R, C, V> next() {
			 ...
			 *
			 * - columnIterator started off pointing to an empty iterator, so we must have
			 * entered the `if` body above at least once. Thus, if we got this far, that
			 * `if` body initialized rowEntry at least once.
			 *
			 * - The only case in which rowEntry is cleared (during remove() below)
			 * happens only if the caller removed every element from columnIterator.
			 * During that process, we would have had to iterate it to exhaustion. Then we
			 * can apply the logic above about an empty columnIterator. (This assumes no
			 * concurrent modification, but behavior under concurrent modification is
			 * undefined, anyway.)
			 */
		...
		public void remove() {
			 ...
			 * requireNonNull is safe because:
			 *
			 * - columnIterator.remove() succeeded, so it must have returned a value, so
			 * it must have been initialized by next() -- which initializes rowEntry, too.
			 *
			 * - rowEntry isn't cleared except below. If it was cleared below, then either
			 * columnIterator.remove() would have failed above (if the user hasn't called
			 * next() since then) or rowEntry would have been initialized by next() (as
			 * discussed above).
			 */
	...
	class Row extends Maps.IteratorBasedAbstractMap<C, V> {
		...
		Iterator<Entry<C, V>> entryIterator() {
			updateBackingRowMapField();
			if (backingRowMap == null) {
				return org.magneton.core.collect.Iterators.emptyModifiableIterator();
			}
	...
	private class Column extends Maps.ViewCachingAbstractMap<R, V> {
					...
					if (entry.getValue().containsKey(columnKey)) {
						...
						class EntryImpl extends AbstractMapEntry<R, V> {
							...
							public V setValue(V value) {
								 ...
								 * above. (Well, it's possible for the map to change
								 * between that call and this one. But if that happens,
								 * the behavior is undefined because of the concurrent
								 * mutation.)
								 *
								 * (Our prototype checker happens to be "smart" enough to
								 * understand this for the *get* call in getValue but not
								 * for the *put* call here.)
								 *
								 * (Arguably we should use requireNonNull rather than
								 * uncheckedCastNullableTToT: We know that V is a non-null
								 * type because that's the only kind of value type that
								 * StandardTable supports. Thus, requireNonNull is safe as
								 * long as the cell is still present. (And if it's not
								 * present, behavior is undefined.) However, that's a
								 * behavior change relative to the old code, so it didn't
								 * seem worth risking.)
								 */
								return NullnessCasts.uncheckedCastNullableTToT(
										entry.getValue().put(columnKey, Preconditions.checkNotNull(value)));
							}
		...
		private class KeySet extends Maps.KeySet<R, V> {
			...
			@Override
			public boolean retainAll(Collection<?> c) {
				return removeFromColumnIf(Maps.<R>keyPredicateOnEntries(Predicates.not(Predicates.in(c))));
			}
		...
		private class Values extends Maps.Values<R, V> {
			...
			@Override
			public boolean remove(@CheckForNull Object obj) {
				return obj != null && removeFromColumnIf(Maps.<V>valuePredicateOnEntries(Predicates.equalTo(obj)));
			}

			@Override
			public boolean removeAll(Collection<?> c) {
				return removeFromColumnIf(Maps.<V>valuePredicateOnEntries(Predicates.in(c)));
			}

			@Override
			public boolean retainAll(Collection<?> c) {
				return removeFromColumnIf(Maps.<V>valuePredicateOnEntries(Predicates.not(Predicates.in(c))));
			}
	...
	private class ColumnKeySet extends TableSet<C> {
		...
		public boolean removeAll(Collection<?> c) {
			...
			while (iterator.hasNext()) {
				...
				if (org.magneton.core.collect.Iterators.removeAll(map.keySet().iterator(), c)) {
					changed = true;
					if (map.isEmpty()) {
						iterator.remove();
					}
		...
		class EntrySet extends TableSet<Entry<R, Map<C, V>>> {
			...
			public Iterator<Entry<R, Map<C, V>>> iterator() {
				return Maps.asMapEntryIterator(backingMap.keySet(),
						new org.magneton.core.base.Function<R, Map<C, V>>() {
							@Override
	...
	private class ColumnMap extends Maps.ViewCachingAbstractMap<C, Map<R, V>> {
		...
		class ColumnMapEntrySet extends TableSet<Entry<C, Map<R, V>>> {
			...
			public boolean remove(@CheckForNull Object obj) {
				/*
				 * `o instanceof Entry` is guaranteed by `contains`, but we check it here
				 * to satisfy our nullness checker.
				 */
		...
		private class ColumnMapValues extends Maps.Values<C, Map<R, V>> {
			...
			@Override
			public boolean remove(@CheckForNull Object obj) {
				for (Entry<C, Map<R, V>> entry : entrySet()) {

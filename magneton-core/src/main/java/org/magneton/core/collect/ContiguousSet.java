 ...
 *
 * <p>
 * Note that because bounded ranges over {@code int} and {@code long} values are so
 * common, this particular example can be written as just:
 *
 * <pre>{@code
 ...
 *
 * <p>
 * <b>Warning:</b> Be extremely careful what you do with conceptually large instances
 * (such as {@code ContiguousSet.create(Range.greaterThan(0), DiscreteDomain.integers()}).
...
public abstract class ContiguousSet<C extends Comparable> extends ImmutableSortedSet<C> {
	 ...
	 * @throws IllegalArgumentException if neither range nor the domain has a lower bound,
	 * or if neither has an upper bound
	 * @since 13.0
	 */
	public static <C extends Comparable> ContiguousSet<C> create(Range<C> range, DiscreteDomain<C> domain) {
		...
		if (effectiveRange.isEmpty()) {
			...
			/*
			 * requireNonNull is safe because the effectiveRange operations above would
			 * have thrown or effectiveRange.isEmpty() would have returned true.
			 */
	...
	/**
	 * Returns a nonempty contiguous set containing all {@code int} values from
	 * {@code lower} (inclusive) to {@code upper} (inclusive). (These are the same values
	 * contained in {@code
	...
	/**
	 * Returns a nonempty contiguous set containing all {@code long} values from
	 * {@code lower} (inclusive) to {@code upper} (inclusive). (These are the same values
	 * contained in {@code
	 * Range.closed(lower, upper)}.)
	 * @throws IllegalArgumentException if {@code lower} is greater than {@code upper}
	...
	/**
	 * Returns a contiguous set containing all {@code int} values from {@code lower}
	 * (inclusive) to {@code upper} (exclusive). If the endpoints are equal, an empty set
	 * is returned. (These are the same values contained in
	 * {@code Range.closedOpen(lower, upper)}.)
	 * @throws IllegalArgumentException if {@code lower} is greater than {@code upper}
	 * @since 23.0
	 */
	...
	/**
	 * Returns a contiguous set containing all {@code long} values from {@code lower}
	 * (inclusive) to {@code upper} (exclusive). If the endpoints are equal, an empty set
	 * is returned. (These are the same values contained in
	 * {@code Range.closedOpen(lower, upper)}.)
	 * @throws IllegalArgumentException if {@code lower} is greater than {@code upper}
	 ...
	 * {@link #create}. This method exists only to hide {@link ImmutableSet#builder} from
	 * consumers of {@code
	 * ContiguousSet}.
	...
	public ContiguousSet<C> headSet(C toElement) {
		return headSetImpl(Preconditions.checkNotNull(toElement), false);
	}
	...
	public ContiguousSet<C> tailSet(C fromElement) {
		return tailSetImpl(Preconditions.checkNotNull(fromElement), true);
	}
	 ...
	 * validation.
	 */
	// Supermethod does not exist under GWT.
	...
	/**
	 * Returns a range, closed on both ends, whose endpoints are the minimum and maximum
	 * values contained in this set. This is equivalent to {@code range(CLOSED, CLOSED)}.
	 * @throws NoSuchElementException if this set is empty
	 */
	public abstract Range<C> range();

	/**
	 * Returns the minimal range with the given boundary types for which all values in
	 * this set are {@linkplain Range#contains(Comparable) contained} within the range.
	 *
	 * <p>
	 * Note that this method will return ranges with unbounded endpoints if
	 * {@link org.magneton.core.collect.BoundType#OPEN} is requested for a domain minimum
	 * or maximum. For example, if {@code set} was created from the range
	 * {@code [1..Integer.MAX_VALUE]} then {@code set.range(CLOSED, OPEN)} must return
	 * {@code
	 * [1..∞)}.
	 * @throws NoSuchElementException if this set is empty
	 */
	public abstract Range<C> range(org.magneton.core.collect.BoundType lowerBoundType, BoundType upperBoundType);

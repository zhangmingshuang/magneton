 ...
 * operations: {@link #next}, {@link #previous} and {@link #distance}, according to their
 * specifications. The methods {@link #minValue} and {@link #maxValue} should also be
 * overridden for bounded types.
 *
 * <p>
 * A discrete domain always represents the <i>entire</i> set of values of its type; it
 * cannot represent partial domains such as "prime integers" or "strings of length 5."
 *
 * <p>
...
public abstract class DiscreteDomain<C extends Comparable> {
	...
	/**
	 * Returns a signed value indicating how many nested invocations of {@link #next} (if
	 * positive) or {@link #previous} (if negative) are needed to reach {@code end}
	 * starting from {@code start}. For example, if {@code end = next(next(next(start)))},
	 * then {@code distance(start, end) == 3} and {@code distance(end, start) == -3}. As
	 * well, {@code distance(a, a)} is always zero.
	 *
	 * <p>
	 ...
	 * {@link Long#MAX_VALUE} if the distance is too small or too large, respectively.
	 */
	public abstract long distance(C start, C end);

	/**
	 * Returns the minimum value of type {@code C}, if it has one. The minimum value is
	 * the unique value for which {@link Comparable#compareTo(Object)} never returns a
	 * positive value for any input of type {@code C}.
	 *
	 * <p>
	 * The default implementation throws {@code NoSuchElementException}.
	 * @return the minimum value of type {@code C}; never null
	 * @throws NoSuchElementException if the type has no (practical) minimum value; for
	 * example, {@link BigInteger}
	 */
	...
	/**
	 * Returns the maximum value of type {@code C}, if it has one. The maximum value is
	 * the unique value for which {@link Comparable#compareTo(Object)} never returns a
	 * negative value for any input of type {@code C}.
	 *
	 * <p>
	 * The default implementation throws {@code NoSuchElementException}.
	 * @return the maximum value of type {@code C}; never null
	 * @throws NoSuchElementException if the type has no (practical) maximum value; for
	 * example, {@link BigInteger}
	 */
	@CanIgnoreReturnValue
	public C maxValue() {

...
final class SparseImmutableTable<R, C, V> extends RegularImmutableTable<R, C, V> {
	...
	static final ImmutableTable<Object, Object, Object> EMPTY = new SparseImmutableTable<>(
			org.magneton.core.collect.ImmutableList.<Table.Cell<Object, Object, Object>>of(),
			org.magneton.core.collect.ImmutableSet.of(), org.magneton.core.collect.ImmutableSet.of());
	...
	// For each cell in iteration order, the index of that cell's column key in the list
	// of column
	// keys present in that row.
	// We don't modify this after construction.
	private final int[] cellColumnInRowIndices;

	SparseImmutableTable(ImmutableList<Cell<R, C, V>> cellList, org.magneton.core.collect.ImmutableSet<R> rowSpace,
		...
		for (int i = 0; i < cellList.size(); i++) {
			...
			/*
			 * These requireNonNull calls are safe because we construct the maps to hold
		...
		org.magneton.core.collect.ImmutableMap.Builder<R, org.magneton.core.collect.ImmutableMap<C, V>> rowBuilder = new org.magneton.core.collect.ImmutableMap.Builder<>(
				rows.size());
		for (Entry<R, Map<C, V>> row : rows.entrySet()) {
			rowBuilder.put(row.getKey(), org.magneton.core.collect.ImmutableMap.copyOf(row.getValue()));
		}
		rowMap = rowBuilder.build();

		org.magneton.core.collect.ImmutableMap.Builder<C, org.magneton.core.collect.ImmutableMap<R, V>> columnBuilder = new org.magneton.core.collect.ImmutableMap.Builder<>(
				columns.size());
		for (Entry<C, Map<R, V>> col : columns.entrySet()) {
			columnBuilder.put(col.getKey(), org.magneton.core.collect.ImmutableMap.copyOf(col.getValue()));
		}
	...
	public org.magneton.core.collect.ImmutableMap<R, Map<C, V>> rowMap() {
		...
		return org.magneton.core.collect.ImmutableMap.<R, Map<C, V>>copyOf(rowMap);
	}
	...
	Table.Cell<R, C, V> getCell(int index) {
		...
		Entry<R, org.magneton.core.collect.ImmutableMap<C, V>> rowEntry = rowMap.entrySet().asList().get(rowIndex);
		org.magneton.core.collect.ImmutableMap<C, V> row = rowEntry.getValue();

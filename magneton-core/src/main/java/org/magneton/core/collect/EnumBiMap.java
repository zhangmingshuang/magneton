...
/**
 * A {@code BiMap} backed by two {@code EnumMap} instances. Null keys and values are not
 * permitted. An {@code EnumBiMap} and its inverse are both serializable.
 *
 * <p>
...
public final class EnumBiMap<K extends Enum<K>, V extends Enum<V>> extends AbstractBiMap<K, V> {
	...
	/**
	 * Returns a new bimap with the same mappings as the specified map. If the specified
	 * map is an {@code EnumBiMap}, the new bimap has the same types as the provided map.
	 * Otherwise, the specified map must contain at least one mapping, in order to
	 * determine the key and value types.
	 * @param map the map whose mappings are to be placed in this map
	 * @throws IllegalArgumentException if map is not an {@code EnumBiMap} instance and
	...
	/**
	 * @serialData the key class, value class, number of entries, first key, first value,

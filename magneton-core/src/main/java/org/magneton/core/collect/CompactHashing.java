...
final class CompactHashing {
	...
	static final int MAX_SIZE = org.magneton.core.primitives.Ints.MAX_POWER_OF_TWO - 1;
	...
	/**
	 * Minimum size of the hash table of a compact hash-based collection. Because small
	 * hash tables use a byte[], any smaller size uses the same amount of memory due to
	 * object padding.
	 */
	...
	/**
	 * Returns the power of 2 hashtable size required to hold the expected number of items
	 * or the minimum hashtable size, whichever is greater.
	 */
	static int tableSize(int expectedSize) {
		// We use entries next == 0 to indicate UNSET, so actual capacity is 1 less than
		// requested.
	...
	/**
	 * Returns {@code table[index]}, where {@code table} is actually a {@code byte[]},
	 * {@code
	 * short[]}, or {@code int[]}. When it is a {@code byte[]} or {@code short[]}, the
	 * returned value is unsigned, so the range of possible returned values is 0–255 or
	...
	/**
	 * Sets {@code table[index]} to {@code entry}, where {@code table} is actually a
	 * {@code byte[]}, {@code short[]}, or {@code int[]}. The value of {@code entry}
	 * should fit in the size of the assigned array element, when seen as an unsigned
	 * value. So if {@code table} is a {@code byte[]} then we should have
	 * {@code 0 ≤ entry ≤ 255}, and if {@code table} is a {@code short[]} then we should
	 * have {@code 0 ≤ entry ≤ 65535}. It is the caller's responsibility to ensure this.
	 */
	 ...
	 *
	 * <p>
	 * For hashtable sizes less than or equal to 32, the returned power of 2 is 4x the
	 * current hashtable size to reduce expensive rehashing. Otherwise the returned power
	 * of 2 is 2x the current hashtable size.
	 */
	...
	static int remove(@CheckForNull Object key, @CheckForNull Object value, int mask, Object table, int[] entries,
			@Nullable Object[] keys, @CheckForNull @Nullable Object[] values) {
		...
		do {
			...
			if (getHashPrefix(entry, mask) == hashPrefix && org.magneton.core.base.Objects.equal(key, keys[entryIndex])

...
public final class Escapers {
	...
	/**
	 * Returns a builder for creating simple, fast escapers. A builder instance can be
	 * reused and each escaper that is created will be a snapshot of the current builder
	 * state. Builders are not thread safe.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * For performance reasons escapers created by this builder are not Unicode aware and
	 * will not validate the well-formedness of their input.
	 */
	...
	/**
	 * Returns a {@link UnicodeEscaper} equivalent to the given escaper instance. If the
	 * escaper is already a UnicodeEscaper then it is simply returned, otherwise it is
	 * wrapped in a UnicodeEscaper.
	 *
	 * <p>
	 * When a {@link CharEscaper} escaper is wrapped by this method it acquires extra
	 * behavior with respect to the well-formedness of Unicode character sequences and
	 * will throw {@link IllegalArgumentException} when given bad input.
	...
	/**
	 * Returns a string that would replace the given character in the specified escaper,
	 * or {@code
	 * null} if no replacement should be made. This method is intended for use in tests
	 * through the {@code EscaperAsserts} class; production users of {@link CharEscaper}
	 * should limit themselves to its public interface.
	 ...
	 * or {@code
	 * null} if no replacement should be made. This method is intended for use in tests
	 * through the {@code EscaperAsserts} class; production users of
	 ...
	 *
	 * <p>
	 * Typically an escaper needs to deal with the escaping of high valued characters or
	 * code points. In these cases it is necessary to extend either
	 * {@link org.magneton.core.escape.ArrayBasedCharEscaper} or
	 * {@link ArrayBasedUnicodeEscaper} to provide the desired behavior. However this
	 * builder is suitable for creating escapers that replace a relative small set of
	 * characters.
	 *
	 * @author David Beaumont
	 * @since 15.0
	...
	public static final class Builder {
		 ...
		 * that have no explicit replacement are considered 'safe' and remain unescaped in
		 * the output. If {@code
		 ...
		 * have no explicit replacement. If {@code unsafeReplacement} is {@code null} then
		 * no replacement will occur, if it is {@code ""} then the unsafe characters are
		 ...
		 * character will be replaced by the given string whenever it occurs in the input,

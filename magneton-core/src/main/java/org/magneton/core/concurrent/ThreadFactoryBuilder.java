 ...
 *
 * <p>
 * If no backing thread factory is provided, a default backing thread factory is used as
 * if by calling
...
public final class ThreadFactoryBuilder {
	...
	// Split out so that the anonymous ThreadFactory can't contain a reference back to the
	// builder.
	...
	private static ThreadFactory doBuild(ThreadFactoryBuilder builder) {
		...
		return new ThreadFactory() {
			...
			public Thread newThread(Runnable runnable) {
				...
				if (nameFormat != null) {
					// requireNonNull is safe because we create `count` if (and only if)
					// we have a nameFormat.
	...
	/**
	 * Sets the naming format to use when naming threads ({@link Thread#setName}) which
	 * are created with this ThreadFactory.
	 * @param nameFormat a {@link String#format(String, Object...)}-compatible format
	 * String, to which a unique integer (0, 1, etc.) will be supplied as the single
	 * parameter. This integer will be unique to the built instance of the ThreadFactory
	 * and will be assigned sequentially. For example, {@code "rpc-pool-%d"} will generate
	 * thread names like {@code "rpc-pool-0"}, {@code
	 ...
	 * @param daemon whether or not new Threads created with this ThreadFactory will be
	 * daemon threads
	 * @return this for the builder pattern
	 */
	 ...
	 * ThreadFactory.
	 * @param uncaughtExceptionHandler the uncaught exception handler for new Threads
	 * created with this ThreadFactory
	 * @return this for the builder pattern
	 */
	 ...
	 * ThreadFactory. Threads will be created by invoking #newThread(Runnable) on this
	 * backing {@link ThreadFactory}.
	 * @param backingThreadFactory the backing {@link ThreadFactory} which will be
	 * delegated to during thread creation.
	 * @return this for the builder pattern
	 * @see MoreExecutors
	 */
	 ...
	 * process. After building, it is still possible to change the options used to build
	 * the ThreadFactory and/or build again. State is not shared amongst built instances.

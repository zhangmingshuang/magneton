...
/**
 * A {@link ScheduledExecutorService} that returns {@link ListenableFuture} instances from
 * its {@code ExecutorService} methods. To create an instance from an existing
...
public interface ListeningScheduledExecutorService extends ScheduledExecutorService, ListeningExecutorService {
	...
	default ListenableScheduledFuture<?> schedule(Runnable command, Duration delay) {
		return this.schedule(command, toNanosSaturated(delay), TimeUnit.NANOSECONDS);
	}

	/** @since 15.0 (previously returned ScheduledFuture) */
	@Override
	<V extends Object> ListenableScheduledFuture<V> schedule(Callable<V> callable, long delay, TimeUnit unit);
	 ...
	 * @since 29.0
	 */
	default <V extends Object> ListenableScheduledFuture<V> schedule(Callable<V> callable, Duration delay) {
		return this.schedule(callable, toNanosSaturated(delay), TimeUnit.NANOSECONDS);
	}
	 ...
	 * {@link #scheduleAtFixedRate(Runnable, long, long, TimeUnit)}.
	 *
	 * @since 29.0
	 */
	default ListenableScheduledFuture<?> scheduleAtFixedRate(Runnable command, Duration initialDelay, Duration period) {
		return this.scheduleAtFixedRate(command, toNanosSaturated(initialDelay), toNanosSaturated(period),
	 ...
	 * @since 29.0
	 */
	default ListenableScheduledFuture<?> scheduleWithFixedDelay(Runnable command, Duration initialDelay,
			Duration delay) {
		return this.scheduleWithFixedDelay(command, toNanosSaturated(initialDelay), toNanosSaturated(delay),
				TimeUnit.NANOSECONDS);
	}

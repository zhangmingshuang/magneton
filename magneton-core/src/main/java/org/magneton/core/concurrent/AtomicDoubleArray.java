...
import org.magneton.core.primitives.ImmutableLongArray;
 ...
 *
 * <p>
 * <a id="bitEquals"></a>This class compares primitive {@code double} values in methods
 * such as {@link #compareAndSet} by comparing their bitwise representation using
 * {@link Double#doubleToRawLongBits}, which differs from both the primitive double
...
public class AtomicDoubleArray implements java.io.Serializable {
	...
	/**
	 * Creates a new {@code AtomicDoubleArray} of the given length, with all elements
	 * initially zero.
	...
	/**
	 * Creates a new {@code AtomicDoubleArray} with the same length as, and all elements
	...
	/**
	 * Atomically sets the element at position {@code i} to the given value and returns
	 * the old value.
	 * @param i the index
	 * @param newValue the new value
	...
	/**
	 * Atomically sets the element at position {@code i} to the given updated value if the
	 * current value is <a href="#bitEquals">bitwise equal</a> to the expected value.
	 * @param i the index
	...
	public final boolean compareAndSet(int i, double expect, double update) {
		return this.longs.compareAndSet(i, doubleToRawLongBits(expect), doubleToRawLongBits(update));
	}
	 ...
	 * fail spuriously</a> and does not provide ordering guarantees, so is only rarely an
	...
	public final boolean weakCompareAndSet(int i, double expect, double update) {
		return this.longs.weakCompareAndSet(i, doubleToRawLongBits(expect), doubleToRawLongBits(update));
	}
	...
	/**
	 * Atomically updates the element at index {@code i} with the results of applying the
	 * given function to the curernt and given values.
	...
	/**
	 * Atomically updates the element at index {@code i} with the results of applying the
	 * given function to the curernt value.
	 * @param i the index to update
	 ...
	 * elements (each a {@code double}) in the proper order.
	 */

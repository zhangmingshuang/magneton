...
import org.magneton.core.collect.MapMaker;
 ...
 * {@link ReentrantReadWriteLock} instances that detect potential deadlock by checking for
 * cycles in lock acquisition order.
 *
 * <p>
 * Potential deadlocks detected when calling the {@code lock()},
 * {@code lockInterruptibly()}, or {@code tryLock()} methods will result in the execution
 * of the {@link Policy} specified when creating the factory. The currently available
 * policies are:
 *
 * <ul>
 * <li>DISABLED
 ...
 *
 * <p>
 * The locks created by a factory instance will detect lock acquisition cycles with locks
 * created by other {@code CycleDetectingLockFactory} instances (except those with
 * {@code Policy.DISABLED}). A lock's behavior when a cycle is detected, however, is
 * defined by the {@code Policy} of the factory that created it. This allows detection of
 * cycles across components while delegating control over lock behavior to individual
 * components.
 *
 * <p>
 * Applications are encouraged to use a {@code CycleDetectingLockFactory} to create any
 * locks for which external/unmanaged code is executed while the lock is held. (See
 * caveats under <strong>Performance</strong>).
 *
 * <p>
 ...
 * example involving two locks and two threads, deadlock occurs when one thread acquires
 * Lock A, and then Lock B, while another thread acquires Lock B, and then Lock A:
 *
 * <pre>
 ...
 *
 * <ul>
 * <li>Each lock adds (and removes) itself to/from a ThreadLocal Set of acquired locks
 * when the Thread acquires its first hold (and releases its last remaining hold).
 * <li>Before the lock is acquired, the lock is checked against the current set of
 * acquired locks---to each of the acquired locks, an edge from the soon-to-be-acquired
 * lock is either verified or created.
 * <li>If a new edge needs to be created, the outgoing edges of the acquired locks are
 * traversed to check for a cycle that reaches the lock to be acquired. If no cycle is
 * detected, a new "safe" edge is created.
 * <li>If a cycle is detected, an "unsafe" (cyclic) edge is created to represent a
 * potential deadlock situation, and the appropriate Policy is executed.
 * </ul>
 *
 * <p>
 * Note that detection of potential deadlock does not necessarily indicate that deadlock
 * will happen, as it is possible that higher level application logic prevents the cyclic
 * lock acquisition from occurring. One example of a false positive is:
 *
 * <pre>
 * LockA -&gt; LockB -&gt; LockC
 * LockA -&gt; LockC -&gt; LockB
 ...
 * equivalent to traditional exclusive locks. Although this increases the false positives
 * that the locks detect (i.e. cycles that will not actually result in deadlock), it
 * simplifies the algorithm and implementation considerably. The assumption is that a user
 * of this factory wishes to eliminate any cyclic acquisition ordering.
 *
 * <p>
 * <strong>Explicit Lock Acquisition Ordering</strong>
 *
 * <p>
 * The {@link WithExplicitOrdering} class can be used to enforce an application-specific
 * ordering in addition to performing general cycle detection.
 *
 * <p>
 * <strong>Garbage Collection</strong>
 *
 * <p>
 * In order to allow proper garbage collection of unused locks, the edges of the lock
 * graph are weak references.
 *
 * <p>
 ...
 *
 * <ul>
 * <li>for an unnested {@code lock()} and {@code unlock()}, a cycle detecting lock takes
 * 38ns as opposed to the 24ns taken by a plain lock.
...
public class CycleDetectingLockFactory {
	...
	private static final ConcurrentMap<Class<? extends Enum<?>>, Map<? extends Enum<?>, LockGraphNode>> lockGraphNodesPerType = new org.magneton.core.collect.MapMaker()
			.weakKeys().makeMap();
	 ...
	 * {@link #aboutToAcquire(CycleDetectingLock)} and
	...
	private static final ThreadLocal<ArrayList<LockGraphNode>> acquiredLocks = new ThreadLocal<ArrayList<LockGraphNode>>() {
		...
		protected ArrayList<LockGraphNode> initialValue() {
			return org.magneton.core.collect.Lists.<LockGraphNode>newArrayListWithCapacity(3);
		}
	};
	 ...
	 * disallowedPriorLocks} prepopulated with nodes according to the natural ordering of
	 * the associated Enum values.
	 */
	@VisibleForTesting
	 ...
	 * {@code "EnumClass.name"}, which is used in exception and warning output.
	 */
	...
	/**
	 * CycleDetectingLock implementations must call this method in a {@code finally}
	 * clause after any attempt to change the lock state, including both lock and unlock
	 * attempts. Failure to do so can result in corrupting the acquireLocks set.
	 */
	 ...
	 * {@code lockName} is used in the warning or exception output to help identify the
	 * locks involved in the detected deadlock.
	 */
	public ReentrantLock newReentrantLock(String lockName, boolean fair) {
		return this.policy == Policies.DISABLED ? new ReentrantLock(fair)
	...
	/**
	 * Creates a {@link ReentrantReadWriteLock} with the given fairness policy. The
	 * {@code lockName} is used in the warning or exception output to help identify the
	 * locks involved in the detected deadlock.
	 */
	public ReentrantReadWriteLock newReentrantReadWriteLock(String lockName, boolean fair) {
		return this.policy == Policies.DISABLED ? new ReentrantReadWriteLock(fair)
				: new CycleDetectingReentrantReadWriteLock(new LockGraphNode(lockName), fair);
	}

	/**
	 * CycleDetectingLock implementations must call this method before attempting to
	 * acquire the lock.
	 */
	...
	public enum Policies implements Policy {
		 ...
		 * {@code
		 * PotentialDeadlockException} indicating the potential deadlock, which includes
		 * stack traces illustrating the cycle in lock acquisition order.
		 */
		THROW {
			@Override
		 ...
		 * {@link Level#SEVERE} message indicating the potential deadlock, which includes
		 * stack traces illustrating the cycle in lock acquisition order.
		 */
		WARN {
			@Override
		...
		/**
		 * Disables cycle detection. This option causes the factory to return unmodified
		 * lock implementations provided by the JDK, and is provided to allow applications
		 ...
		 * participate the cycle detection performed by locks created by other factories.
		 */
		DISABLED {
			@Override
			public void handlePotentialDeadlock(PotentialDeadlockException e) {
	 ...
	 * Clients can use one of the predefined {@link Policies} or specify a custom
	 * implementation. Implementations must be thread-safe.
	 *
	 * @since 13.0
	 */
	...
	public interface Policy {
		...
		/**
		 * Called when a potential deadlock is encountered. Implementations can throw the
		 * given {@code
		 ...
		 * assumption that it is the application's wish to prohibit any cyclical lock
		 * acquisitions.
		 */
	...
	/**
	 * Internal Lock implementations implement the {@code CycleDetectingLock} interface,
	 * allowing the detection logic to treat all locks in the same manner.
	 */
	...
	/**
	 * A {@code CycleDetectingLockFactory.WithExplicitOrdering} provides the additional
	 * enforcement of an application-specified ordering of lock acquisitions. The
	 * application defines the allowed ordering with an {@code Enum} whose values each
	 * correspond to a lock type. The order in which the values are declared dictates the
	 * allowed order of lock acquisition. In other words, locks corresponding to smaller
	 * values of {@link Enum#ordinal()} should only be acquired before locks with larger
	 * ordinals. Example:
	 *
	 * <pre>{@code
	 * enum MyLockOrder {
	 ...
	 * explicitly ordered locks participate in general cycle detection with all other
	 * cycle detecting locks, and a lock's behavior when detecting a cyclic lock
	 * acquisition is defined by the {@code Policy} of the factory that created it.
	 *
	 * <p>
	 * Note, however, that although multiple locks can be created for a given Enum value,
	 * whether it be through separate factory instances or through multiple calls to the
	 * same factory, attempting to acquire multiple locks with the same Enum value (within
	 * the same thread) will result in an IllegalStateException regardless of the
	 * factory's policy. For example:
	 *
	 * <pre>{@code
	 ...
	 *
	 * <p>
	 * It is the responsibility of the application to ensure that multiple lock instances
	 * with the same rank are never acquired in the same thread.
	 *
	...
	public static final class WithExplicitOrdering<E extends Enum<E>> extends CycleDetectingLockFactory {
		...
		/**
		 * Creates a {@link ReentrantLock} with the given fairness policy and rank. The
		 * values returned by {@link Enum#getDeclaringClass()} and {@link Enum#name()} are
		 * used to describe the lock in warning or exception output.
		 * @throws IllegalStateException If the factory has already created a {@code Lock}
		 * with the specified rank.
		 */
		public ReentrantLock newReentrantLock(E rank, boolean fair) {
			return this.policy == Policies.DISABLED ? new ReentrantLock(fair)
					// requireNonNull is safe because createNodes inserts an entry for
					// every E.
					// (If the caller passes `null` for the `rank` parameter, this will
					// throw, but that's OK.)
					: new CycleDetectingReentrantLock(requireNonNull(this.lockGraphNodes.get(rank)), fair);
		}
		...
		/**
		 * Creates a {@link ReentrantReadWriteLock} with the given fairness policy and
		 * rank. The values returned by {@link Enum#getDeclaringClass()} and
		...
		public ReentrantReadWriteLock newReentrantReadWriteLock(E rank, boolean fair) {
			return this.policy == Policies.DISABLED ? new ReentrantReadWriteLock(fair)
					...
					// (If the caller passes `null` for the `rank` parameter, this will
					// throw, but that's OK.)
					: new CycleDetectingReentrantReadWriteLock(requireNonNull(this.lockGraphNodes.get(rank)), fair);
		}

	}

	/**
	 * A Throwable used to record a stack trace that illustrates an example of a specific
	 * lock acquisition ordering. The top of the stack trace is truncated such that it
	 * starts with the acquisition of the lock in question, e.g.
	 *
	 * <pre>
	...
	private static class ExampleStackTrace extends IllegalStateException {
		...
		static final ImmutableSet<String> EXCLUDED_CLASS_NAMES = ImmutableSet.of(
				CycleDetectingLockFactory.class.getName(), ExampleStackTrace.class.getName(),
	...
	/**
	 * Represents a detected cycle in lock acquisition ordering. The exception includes a
	 * causal chain of {@code ExampleStackTrace} instances to illustrate the cycle, e.g.
	 *
	 * <pre>
	...
	public static final class PotentialDeadlockException extends ExampleStackTrace {
		...
		private PotentialDeadlockException(LockGraphNode node1, LockGraphNode node2,
		...
		/**
		 * Appends the chain of messages from the {@code conflictingStackTrace} to the
		 * original {@code
		 * message}.
		 */
		@Override
	...
	/**
	 * A {@code LockGraphNode} associated with each lock instance keeps track of the
	 * directed edges in the lock acquisition graph.
	 */
	private static class LockGraphNode {

		/**
		 * The map tracking the locks that are known to be acquired before this lock, each
		 * associated with an example stack trace. Locks are weakly keyed to allow proper
		 * garbage collection when they are no longer referenced.
		 */
		final Map<LockGraphNode, ExampleStackTrace> allowedPriorLocks = new org.magneton.core.collect.MapMaker()
				.weakKeys().makeMap();

		/**
		 * The map tracking lock nodes that can cause a lock acquisition cycle if acquired
		 * before this node.
		 */
		...
		/**
		 * Checks the acquisition-ordering between {@code this}, which is about to be
		 * acquired, and the specified {@code acquiredLock}.
		 *
		 * <p>
		 * When this method returns, the {@code acquiredLock} should be in either the
		 * {@code
		 * preAcquireLocks} map, for the case in which it is safe to acquire {@code this}
		 * after the {@code acquiredLock}, or in the {@code disallowedPriorLocks} map, in
		 * which case it is not safe.
		 */
		void checkAcquiredLock(Policy policy, LockGraphNode acquiredLock) {
			...
			Preconditions.checkState(this != acquiredLock, "Attempted to acquire multiple locks with the same rank %s",
			...
			if (previousDeadlockException != null) {
				...
				PotentialDeadlockException exception = new PotentialDeadlockException(acquiredLock, this,
		 ...
		 * {@code
		 * allowedPriorLocks} to find a path between {@code this} and the specified
		 * {@code lock}.
		 * @return If a path was found, a chained {@link ExampleStackTrace} illustrating
		 * the path to the {@code lock}, or {@code null} if no path was found.
		 */
		@CheckForNull

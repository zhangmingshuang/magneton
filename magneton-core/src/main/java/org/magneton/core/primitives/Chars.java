...
/**
 * Static utility methods pertaining to {@code char} primitives, that are not already
 * found in either {@link Character} or {@link Arrays}.
 *
 * <p>
 * All the operations in this class treat {@code char} values strictly numerically; they
 * are neither Unicode-aware nor locale-dependent.
 *
 * <p>
...
public final class Chars {
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Character#BYTES} instead.
	 */
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Character#hashCode(char)} instead.
	 ...
	 * {@link Character#MAX_VALUE} or less than {@link Character#MIN_VALUE}
	 */
	...
	/**
	 * Compares the two specified {@code char} values. The sign of the value returned is
	 * the same as that of {@code ((Character) a).compareTo(b)}.
	 *
	 * <p>
	 * <b>Note for Java 7 and later:</b> this method should be treated as deprecated; use
	 * the equivalent {@link Character#compare} method instead.
	 ...
	 * example, {@code concat(new char[] {a, b}, new char[] {}, new char[] {c}} returns
	 ...
	 * equivalent to {@code ByteBuffer.allocate(2).putChar(value).array()}. For example,
	...
	/**
	 * Returns the {@code char} value whose big-endian representation is stored in the
	 * first 2 bytes of {@code bytes}; equivalent to
	 * {@code ByteBuffer.wrap(bytes).getChar()}. For example, the input byte array
	...
	/**
	 * Returns the {@code char} value whose byte representation is the given 2 bytes, in
	 * big-endian order; equivalent to {@code Chars.fromByteArray(new byte[] {b1, b2})}.
	 *
	 * @since 7.0
	 */
	 ...
	 * not advisable for sorting user-visible strings as the ordering may not match the
	 * conventions of the user's locale. That is, it compares, using
	 * {@link #compare(char, char)}), the first pair of values that follow any common
	 * prefix, or when one array is a prefix of the other, treats the shorter array as the
	 * lesser. For example, {@code [] < ['a'] < ['a', 'b'] < ['b']}.
	 *
	 * <p>
	...
	/**
	 * Copies a collection of {@code Character} instances into a new array of primitive
	 * {@code char} values.
	 *
	 * <p>
	 ...
	 * Collections.reverse(Chars.asList(array).subList(fromIndex, toIndex))}, but is

 ...
 * {@link SignedBytes}, and the methods for which signedness is not an issue are in
 * {@link Bytes}.
 *
 * <p>
...
public final class UnsignedBytes {
	...
	/**
	 * Returns the value of the given byte as an integer, when treated as unsigned. That
	 * is, returns {@code value + 256} if {@code value} is negative; {@code value} itself
	 * otherwise.
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Byte#toUnsignedInt(byte)} instead.
	 *
	 * @since 6.0
	 */
	...
	/**
	 * Returns the {@code byte} value that, when treated as unsigned, is nearest in value
	 * to {@code
	 * value}.
	...
	/**
	 * Compares the two specified {@code byte} values, treating them as unsigned values
	 * between 0 and 255 inclusive. For example, {@code (byte) -127} is considered greater
	 * than {@code (byte) 127} because it is seen as having the value of positive
	 * {@code 129}.
	...
	/**
	 * Returns the unsigned {@code byte} value represented by a string with the given
	 * radix.
	 * @param string the string containing the unsigned {@code byte} representation to be
	 * parsed.
	 * @param radix the radix to use while parsing {@code string}
	 * @throws NumberFormatException if the string does not contain a valid unsigned
	 * {@code byte} with the given radix, or if {@code radix} is not between
	 * {@link Character#MIN_RADIX} and {@link Character#MAX_RADIX}.
	 * @throws NullPointerException if {@code string} is null (in contrast to
	 * {@link Byte#parseByte(String)})
	 * @since 13.0
	 ...
	 * {@code separator}. For example, {@code join(":", (byte) 1, (byte) 2, (byte) 255)}
	 ...
	 * That is, it compares, using {@link #compare(byte, byte)}), the first pair of values
	 ...
	 * {@code [] < [0x01] < [0x01, 0x7F] < [0x01, 0x80] < [0x02]}. Values are treated as
	 * unsigned.
	 *
	 * <p>
	 ...
	 * treating its elements as unsigned bytes.
	 *
	 * @since 23.1
	 */
	 ...
	 * {@code toIndex} exclusive in descending order, interpreting them as unsigned 8-bit
	 * integers.
	 *
	 * @since 23.1
	 */
	...
	/**
	 * Provides a lexicographical comparator implementation; either a Java implementation
	 * or a faster implementation based on {@link Unsafe}.
	 *
	 * <p>
	...
	static class LexicographicalComparatorHolder {
		...
		enum UnsafeComparator implements Comparator<byte[]> {
			 ...
			 *
			 * In UnsignedBytesBenchmark, accessing the following objects via static final
			 * fields is the fastest (more than twice as fast as the Java implementation,
			 * vs ~1.5x with non-final static fields, on x86_32) under the Hotspot server
			 * compiler. The reason is obviously that the non-final fields need to be
			 * reloaded inside the loop.
			 *
			 * And, no, defining (final or not) local variables out of the loop still
			 * isn't as good because the null check on the theUnsafe object remains inside
			 * the loop and BYTE_ARRAY_BASE_OFFSET doesn't get constant-folded.
			 *
			...
			private static Unsafe getUnsafe() {
				...
				try {
					return java.security.AccessController
							.doPrivileged(new java.security.PrivilegedExceptionAction<Unsafe>() {
								@Override
			...
			public int compare(byte[] left, byte[] right) {
				 ...
				 * bytes is no slower than 4 bytes even on 32-bit. On the other hand, it
				 * is substantially faster on 64-bit.
				 */
				for (i = 0; i < strideLimit; i += stride) {
					...
					if (lw != rw) {
						 ...
						 * gives us the shift to get that least significant nonzero byte.
						 */

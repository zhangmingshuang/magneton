 ...
 * {@link org.magneton.core.primitives.Longs}, as well as signed versions of methods for
 * which signedness is an issue.
 *
 * <p>
 * In addition, this class provides several static methods for converting a {@code long}
 * to a {@code String} and a {@code String} to a {@code long} that treat the {@code long}
 * as an unsigned number.
 *
 * <p>
 * Users of these utilities must be <i>extremely careful</i> not to mix up signed and
 * unsigned {@code long} values. When possible, it is recommended that the
 * {@link UnsignedLong} wrapper class be used, at a small efficiency penalty, to enforce
 * the distinction in the type system.
 *
 * <p>
 * See the Guava User Guide article on <a href=
 * "https://github.com/google/guava/wiki/PrimitivesExplained#unsigned-support">unsigned
...
public final class UnsignedLongs {
	...
	/**
	 * A (self-inverse) bijection which converts the ordering on unsigned longs to the
	 * ordering on longs, that is, {@code a <= b} as unsigned longs if and only if
	 * {@code flip(a) <= flip(b)} as signed longs.
	 */
	...
	/**
	 * Compares the two specified {@code long} values, treating them as unsigned values
	 * between {@code
	 * 0} and {@code 2^64 - 1} inclusive.
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#compareUnsigned(long, long)} instead.
	 ...
	 * other value in the array according to {@link #compare}
	 ...
	 * {@code
	 * separator}. For example, {@code join("-", 1, 2, 3)} returns the string
	 * {@code "1-2-3"}.
	 ...
	 * the shorter array as the lesser. For example,
	 ...
	 * treating its elements as unsigned 64-bit integers.
	 *
	 * @since 23.1
	 */
	...
	/**
	 * Sorts the elements of {@code array} in descending order, interpreting them as
	 ...
	 * {@code toIndex} exclusive in descending order, interpreting them as unsigned 64-bit
	 * integers.
	 *
	 * @since 23.1
	 */
	...
	/**
	 * Returns dividend / divisor, where the dividend and divisor are treated as unsigned
	 * 64-bit quantities.
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#divideUnsigned(long, long)} instead.
	...
	public static long divide(long dividend, long divisor) {
		 ...
		 * value. This follows from fact that floor(floor(x)/i) == floor(x/i) for any real
		 * x and integer i != 0. The proof is not quite trivial.
		 */
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#remainderUnsigned(long, long)} instead.
	 * @param dividend the dividend (numerator)
	 * @param divisor the divisor (denominator)
	 * @throws ArithmeticException if divisor is 0
	 * @since 11.0
	...
	public static long remainder(long dividend, long divisor) {
		 ...
		 * Otherwise, approximate the quotient, check, and correct if necessary. Our
		 * approximation is guaranteed to be either exact or one less than the correct
		 * value. This follows from the fact that floor(floor(x)/i) == floor(x/i) for any
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#parseUnsignedLong(String)} instead.
	...
	/**
	 * Returns the unsigned {@code long} value represented by a string with the given
	 * radix.
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#parseUnsignedLong(String, int)} instead.
	 * @param string the string containing the unsigned {@code long} representation to be
	 * parsed.
	 * @param radix the radix to use while parsing {@code string}
	 * @throws NumberFormatException if the string does not contain a valid unsigned
	 * {@code long} with the given radix, or if {@code radix} is not between
	 ...
	 *
	 * <p>
	 * Accepts a decimal, hexadecimal, or octal number given by specifying the following
	 * prefix:
	 *
	 * <ul>
	...
	public static long decode(String stringValue) {
		org.magneton.core.primitives.ParseRequest request = org.magneton.core.primitives.ParseRequest
	 ...
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#toUnsignedString(long)} instead.
	 */
	...
	/**
	 * Returns a string representation of {@code x} for the given radix, where {@code x}
	 * is treated as unsigned.
	 *
	 * <p>
	 * <b>Java 8 users:</b> use {@link Long#toUnsignedString(long, int)} instead.
	...
	public static String toString(long x, int radix) {
		Preconditions.checkArgument(radix >= Character.MIN_RADIX && radix <= Character.MAX_RADIX,
	...
	/*
	 * We move the static constants into this class so ProGuard can inline UnsignedLongs
	 * entirely unless the user is actually calling a parse method.
	 */

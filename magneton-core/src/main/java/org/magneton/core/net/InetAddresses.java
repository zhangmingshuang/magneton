 ...
 *
 * <p>
 * <b>Important note:</b> Unlike {@code InetAddress.getByName()}, the methods of this
 * class never cause DNS services to be accessed. For this reason, you should prefer these
 * methods as much as possible over their JDK equivalents whenever you are expecting to
 * handle only IP address string literals -- there is no blocking DNS penalty for a
 * malformed string.
 *
 * <p>
 * When dealing with {@link Inet4Address} and {@link Inet6Address} objects as byte arrays
 * (vis. {@code InetAddress.getAddress()}) they are 4 and 16 bytes in length,
 * respectively, and represent the address in network byte order.
 *
 * <p>
 ...
 * <dt>From the IPv6 reserved documentation prefix ({@code 2001:db8::/32}),
 * {@code "2001:db8::1"}.
 ...
 *
 * <p>
 * "IPv4 mapped" addresses were originally a representation of IPv4 addresses for use on
 * an IPv6 socket that could receive both IPv4 and IPv6 connections (by disabling the
 * {@code IPV6_V6ONLY} socket option on an IPv6 socket). Yes, it's confusing.
 * Nevertheless, these "mapped" addresses were never supposed to be seen on the wire. That
 * assumption was dropped, some say mistakenly, in later RFCs with the apparent aim of
 * making IPv4-to-IPv6 transition simpler.
 *
 * <p>
 * Technically one <i>can</i> create a 128bit IPv6 address with the wire format of a
 * "mapped" address, as shown above, and transmit it in an IPv6 packet header. However,
 * Java's InetAddress creation methods appear to adhere doggedly to the original intent of
 * the "mapped" address: all "mapped" addresses return {@link Inet4Address} objects.
 *
 * <p>
 * For added safety, it is common for IPv6 network operators to filter all packets where
 * either the source or destination address appears to be a "compat" or "mapped" address.
 * Filtering suggestions usually recommend discarding any packets with source or
 * destination addresses in the invalid range {@code ::/3}, which includes both of these
 * bizarre address formats. For more information on "bogons", including lists of IPv6
 * bogon space, see:
 *
 * <ul>
 * <li><a target="_parent" href=
 * "http://en.wikipedia.org/wiki/Bogon_filtering">http://en.wikipedia.
 * org/wiki/Bogon_filtering</a>
 * <li><a target="_parent" href=
 * "http://www.cymru.com/Bogons/ipv6.txt">http://www.cymru.com/Bogons/ ipv6.txt</a>
 * <li><a target="_parent" href=
 * "http://www.cymru.com/Bogons/v6bogon.html">http://www.cymru.com/
 * Bogons/v6bogon.html</a>
 * <li><a target="_parent" href=
 * "http://www.space.net/~gert/RIPE/ipv6-filters.html">http://www.
...
public final class InetAddresses {
	...
	private static final org.magneton.core.base.CharMatcher IPV4_DELIMITER_MATCHER = org.magneton.core.base.CharMatcher
			.is(IPV4_DELIMITER);

	private static final org.magneton.core.base.CharMatcher IPV6_DELIMITER_MATCHER = CharMatcher.is(IPV6_DELIMITER);
	...
	/**
	 * Returns an {@link Inet4Address}, given a byte array representation of the IPv4
	 * address.
	...
	private static Inet4Address getInet4Address(byte[] bytes) {
		Preconditions.checkArgument(bytes.length == 4, "Byte array has invalid length for an IPv4 address: %s != 4.",
				bytes.length);
	 ...
	 *
	 * <p>
	 * Anything after a {@code %} in an IPv6 address is ignored (assumed to be a Scope
	 * ID).
	 *
	 * <p>
	 * This method accepts non-ASCII digits, for example {@code "１９２.１６８.０.１"} (those are
	 * fullwidth characters). That is consistent with {@link InetAddress}, but not with
	 * various RFCs. If you want to accept ASCII digits only, you can use something like
	 * {@code
	...
	private static byte[] textToNumericFormatV6(String ipString) {
		...
		int partsSkipped = IPV6_PART_COUNT - (delimiterCount + 1); // estimate; may be
																	// modified later
		...
		for (int i = 0; i < ipString.length() - 1; i++) {
			if (ipString.charAt(i) == IPV6_DELIMITER && ipString.charAt(i + 1) == IPV6_DELIMITER) {
				...
				hasSkip = true;
				partsSkipped++; // :: means we skipped an extra part in between the two
								// delimiters.
				if (i == 0) {
					partsSkipped++; // Begins with ::, so we skipped the part preceding
									// the first :
				}
		...
		try {
			// Iterate through the parts of the ip string.
			// Invariant: start is always the beginning of a hextet, or the second ':' of
			// the skip
			// sequence "::"
			int start = 0;
	...
	private static byte parseOctet(String ipString, int start, int end) {
		// Note: we already verified that this string contains only hex digits, but the
		// string may still
	 ...
	 *
	 * <p>
	 * {@link InetAddress#getByAddress} is documented as throwing a checked exception "if
	 * IP address is of illegal length." We replace it with an unchecked exception, for
	 * use by callers who already know that addr is an array of length 4 or 16.
	 ...
	 *
	 * <p>
	 * For IPv4 addresses, this is identical to {@link InetAddress#getHostAddress()}, but
	 * for IPv6 addresses, the output follows
	 * <a href="http://tools.ietf.org/html/rfc5952">RFC 5952</a> section 4. The main
	 * difference is that this method uses "::" for zero compression, while Java's version
	 * uses the uncompressed form.
	 *
	 * <p>
	 * This method uses hexadecimal for all IPv6 addresses, including IPv4-mapped IPv6
	 * addresses such as "::c000:201". The output does not include a Scope ID.
	 ...
	 *
	 * <p>
	 * Only runs of two or more hextets are considered. In case of a tie, the leftmost run
	 * wins. If a qualifying run is found, its hextets are replaced by the sentinel value
	 * -1.
	 ...
	 *
	 * <p>
	 * In order for "::" compression to work, the input should contain negative sentinel
	 * values in place of the elided zeroes.
	...
	/**
	 * Returns the string representation of an {@link InetAddress} suitable for inclusion
	 * in a URI.
	 *
	 * <p>
	 * For IPv4 addresses, this is identical to {@link InetAddress#getHostAddress()}, but
	 * for IPv6 addresses it compresses zeroes and surrounds the text with square
	 * brackets; for example {@code
	 * "[2001:db8::1]"}.
	 *
	 * <p>
	 * Per section 3.2.2 of
	 * <a target="_parent" href="http://tools.ietf.org/html/rfc3986#section-3.2.2">RFC
	 * 3986</a>, a URI containing an IPv6 string literal is of the form
	 ...
	 * {@link InetAddress#toString()} when an IP address string literal is desired. This
	 * is because {@link InetAddress#toString()} prints the hostname and the IP address
	 * string joined by a "/".
	 ...
	 * encoded in the format specified by RFC 3986 section 3.2.2.
	 *
	 * <p>
	 * This method is similar to {@link InetAddresses#forString(String)}, however, it
	 * requires that IPv6 addresses are surrounded by square brackets.
	 *
	 * <p>
	 * This method is the inverse of {@link InetAddresses#toUriString(InetAddress)}.
	 *
	 * <p>
	 * This method accepts non-ASCII digits, for example {@code "１９２.１６８.０.１"} (those are
	 * fullwidth characters). That is consistent with {@link InetAddress}, but not with
	 ...
	 * @throws IllegalArgumentException if {@code hostAddr} is not a valid IPv4 address,
	...
	/**
	 * Returns {@code true} if the supplied string is a valid URI IP string literal,
	 * {@code false} otherwise.
	 *
	 * <p>
	 * This method accepts non-ASCII digits, for example {@code "１９２.１６８.０.１"} (those are
	 ...
	 *
	 * <p>
	 * An "IPv4 compatible", or "compat", address is one with 96 leading bits of zero,
	 * with the remaining 32 bits interpreted as an IPv4 address. These are conventionally
	 * represented in string literals as {@code "::192.168.0.1"}, though
	 * {@code "::c0a8:1"} is also considered an IPv4 compatible address (and equivalent to
	 * {@code "::192.168.0.1"}).
	 *
	 * <p>
	 * For more on IPv4 compatible addresses see section 2.5.5.1 of
	 * <a target="_parent" href="http://tools.ietf.org/html/rfc4291#section-2.5.5.1">RFC
	 * 4291</a>.
	 *
	 * <p>
	 * NOTE: This method is different from {@link Inet6Address#isIPv4CompatibleAddress} in
	 * that it more correctly classifies {@code "::"} and {@code "::1"} as proper IPv6
	 * addresses (which they are), NOT IPv4 compatible addresses (which they are generally
	 * NOT considered to be).
	 * @param ip {@link Inet6Address} to be examined for embedded IPv4 compatible address
	 * format
	 ...
	 * @throws IllegalArgumentException if the argument is not a valid IPv4 compatible
	 * address
	 */
	 ...
	 *
	 * <p>
	 * 6to4 addresses begin with the {@code "2002::/16"} prefix. The next 32 bits are the
	 * IPv4 address of the host to which IPv6-in-IPv4 tunneled packets should be routed.
	 *
	 * <p>
	 * For more on 6to4 addresses see section 2 of
	 * <a target="_parent" href="http://tools.ietf.org/html/rfc3056#section-2">RFC
	 * 3056</a>.
	 ...
	 *
	 * <p>
	 * From RFC 5214: "ISATAP interface identifiers are constructed in Modified EUI-64
	 * format [...] by concatenating the 24-bit IANA OUI (00-00-5E), the 8-bit hexadecimal
	 * value 0xFE, and a 32-bit IPv4 address in network byte order [...]"
	 *
	 * <p>
	 * For more on ISATAP addresses see section 6.1 of
	 * <a target="_parent" href="http://tools.ietf.org/html/rfc5214#section-6.1">RFC
	 * 5214</a>.
	...
	/**
	 * Examines the Inet6Address to determine if it is an IPv6 address of one of the
	 * specified address types that contain an embedded IPv4 address.
	 *
	 * <p>
	 * NOTE: ISATAP addresses are explicitly excluded from this method due to their
	 * trivial spoofability. With other transition addresses spoofing involves (at least)
	 * infection of one's BGP routing table.
	...
	/**
	 * Examines the Inet6Address to extract the embedded IPv4 client address if the
	 * InetAddress is an IPv6 address of one of the specified address types that contain
	 ...
	 * @throws IllegalArgumentException if the argument does not have a valid embedded
	 * IPv4 address
	 */
	 ...
	 *
	 * <p>
	 * An "IPv4 mapped" address is anything in the range ::ffff:0:0/96 (sometimes written
	 * as ::ffff:0.0.0.0/96), with the last 32 bits interpreted as an IPv4 address.
	 *
	 * <p>
	 * For more on IPv4 mapped addresses see section 2.5.5.2 of
	 * <a target="_parent" href="http://tools.ietf.org/html/rfc4291#section-2.5.5.2">RFC
	 * 4291</a>.
	 *
	 * <p>
	 * Note: This method takes a {@code String} argument because {@link InetAddress}
	 * automatically collapses mapped addresses to IPv4. (It is actually possible to avoid
	 * this using one of the obscure {@link Inet6Address} methods, but it would be unwise
	 * to depend on such a poorly-documented feature.)
	 *
	 * <p>
	 * This method accepts non-ASCII digits. That is consistent with {@link InetAddress},
	 * but not with various RFCs. If you want to accept ASCII digits only, you can use
	 * something like {@code
	 * CharMatcher.ascii().matchesAllOf(ipString)}.
	 * @param ipString {@code String} to be examined for embedded IPv4-mapped IPv6 address
	 * format
	 ...
	 *
	 * <p>
	 * HACK: As long as applications continue to use IPv4 addresses for indexing into
	 * tables, accounting, et cetera, it may be necessary to <b>coerce</b> IPv6 addresses
	 * into IPv4 addresses. This method does so by hashing 64 bits of the IPv6 address
	 * into {@code 224.0.0.0/3} (64 bits into 29 bits):
	 *
	 * <ul>
	 * <li>If the IPv6 address contains an embedded IPv4 address, the function hashes
	 * that.
	 ...
	 *
	 * <p>
	 * NOTE: This method is failsafe for security purposes: ALL IPv6 addresses (except
	 * localhost (::1)) are hashed to avoid the security risk associated with extracting
	 * an embedded IPv4 address that might permit elevated privileges.
	...
	/**
	 * Returns an integer representing an IPv4 address regardless of whether the supplied
	 * argument is an IPv4 address or not.
	 *
	 * <p>
	 * IPv6 addresses are <b>coerced</b> to IPv4 addresses before being converted to
	 * integers.
	 *
	 * <p>
	 * As long as there are applications that assume that all IP addresses are IPv4
	 * addresses and can therefore be converted safely to integers (for whatever purpose)
	 * this function can be used to handle IPv6 addresses as well until the application is
	 * suitably fixed.
	 *
	 * <p>
	 * NOTE: an IPv6 address coerced to an IPv4 address can only be used for such purposes
	 * as rudimentary identification or indexing into a collection of real
	 * {@link InetAddress}es. They cannot be used as real addresses for the purposes of
	 ...
	 * be constrainted to 32 bits, otherwise it is constrained to 128 bits.
	 ...
	 * @throws IllegalArgumentException if the BigInteger is not between 0 and maximum
	 * value for IPv4 or IPv6 respectively
	 */
		...
		for (int i = 0; i < srcPos; i++) {
			if (addressBytes[i] != 0x00) {
				throw formatIllegalArgumentException(
						"BigInteger cannot be converted to InetAddress because it has more than %d" + " bytes: %s",
						numBytes, address);
			}
		}
	...
	/**
	 * Returns an address from a <b>little-endian ordered</b> byte array (the opposite of
	 * what {@link InetAddress#getByAddress} expects).
	 *
	 * <p>
	...
	/**
	 * Returns a new InetAddress that is one less than the passed in address. This method
	...
	/**
	 * Returns a new InetAddress that is one more than the passed in address. This method
	 * works for both IPv4 and IPv6 addresses.
	 ...
	 *
	 * <p>
	 * All of the fields in this class are encoded in various portions of the IPv6 address
	 * as part of the protocol. More protocols details can be found at:
	 * <a target="_parent" href=
	 * "http://en.wikipedia.org/wiki/Teredo_tunneling">http://en.wikipedia.
	...
	public static final class TeredoInfo {
		 ...
		 *
		 * <p>
		 * Both server and client can be {@code null}, in which case the value
		 * {@code "0.0.0.0"} will be assumed.
		 * @throws IllegalArgumentException if either of the {@code port} or the
		 * {@code flags} arguments are out of range of an unsigned short
		 */
		...
		public TeredoInfo(@CheckForNull Inet4Address server, @CheckForNull Inet4Address client, int port, int flags) {
			...
			this.server = org.magneton.core.base.MoreObjects.firstNonNull(server, ANY4);

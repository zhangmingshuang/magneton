 ...
 * brackets, colons, and port numbers. Full validation of the host field (if desired) is
...
public final class HostAndPort implements Serializable {
	 ...
	 * {@link #requireBracketsForIPv6()} to prohibit these.
	 * @param host the host string to parse. Must not contain a port number.
	 * @param port a port number from [0..65535]
	 * @return if parsing was successful, a populated HostAndPort object.
	 * @throws IllegalArgumentException if {@code host} contains a port number, or
	 * {@code port} is out of range.
	 */
	...
	public static HostAndPort fromString(String hostPortString) {
		...
		if (!Strings.isNullOrEmpty(portString)) {
			...
			Preconditions.checkArgument(!portString.startsWith("+") && CharMatcher.ascii().matchesAllOf(portString),
	...
	/**
	 * Parses a bracketed host-port string, throwing IllegalArgumentException if parsing
	 * fails.
	...
	private static String[] getHostAndPortFromBracketedHost(String hostPortString) {
		...
		if (closeBracketIndex + 1 == hostPortString.length()) {
			...
			for (int i = closeBracketIndex + 2; i < hostPortString.length(); ++i) {
				Preconditions.checkArgument(Character.isDigit(hostPortString.charAt(i)), "Port must be numeric: %s",
						hostPortString);
			}
	...
	/**
	 * Returns the portion of this {@code HostAndPort} instance that should represent the
	 * hostname or IPv4/IPv6 literal.
	 *
	 * <p>
	 * A successful parse does not imply any degree of sanity in this field. For
	 * additional validation, see the {@link HostSpecifier} class.
	 *
	 ...
	 *
	 * <p>
	 * You can chain this after {@link #fromString(String)} to include a port in case the
	 * port was omitted from the input string. If a port was already provided, then this
	 * method is a no-op.
	 ...
	 *
	 * <p>
	 * URI formatting requires that IPv6 literals be surrounded by brackets, like
	 * "[2001:db8::1]". Chain this call after {@link #fromString(String)} to increase the
	 * strictness of the parser, and disallow IPv6 literals that don't contain these
	 * brackets.
	 *
	 * <p>
	 * Note that this parser identifies IPv6 literals solely based on the presence of a
	 * colon. To perform actual validation of IP addresses, see the
	 * {@link InetAddresses#forString(String)} method.
	...
	public boolean equals(@CheckForNull Object other) {
		...
		if (other instanceof HostAndPort) {
			...
			return org.magneton.core.base.Objects.equal(host, that.host) && port == that.port;
		}
		return false;
	}

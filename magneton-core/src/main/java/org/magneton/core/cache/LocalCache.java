...
import org.magneton.core.cache.CacheBuilder.NullListener;
import org.magneton.core.cache.CacheBuilder.OneWeigher;
import org.magneton.core.collect.AbstractSequentialIterator;
...
import org.magneton.core.concurrent.SettableFuture;
import org.magneton.core.concurrent.UncheckedExecutionException;
import org.magneton.core.concurrent.Uninterruptibles;
...
import static org.magneton.core.concurrent.Futures.transform;
import static org.magneton.core.concurrent.MoreExecutors.directExecutor;
import static org.magneton.core.concurrent.Uninterruptibles.getUninterruptibly;

/**
 * The concurrent hash map implementation built by
 * {@link org.magneton.core.cache.CacheBuilder}.
 *
 * <p>
...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	/*
	 * The basic strategy is to subdivide the table among Segments, each of which itself
	 * is a concurrently readable hash table. The map supports non-blocking reads and
	 ...
	 * using a page-replacement algorithm to determine which entries to evict when the
	 * capacity has been exceeded.
	 *
	 * The page replacement algorithm's data structures are kept casually consistent with
	 * the map. The ordering of writes to a segment is sequentially consistent. An update
	 * to the map and recording of reads may not be immediately reflected on the
	 * algorithm's data structures. These structures are guarded by a lock and operations
	 * are applied in batches to avoid lock contention. The penalty of applying the
	 * batches is spread across threads so that the amortized cost is slightly higher than
	 * performing just the operation without enforcing the capacity constraint.
	 *
	 * This implementation uses a per-segment queue to record a memento of the additions,
	 * removals, and accesses that were performed on the map. The queue is drained on
	 * writes and when it exceeds its capacity threshold.
	 *
	 * The Least Recently Used page replacement algorithm was chosen due to its
	 * simplicity, high hit rate, and ability to be implemented with O(1) time complexity.
	 * The initial LRU implementation operates per-segment rather than globally for
	 * increased implementation simplicity. We expect the cache hit rate to be similar to
	 * that of a global LRU algorithm.
	 */
	 ...
	 * the constructors with arguments. MUST be a power of two {@code <= 1<<30} to ensure
	 ...
	 * cache's recency ordering information is updated. This is used to avoid lock
	 * contention by recording a memento of reads and delaying a lock acquisition until
	 * the threshold is crossed or a mutation occurs.
	 *
	 * <p>
	 ...
	 * independently to the cleanup queue and both reference queues.
	 */
	...
	/**
	 * Mask value for indexing into segments. The upper bits of a key's hash code are used
	 * to choose the segment.
	 */
	final int segmentMask;

	/**
	 * Shift value for indexing within segments. Helps prevent entries that end up in the
	 * same segment from also ending up in the same bucket.
	 */
	...
	// TODO(fry): define a new type which creates event objects and automates the clear
	// logic
	final Queue<RemovalNotification<K, V>> removalNotificationQueue;

	/**
	 * A listener that is invoked when an entry is removed due to expiration or garbage
	 * collection of soft/weak entries.
	 */
	...
	/**
	 * Accumulates global cache statistics. Note that there are also per-segments stats
	 * counters which must be aggregated to obtain a global stats view.
	 */
	 ...
	 * concurrency level.
	 */
	LocalCache(org.magneton.core.cache.CacheBuilder<? super K, ? super V> builder,
			@Nullable CacheLoader<? super K, V> loader) {
		this.concurrencyLevel = Math.min(builder.getConcurrencyLevel(), MAX_SEGMENTS);
		...
		this.removalNotificationQueue = (this.removalListener == NullListener.INSTANCE)
		...
		// maximumSize/Weight is specified in which case ensure that each segment gets at
		// least 10
		...
		int segmentShift = 0;
		int segmentCount = 1;
		while (segmentCount < this.concurrencyLevel && (!this.evictsBySize() || segmentCount * 20 <= this.maxWeight)) {
			++segmentShift;
			segmentCount <<= 1;
		}
		...
		if (this.evictsBySize()) {
			...
			for (int i = 0; i < this.segments.length; ++i) {
				this.segments[i] = this.createSegment(segmentSize, UNSET_INT, builder.getStatsCounterSupplier().get());
			}
		}
	}

	/** Singleton placeholder that indicates a value is being loaded. */
	// impl never uses a parameter or returns any non-null
	// value
	...
	/**
	 * Applies a supplemental hash function to a given hash code, which defends against
	 * poor quality hash functions. This is critical when the concurrent hash map uses
	 * power-of-two length hash tables, that otherwise encounter collisions for hash codes
	...
	/*
	 * Note: All of this duplicate code sucks, but it saves a lot of memory. If only Java
	 * had mixins! To maintain this code, make a change for the strong reference type.
	 * Then, cut and paste, and replace "Strong" with "Soft" or "Weak" within the pasted
	 * text. The primary difference is that strong entries store the key reference
	 * directly while soft and weak entries delegate to their respective superclasses.
	 */
	...
	/**
	 * This method is a convenience for testing. Code should call {@link Segment#newEntry}
	 * directly.
	 */
	@VisibleForTesting
	...
	/**
	 * This method is a convenience for testing. Code should call {@link Segment#setValue}
	 * instead.
	 */
	// Guarded By Segment.this
	@VisibleForTesting
	ValueReference<K, V> newValueReference(ReferenceEntry<K, V> entry, V value, int weight) {
		...
		return this.valueStrength.referenceValue(this.segmentFor(hash), entry, checkNotNull(value), weight);
	}
	 ...
	 * partially-collected, loading, or expired. Unlike {@link Segment#getLiveValue} this
	 * method does not attempt to cleanup stale entries. As such it should only be called
	 * outside of a segment context, such as during iteration.
	 */
	@Nullable
	 ...
	 * eviction, or eligibility for garbage collection. This should be called every time
	 * expireEntries or evictEntry is called (once the lock is released).
	 */
	...
	public boolean isEmpty() {
		/*
		 * Sum per-segment modCounts to avoid mis-reporting when elements are concurrently
		 * added and removed in one segment while checking another, in which case the
		 * table was never actually empty at any point. (The sum ensures accuracy up
		 * through at least 1<<31 per-segment modifications before recheck.) Method
	...
	ImmutableMap<K, V> getAll(Iterable<? extends K> keys) throws ExecutionException {
		...
		Map<K, V> result = org.magneton.core.collect.Maps.newLinkedHashMap();
		...
		try {
			if (!keysToLoad.isEmpty()) {
				try {
					Map<K, V> newEntries = this.loadAll(unmodifiableSet(keysToLoad), this.defaultLoader);
	 ...
	 * {@code loader} doesn't implement {@code loadAll}.
	 */
	@Nullable
	Map<K, V> loadAll(Set<? extends K> keys, CacheLoader<? super K, V> loader) throws ExecutionException {
		...
		try {
			...
			if (!success) {
				this.globalStatsCounter.recordLoadException(stopwatch.elapsed(NANOSECONDS));
			}
		}
		...
		this.globalStatsCounter.recordLoadSuccess(stopwatch.elapsed(NANOSECONDS));
		return result;
	}

	/**
	 * Returns the internal entry for the specified key. The entry may be loading,
	 * expired, or partially collected.
	 */
	...
	public boolean containsValue(@Nullable Object value) {
		...
		// around in the map
		// such that none of the subsequent iterations observed it, despite the fact that
		// at every point
	...
	enum Strength {
		...
		/*
		 * TODO(kevinb): If we strongly reference the value and aren't loading, we needn't
		 * wrap the value. This could save ~8 bytes per entry.
		 */
		 ...
		 * values referenced at this strength. This strategy will be used unless the user
		 * explicitly specifies an alternate strategy.
		 */
	...
	interface ValueReference<K, V> {
		 ...
		 * block (in the case of FutureValueReference).
		...
		/**
		 * Returns the weight of this entry. This is assumed to be static between calls to
		 * setValue.
		 */
		int getWeight();

		/**
		 * Returns the entry associated with this value reference, or {@code null} if this
		 * value reference is independent of any entry.
		 */
		@Nullable
		...
		/**
		 * Notify pending loads that a new value was set. This is only relevant to loading
		 * value references.
		 */
		void notifyNewValue(@Nullable V newValue);

		/**
		 * Returns true if a new value is currently loading, regardless of whether or not
		 * there is an existing value. It is assumed that the return value of this method
		 * is constant for any given ValueReference instance.
		 */
		boolean isLoading();

		/**
		 * Returns true if this reference contains an active value, meaning one that is
		 * still considered present in the cache. Active values consist of live values,
		 * which are returned by cache lookups, and dead values, which have been evicted
		 * but awaiting removal. Non-active values consist strictly of loading values,
	...
	static class WeakEntry<K, V> extends WeakReference<K> implements ReferenceEntry<K, V> {
		 ...
		 * already extending WeakReference<K>.
		 */
	 ...
	 * ReentrantLock opportunistically, just to simplify some locking and avoid separate
	 * construction.
	 */
	...
	static class Segment<K, V> extends ReentrantLock {
		...
		/*
		 * TODO(fry): Consider copying variables (like evictsBySize) from outer class into
		 ...
		 * state, so can be read without locking. Next fields of nodes are immutable
		 * (final). All list additions are performed at the front of each bin. This makes
		 * it easy to check changes, and also fast to traverse. When nodes would otherwise
		 * be changed, new nodes are created to replace them. This works well for hash
		 * tables since the bin lists tend to be short. (The average length is less than
		 * two.)
		 *
		 * Read operations can thus proceed without locking, but rely on selected uses of
		 * volatiles to ensure that completed write operations performed by other threads
		 * are noticed. For most purposes, the "count" field, tracking the number of
		 * elements, serves as that volatile variable ensuring visibility. This is
		 ...
		 * should not look at table entries if it is 0.
		 *
		 * - All (synchronized) write operations should write to the "count" field after
		 * structurally changing any bin. The operations must not take any action that
		 * could even momentarily cause a concurrent read operation to see inconsistent
		 * data. This is made easier by the nature of the read operations in Map. For
		 * example, no operation can reveal that the table has grown but the threshold has
		 * not yet been updated, so there are no atomicity requirements for this with
		 * respect to reads.
		 *
		 ...
		 * collected, and which need to be cleaned up internally.
		 */
		final @Nullable ReferenceQueue<K> keyReferenceQueue;

		/**
		 * The value reference queue contains value references whose values have been
		 * garbage collected, and which need to be cleaned up internally.
		 */
		final @Nullable ReferenceQueue<V> valueReferenceQueue;

		/**
		 * The recency queue is used to record which entries were accessed for updating
		 * the access list's ordering. It is drained as a batch operation when either the
		 * DRAIN_THRESHOLD is crossed or a write occurs on the segment.
		 */
		final Queue<ReferenceEntry<K, V>> recencyQueue;

		/**
		 * A counter of the number of reads since the last write, used to drain queues on
		 * a small fraction of read operations.
		 */
		final AtomicInteger readCount = new AtomicInteger();

		/**
		 * A queue of elements currently in the map, ordered by write time. Elements are
		 * added to the tail of the queue on write.
		 */
		@GuardedBy("this")
		final Queue<ReferenceEntry<K, V>> writeQueue;

		/**
		 * A queue of elements currently in the map, ordered by access time. Elements are
		 * added to the tail of the queue on access (note that writes count as accesses).
		 */
		@GuardedBy("this")
		 ...
		 * change during a traversal of segments loading size or checking containsValue,
		 * then we might have an inconsistent view of state so (usually) must retry.
		 */
		int modCount;

		/**
		 * The table is expanded when its size exceeds this threshold. (The value of this
		 * field is always {@code (int) (capacity * 0.75)}.)
		 */
		int threshold;
		...
		Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight,
			...
			this.recencyQueue = map.usesAccessQueue() ? new ConcurrentLinkedQueue<ReferenceEntry<K, V>>()
					: LocalCache.<ReferenceEntry<K, V>>discardingQueue();

			this.writeQueue = map.usesWriteQueue() ? new WriteQueue<K, V>()
					: LocalCache.<ReferenceEntry<K, V>>discardingQueue();

			this.accessQueue = map.usesAccessQueue() ? new AccessQueue<K, V>()
					: LocalCache.<ReferenceEntry<K, V>>discardingQueue();
		}
		...
		@GuardedBy("this")
		ReferenceEntry<K, V> newEntry(K key, int hash, @Nullable ReferenceEntry<K, V> next) {
			return this.map.entryFactory.newEntry(this, checkNotNull(key), hash, next);
		}

		/**
		 * Copies {@code original} into a new entry chained to {@code newNext}. Returns
		 * the new entry, or {@code null} if {@code original} was already garbage
		 * collected.
		 */
		@GuardedBy("this")
		ReferenceEntry<K, V> copyEntry(ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext) {
		...
		V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader) throws ExecutionException {
				...
				for (e = first; e != null; e = e.getNext()) {
					...
					if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (valueReference.isLoading()) {
							...
							V value = valueReference.get();
							if (value == null) {
								this.enqueueNotification(entryKey, hash, value, valueReference.getWeight(),
										RemovalCause.COLLECTED);
							}
							else if (this.map.isExpired(e, now)) {
								// This is a duplicate check, as preWriteCleanup already
								// purged expired
								// entries, but let's accommodate an incorrect expiration
								// queue.
								this.enqueueNotification(entryKey, hash, value, valueReference.getWeight(),
			...
			if (createNewEntry) {
				try {
					// Synchronizes on the entry to allow failing fast when a recursive
					// load is
					// detected. This may be circumvented when an entry is copied, but
					// will fail fast most
					// of the time.
					synchronized (e) {
		...
		V compute(K key, int hash, BiFunction<? super K, ? super V, ? extends V> function) {
			...
			try {
				...
				for (e = first; e != null; e = e.getNext()) {
					...
					if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (this.map.isExpired(e, now)) {
							// This is a duplicate check, as preWriteCleanup already
							// purged expired
							// entries, but let's accomodate an incorrect expiration
							// queue.
							this.enqueueNotification(entryKey, hash, valueReference.get(), valueReference.getWeight(),
									RemovalCause.EXPIRED);
						}
				...
				// note valueReference can be an existing value or even itself another
				// loading value if
				...
				if (newValue != null) {
					if (valueReference != null && newValue == valueReference.get()) {
						...
						this.recordWrite(e, 0, now); // no change in weight
						return newValue;
					}
					try {
		...
		ListenableFuture<V> loadAsync(K key, int hash, LoadingValueReference<K, V> loadingValueReference,
		 ...
		 * already doing so. Returns the newly refreshed value associated with {@code key}
		 * if it was refreshed inline, or {@code null} if another thread is performing the
		 * refresh or if an error occurs during refresh.
		 */
		@Nullable
		 ...
		 * value reference is already loading.
		 */
		@Nullable
		 ...
		 * containing garbage collected keys or values.
		 */
		@GuardedBy("this")
		 ...
		 * {@code entry} to the recency queue. At write-time, or when the queue is full
		 * past the threshold, the queue will be drained and the entries therein
		 * processed.
		 *
		 * <p>
		...
		/**
		 * Updates the eviction metadata that {@code entry} was just read. This currently
		 * amounts to adding {@code entry} to relevant eviction lists.
		 *
		 * <p>
		 * Note: this method should only be called under lock, as it directly manipulates
		 * the eviction queues. Unlocked reads should use {@link #recordRead}.
		 */
		@GuardedBy("this")
		 ...
		 * were read in the specified relative order. This currently amounts to adding
		 * them to relevant eviction lists (accounting for the fact that they could have
		 * been removed from the map since being added to the recency queue).
		 */
		@GuardedBy("this")
		 ...
		 * cache if the newest entry exceeds the maximum weight all on its own.
		...
		/**
		 * Gets the value from an entry. Returns null if the entry is invalid,
		 * partially-collected, loading, or expired.
		 */
		V getLiveValue(ReferenceEntry<K, V> entry, long now) {
			if (entry.getKey() == null) {
				this.tryDrainReferenceQueues();
		...
		/**
		 * This method is a convenience for testing. Code should call
		 * {@link LocalCache#containsValue} directly.
		 */
		@VisibleForTesting
		...
		void expand() {
			 ...
			 * expansion, the elements from each bin must either stay at same index, or
			 * move with a power of two offset. We eliminate unnecessary node creation by
			 * catching cases where old nodes can be reused because their next fields
			 * won't change. Statistically, at the default threshold, only about one-sixth
					...
					if (next == null) {
						...
						for (ReferenceEntry<K, V> e = next; e != null; e = e.getNext()) {
							...
							if (newIndex != tailIndex) {
								// The index changed. We'll need to copy the previous
								// entry.
								tailIndex = newIndex;
								tail = e;
							}
					...
					if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
						...
						if (entryValue == null) {
							if (valueReference.isActive()) {
								...
								int newCount = this.count - 1;
								++this.modCount;
								ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e, entryKey, hash,
										entryValue, valueReference, RemovalCause.COLLECTED);
								newCount = this.count - 1;
			...
			try {
				...
				for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
					...
					if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
						...
						++this.modCount;
						ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e, entryKey, hash, entryValue,
		...
		ReferenceEntry<K, V> removeValueFromChain(ReferenceEntry<K, V> first, ReferenceEntry<K, V> entry,
				@Nullable K key, int hash, V value, ValueReference<K, V> valueReference, RemovalCause cause) {
		...
		void removeCollectedEntry(ReferenceEntry<K, V> entry) {
			this.enqueueNotification(entry.getKey(), entry.getHash(), entry.getValueReference().get(),
		...
		boolean reclaimKey(ReferenceEntry<K, V> entry, int hash) {
			...
			try {
				...
				for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
					if (e == entry) {
						++this.modCount;
						ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e, e.getKey(), hash,
		...
		boolean reclaimValue(K key, int hash, ValueReference<K, V> valueReference) {
			...
			try {
				...
				this.unlock();
				if (!this.isHeldByCurrentThread()) { // don't cleanup inside of put
					this.postWriteCleanup();
				}
			}
		}
		 ...
		 * writes. If cleanup is not observed after a sufficient number of reads, try
		 * cleaning up from the read thread.
		 */
		void postReadCleanup() {
			if ((this.readCount.incrementAndGet() & DRAIN_THRESHOLD) == 0) {
				this.cleanUp();
			}
		}
		 ...
		 * every time a write thread acquires the segment lock, immediately after
		 * acquiring the lock.
		 *
		 * <p>
	...
	static class LoadingValueReference<K, V> implements ValueReference<K, V> {
		...
		public void notifyNewValue(@Nullable V newValue) {
			if (newValue != null) {
				...
				// The pending load was removed. Delay notifications until loading
				// completes.
				this.oldValue = unset();
			}
		...
		public ListenableFuture<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
			try {
				...
				return transform(newValue, new org.magneton.core.base.Function<V, V>() {
					@Override
		...
		public long elapsedNanos() {
			return this.stopwatch.elapsed(NANOSECONDS);
		}

		@Override
		public V waitForValue() throws ExecutionException {
			return getUninterruptibly(this.futureValue);
		}
	...
	/**
	 * A custom queue for managing eviction order. Note that this is tightly integrated
	 * with {@code
	 * ReferenceEntry}, upon which it relies to perform its linking.
	 *
	 * <p>
	 * Note that this entire implementation makes the assumption that all elements which
	 * are in the map are also in this queue, and that all elements not in the queue are
	 * not in the map.
	 *
	 * <p>
	 * The benefits of creating our own queue are that (1) we can replace elements in the
	 * middle of the queue as part of copyWriteEntry, and (2) the contains method is
	 * highly optimized for the current model.
	 */
	static final class WriteQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
		...
		public boolean offer(ReferenceEntry<K, V> entry) {
			...
			// add to tail
			connectWriteOrder(this.head.getPreviousInWriteQueue(), entry);
	...
	static final class AccessQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>> {
		...
		public boolean offer(ReferenceEntry<K, V> entry) {
			...
			// add to tail
			connectAccessOrder(this.head.getPreviousInAccessQueue(), entry);
	...
	/**
	 * Serializes the configuration of a LocalCache, reconstituting it as a Cache using
	 * CacheBuilder upon deserialization. An instance of this class is fit for use by the
	 * writeReplace of LocalManualCache.
	 *
	 * <p>
	 * Unfortunately, readResolve() doesn't get called when a circular dependency is
	 * present, so the proxy must be able to behave as the cache itself.
	 */
	static class ManualSerializationProxy<K, V> extends ForwardingCache<K, V> implements Serializable {
		...
		private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
			...
			org.magneton.core.cache.CacheBuilder<K, V> builder = this.recreateCacheBuilder();
	...
	/**
	 * Serializes the configuration of a LocalCache, reconstituting it as an LoadingCache
	 * using CacheBuilder upon deserialization. An instance of this class is fit for use
	 * by the writeReplace of LocalLoadingCache.
	 *
	 * <p>
	 * Unfortunately, readResolve() doesn't get called when a circular dependency is
	 * present, so the proxy must be able to behave as the cache itself.
	...
	static class LocalManualCache<K, V> implements Cache<K, V>, Serializable {
		...
		LocalManualCache(org.magneton.core.cache.CacheBuilder<? super K, ? super V> builder) {
	...
	/**
	 * Custom Entry class used by EntryIterator.next(), that relays setValue changes to
	 * the underlying map.
	 */
	final class WriteThroughEntry implements Entry<K, V> {
		...
		public V setValue(V newValue) {
			...
			this.value = newValue; // only if put succeeds
			return oldValue;
		}
	...
	abstract class AbstractCacheSet<T> extends AbstractSet<T> {
		...
		// super.toArray() may misbehave if size() is inaccurate, at least on old versions
		// of Android.
		// https://code.google.com/p/android/issues/detail?id=36519 /
		// http://r.android.com/47508

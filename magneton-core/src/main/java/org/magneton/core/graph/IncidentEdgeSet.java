...
abstract class IncidentEdgeSet<N> extends AbstractSet<EndpointPair<N>> {
	...
	final org.magneton.core.graph.BaseGraph<N> graph;

	IncidentEdgeSet(org.magneton.core.graph.BaseGraph<N> graph, N node) {
		this.graph = graph;
		this.node = node;
	}
	...
	public boolean contains(@CheckForNull Object obj) {
		...
		if (graph.isDirected()) {
			...
			return (node.equals(nodeV) && adjacent.contains(nodeU)) || (node.equals(nodeU) && adjacent.contains(nodeV));
		}
	}

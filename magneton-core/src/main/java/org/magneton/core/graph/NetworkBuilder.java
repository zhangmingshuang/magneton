...
/**
 * A builder for constructing instances of {@link org.magneton.core.graph.MutableNetwork}
 * or {@link ImmutableNetwork} with user-defined properties.
 *
 * <p>
 ...
 * <li>orders {@link org.magneton.core.graph.Network#nodes()} and
 * {@link org.magneton.core.graph.Network#edges()} in the order in which the elements were
 * added
 * </ul>
 *
 * <p>
 ...
 * the builder is constructed based on an existing {@code Network} using
 * {@link #from(org.magneton.core.graph.Network)}.
 * @param <E> The most general edge type this builder will support. This is normally
 * {@code Object} unless it is constrained by using a method like {@link #edgeOrder}, or
 * the builder is constructed based on an existing {@code Network} using
 * {@link #from(org.magneton.core.graph.Network)}.
 * @since 20.0
 */
...
public final class NetworkBuilder<N, E> extends AbstractGraphBuilder<N> {
	...
	/**
	 * Returns a {@link NetworkBuilder} initialized with all properties queryable from
	 * {@code
	 * network}.
	 *
	 * <p>
	 * The "queryable" properties are those that are exposed through the
	 * {@link org.magneton.core.graph.Network} interface, such as
	 * {@link org.magneton.core.graph.Network#isDirected()}. Other properties, such as
	 * {@link #expectedNodeCount(int)}, are not set in the new builder.
	 */
	public static <N, E> NetworkBuilder<N, E> from(org.magneton.core.graph.Network<N, E> network) {
		return new NetworkBuilder<N, E>(network.isDirected()).allowsParallelEdges(network.allowsParallelEdges())
				.allowsSelfLoops(network.allowsSelfLoops()).nodeOrder(network.nodeOrder())
				.edgeOrder(network.edgeOrder());
	}

	/**
	 * Returns an {@link ImmutableNetwork.Builder} with the properties of this
	 * {@link NetworkBuilder}.
	 *
	 * <p>
	...
	/**
	 * Specifies whether the network will allow parallel edges. Attempting to add a
	 * parallel edge to a network that does not allow them will throw an
	...
	/**
	 * Specifies whether the network will allow self-loops (edges that connect a node to
	 ...
	 * {@link org.magneton.core.graph.Network#nodes()}.
	 *
	 * <p>
	 * The default value is {@link ElementOrder#insertion() insertion order}.
	 */
	...
	/**
	 * Returns an empty {@link org.magneton.core.graph.MutableNetwork} with the properties
	 * of this {@link NetworkBuilder}.
	 */

...
/**
 * A {@link org.magneton.core.graph.MapIteratorCache} that adds additional caching. In
 * addition to the caching provided by {@link org.magneton.core.graph.MapIteratorCache},
 * this structure caches values for the two most recently retrieved keys.
 *
 * @author James Sexton
 */
@ElementTypesAreNonnullByDefault
final class MapRetrievalCache<K, V> extends org.magneton.core.graph.MapIteratorCache<K, V> {
	...
	V getIfCached(@CheckForNull Object key) {
		...
		// Store a local reference to the cache entry. If the backing map is immutable,
		// this,
	...
	private void addToCache(CacheEntry<K, V> entry) {
		// Slide new entry into first cache position. Drop previous entry in second cache
		// position.

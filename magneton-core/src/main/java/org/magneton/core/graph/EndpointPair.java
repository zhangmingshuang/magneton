...
import static org.magneton.core.graph.GraphConstants.NOT_AVAILABLE_ON_UNDIRECTED;

/**
 * An immutable pair representing the two endpoints of an edge in a graph. The
 * {@link EndpointPair} of a directed edge is an ordered pair of nodes ({@link #source()}
 * and {@link #target()}). The {@link EndpointPair} of an undirected edge is an unordered
 * pair of nodes ({@link #nodeU()} and {@link #nodeV()}).
 *
 * <p>
...
public abstract class EndpointPair<N> implements Iterable<N> {
	...
	public static <N> EndpointPair<N> unordered(N nodeU, N nodeV) {
		// Swap nodes on purpose to prevent callers from relying on the "ordering" of an
		// unordered pair.
	...
	/**
	 * If this {@link EndpointPair} {@link #isOrdered()}, returns the node which is the
	 * source.
	...
	/**
	 * If this {@link EndpointPair} {@link #isOrdered()} returns the {@link #source()};
	...
	/**
	 * Returns the node {@link #adjacentNode(Object) adjacent} to {@link #nodeU()} along
	 * the origin edge. If this {@link EndpointPair} {@link #isOrdered()}, this is equal
	 * to {@link #target()}.
	 */
	 ...
	 * @throws IllegalArgumentException if this {@link EndpointPair} does not contain
	 * {@code node}
	 * @since 20.0 (but the argument type was changed from {@code Object} to {@code N} in
	 * 31.0)
	 */
	 ...
	 * {@link #target()} are equal. Two unordered {@link EndpointPair}s are equal if they
	 * contain the same nodes. An ordered {@link EndpointPair} is never equal to an
	 * unordered {@link EndpointPair}.
	 */
	@Override
	public abstract boolean equals(@CheckForNull Object obj);

	/**
	 * The hashcode of an ordered {@link EndpointPair} is equal to
	 * {@code Objects.hashCode(source(),
	...
	private static final class Unordered<N> extends EndpointPair<N> {
		...
		public boolean equals(@CheckForNull Object obj) {
			...
			// nodeV().equals(other.nodeV());
			// boolean condition2 = nodeU().equals(other.nodeV()) &&
			// nodeV().equals(other.nodeU());
			// return condition1 || condition2;
			if (nodeU().equals(other.nodeU())) { // check condition1
				// Here's the tricky bit. We don't have to explicitly check for condition2
				// in this case.
				// Why? The second half of condition2 requires that nodeV equals
				// other.nodeU.
				// We already know that nodeU equals other.nodeU. Combined with the
				// earlier statement,
				// and the transitive property of equality, this implies that nodeU equals
				// nodeV.
				// If nodeU equals nodeV, condition1 == condition2, so checking condition1
				// is sufficient.
				return nodeV().equals(other.nodeV());
			}
			return nodeU().equals(other.nodeV()) && nodeV().equals(other.nodeU()); // check
																					// condition2
		}

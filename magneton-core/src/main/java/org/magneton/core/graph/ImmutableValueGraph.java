...
/**
 * A {@link org.magneton.core.graph.ValueGraph} whose elements and structural
 * relationships will never change. Instances of this class may be obtained with
 * {@link #copyOf(org.magneton.core.graph.ValueGraph)}.
 *
 * <p>
...
// Extends StandardValueGraph but uses ImmutableMaps.
@ElementTypesAreNonnullByDefault
public final class ImmutableValueGraph<N, V> extends StandardValueGraph<N, V> {

	private ImmutableValueGraph(org.magneton.core.graph.ValueGraph<N, V> graph) {
		super(org.magneton.core.graph.ValueGraphBuilder.from(graph), getNodeConnections(graph), graph.edges().size());
	}

	/** Returns an immutable copy of {@code graph}. */
	public static <N, V> ImmutableValueGraph<N, V> copyOf(org.magneton.core.graph.ValueGraph<N, V> graph) {
		return (graph instanceof ImmutableValueGraph) ? (ImmutableValueGraph<N, V>) graph
	...
	private static <N, V> ImmutableMap<N, GraphConnections<N, V>> getNodeConnections(
			org.magneton.core.graph.ValueGraph<N, V> graph) {
	...
	private static <N, V> GraphConnections<N, V> connectionsOf(ValueGraph<N, V> graph, N node) {
		Function<N, V> successorNodeToValueFn = (N successorNode) ->
	...
	public org.magneton.core.graph.ElementOrder<N> incidentEdgeOrder() {
		return org.magneton.core.graph.ElementOrder.stable();
	}

	@Override
	public org.magneton.core.graph.ImmutableGraph<N> asGraph() {
		return new ImmutableGraph<>(this); // safe because the view is effectively
											// immutable
	}

	/**
	 * A builder for creating {@link ImmutableValueGraph} instances, especially
	...
	public static class Builder<N, V> {
		 ...
		 * present, and sets a value for that edge to {@code value} (overwriting the
		...
		/**
		 * Adds an edge connecting {@code endpoints} if one is not already present, and
		 * sets a value for that edge to {@code value} (overwriting the existing value, if
		 * any).
		 *
		 * <p>
		 * If the graph is directed, the resultant edge will be directed; otherwise, it
		...
		/**
		 * Returns a newly-created {@code ImmutableValueGraph} based on the contents of
		 * this {@code
		 * Builder}.
		 */

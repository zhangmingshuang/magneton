...
import static org.magneton.core.collect.Iterables.transform;
...
final class Types {
	...
	private static final org.magneton.core.base.Joiner COMMA_JOINER = Joiner.on(", ").useForNull("null");
	...
	/**
	 * Returns a type where {@code rawType} is parameterized by {@code arguments} and is
	 * owned by {@code ownerType}.
	 */
	static ParameterizedType newParameterizedTypeWithOwner(@CheckForNull Type ownerType, Class<?> rawType,
			Type... arguments) {
		if (ownerType == null) {
			return newParameterizedType(rawType, arguments);
		}
		// ParameterizedTypeImpl constructor already checks, but we want to throw NPE
		// before IAE
	...
	/**
	 * Returns {@code ? extends X} if any of {@code bounds} is a subtype of {@code X[]};
	 * or null otherwise.
	 */
	@CheckForNull
	...
	private static <D extends GenericDeclaration> TypeVariable<D> newTypeVariableImpl(D genericDeclaration, String name,
		...
		TypeVariable<D> typeVariable = Reflection.newProxy(TypeVariable.class,
	...
	private static Iterable<Type> filterUpperBounds(Iterable<Type> bounds) {
		return Iterables.filter(bounds, org.magneton.core.base.Predicates.not(Predicates.<Type>equalTo(Object.class)));
	}
	...
	/**
	 * Decides what owner type to use for constructing {@link ParameterizedType} from a
	 * raw class.
	 */
	private enum ClassOwnership {
		...
		private static ClassOwnership detectJvmBehavior() {
			...
			// requireNonNull is safe because we're examining a type that's known to have
			// a superclass.
			ParameterizedType parameterizedType = requireNonNull((ParameterizedType) subclass.getGenericSuperclass());
	...
	// TODO(benyu): Once behavior is the same for all Java versions we support, delete
	// this.
	enum JavaVersion {
		...
		JAVA8 {
			...
			String typeName(Type type) {
				try {
					 ...
					 * ReflectiveOperationException, which Animal Sniffer would reject.
					 * (Old versions of Android don't *seem* to mind, but there might be
					 * edge cases of which we're unaware.)
					 */
				}
	...
	private static final class GenericArrayTypeImpl implements GenericArrayType, Serializable {
		...
		public boolean equals(@CheckForNull Object obj) {
			if (obj instanceof GenericArrayType) {
				...
				return org.magneton.core.base.Objects.equal(getGenericComponentType(), that.getGenericComponentType());
			}
			return false;
		}
	...
	private static final class ParameterizedTypeImpl implements ParameterizedType, Serializable {
		...
		ParameterizedTypeImpl(@CheckForNull Type ownerType, Class<?> rawType, Type[] typeArguments) {
			...
			argumentsList = JavaVersion.CURRENT.usedInGenericType(typeArguments);
		}
		...
		public String toString() {
			...
			return builder.append(rawType.getName()).append('<')
					.append(COMMA_JOINER.join(transform(argumentsList, JavaVersion.CURRENT::typeName))).append('>')
					.toString();
		}
		...
		public boolean equals(@CheckForNull Object other) {
			...
			return getRawType().equals(that.getRawType()) && Objects.equal(getOwnerType(), that.getOwnerType())
	...
	/**
	 * Invocation handler to work around a compatibility problem between Java 7 and Java
	 * 8.
	 *
	 * <p>
	 * Java 8 introduced a new method {@code getAnnotatedBounds()} in the
	 * {@link TypeVariable} interface, whose return type {@code AnnotatedType[]} is also
	 * new in Java 8. That means that we cannot implement that interface in source code in
	 * a way that will compile on both Java 7 and Java 8. If we include the
	 * {@code getAnnotatedBounds()} method then its return type means it won't compile on
	 * Java 7, while if we don't include the method then the compiler will complain that
	 * an abstract method is unimplemented. So instead we use a dynamic proxy to get an
	 * implementation. If the method being called on the {@code TypeVariable} instance has
	 * the same name as one of the public methods of {@link TypeVariableImpl}, the proxy
	 * calls the same method on its instance of {@code TypeVariableImpl}. Otherwise it
	 * throws {@link UnsupportedOperationException}; this should only apply to
	 * {@code getAnnotatedBounds()}. This does mean that users on Java 8 who obtain an
	 * instance of {@code TypeVariable} from {@link TypeResolver#resolveType} will not be
	 * able to call {@code getAnnotatedBounds()} on it, but that should hopefully be rare.
	 *
	 * <p>
	 * TODO(b/147144588): We are currently also missing the methods inherited from
	 * {@link AnnotatedElement}, which {@code TypeVariable} began to extend only in Java
	 * 8. Those methods refer only to types present in Java 7, so we could implement them
	 * in {@code TypeVariableImpl} today. (We could probably then make
	 * {@code TypeVariableImpl} implement {@code AnnotatedElement} so that we get partial
	 * compile-time checking.)
	 *
	 * <p>
	 * This workaround should be removed at a distant future time when we no longer
	 * support Java versions earlier than 8.
	 */
			...
			for (Method method : TypeVariableImpl.class.getMethods()) {
				if (method.getDeclaringClass().equals(TypeVariableImpl.class)) {
					try {
						...
						// OK: the method is accessible to us anyway. The setAccessible
						// call is only for
		...
		public boolean equals(@CheckForNull Object obj) {
			if (NativeTypeVariableEquals.NATIVE_TYPE_VARIABLE_ONLY) {
				...
				if (obj != null && Proxy.isProxyClass(obj.getClass())
					...
					TypeVariableInvocationHandler typeVariableInvocationHandler = (TypeVariableInvocationHandler) Proxy
	...
	/**
	 * Per <a href="https://code.google.com/p/guava-libraries/issues/detail?id=1635">issue
	 * 1635</a>, In JDK 1.7.0_51-b13, {@link TypeVariableImpl#equals(Object)} is changed
	 * to no longer be equal to custom TypeVariable implementations. As a result, we need
	 * to make sure our TypeVariable implementation respects symmetry. Moreover, we don't
	 * want to reconstruct a native type variable {@code <A>} using our implementation
	 * unless some of its bounds have changed in resolution. This avoids creating unequal
	 * TypeVariable implementation unnecessarily. When the bounds do change, however, it's
	 * fine for the synthetic TypeVariable to be unequal to any native TypeVariable
	 * anyway.
	 */
	static final class NativeTypeVariableEquals<X> {

		static final boolean NATIVE_TYPE_VARIABLE_ONLY = !NativeTypeVariableEquals.class.getTypeParameters()[0]

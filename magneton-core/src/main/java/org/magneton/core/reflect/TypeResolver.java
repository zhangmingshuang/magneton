...
/**
 * An object of this class encapsulates type mappings from type variables. Mappings are
 * established with {@link #where} and types are resolved using {@link #resolveType}.
 *
 * <p>
 * Note that usually type mappings are already implied by the static type hierarchy (for
 * example, the {@code E} type variable declared by class {@code List} naturally maps to
 * {@code String} in the context of {@code class MyStringList implements List<String>}. In
 * such case, prefer to use {@link TypeToken#resolveType} since it's simpler and more type
 * safe. This class should only be used when the type mapping isn't implied by the static
 * type hierarchy, but provided through other means such as an annotation or external
 * configuration file.
 *
 * @author Ben Yu
 * @since 15.0
...
public final class TypeResolver {
	 ...
	 *
	 * <p>
	 * For example, when resolving {@code List<T>} in the context of {@code ArrayList<?>},
	 * {@code
	 * <T>} is covariantly resolved to {@code <?>} such that return type of
	 * {@code List::get} is {@code <?>}.
	 */
	 ...
	 * For example, when resolving {@code List<T>} in the context of {@code ArrayList<?>},
	 * {@code
	 * <T>} cannot be invariantly resolved to {@code <?>} because otherwise the parameter
	 * type of {@code List::set} will be {@code <?>} and it'll falsely say any object can
	 * be passed into {@code ArrayList<?>::set}.
	 *
	 * <p>
	 * Instead, {@code <?>} will be resolved to a capture in the form of a type variable
	 * {@code
	 * <capture-of-? extends Object>}, effectively preventing {@code set} from accepting
	 * any type.
	 */
	...
	private static void populateTypeMappings(Map<TypeVariableKey, Type> mappings, Type from, Type to) {
		...
		new TypeVisitor() {
			...
			void visitParameterizedType(ParameterizedType fromParameterizedType) {
				...
				Preconditions.checkArgument(fromArgs.length == toArgs.length, "%s not compatible with %s",
			...
			void visitClass(Class<?> fromClass) {
				...
				// And we don't support "assuming String is T"; user has to say "assuming
				// T is String".
	...
	/**
	 * Returns a new {@code TypeResolver} with type variables in {@code formal} mapping to
	 * types in {@code actual}.
	 *
	 * <p>
	 * For example, if {@code formal} is a {@code TypeVariable T}, and {@code actual} is
	 * {@code
	 * String.class}, then {@code new TypeResolver().where(formal, actual)} will
	 * {@linkplain #resolveType resolve} {@code ParameterizedType List<T>} to
	 * {@code List<String>}, and resolve {@code Map<T, Something>} to
	 * {@code Map<String, Something>} etc. Similarly, {@code formal} and {@code actual}
	 * can be {@code Map<K, V>} and {@code Map<String, Integer>} respectively, or they can
	 * be {@code E[]} and {@code String[]} respectively, or even any arbitrary combination
	 * thereof.
	 * @param formal The type whose type variables or itself is mapped to other type(s).
	 * It's almost always a bug if {@code formal} isn't a type variable and contains no
	 * type variable. Make sure you are passing the two parameters in the right order.
	 * @param actual The type that the formal type variable(s) are mapped to. It can be or
	 * contain yet other type variables, in which case these type variables will be
	 * further resolved if corresponding mappings exist in the current
	 * {@code TypeResolver} instance.
	 */
	public TypeResolver where(Type formal, Type actual) {
		Map<TypeVariableKey, Type> mappings = org.magneton.core.collect.Maps.newHashMap();
	...
	private static class TypeTable {
		...
		/**
		 * Returns a new {@code TypeResolver} with {@code variable} mapping to
		 * {@code type}.
		 */
		...
		/**
		 * Resolves {@code var} using the encapsulated type mapping. If it maps to yet
		 * another non-reified type or has bounds, {@code forDependants} is used to do
		 * further resolution, which doesn't try to resolve any type variable on generic
		 * declarations that are already being resolved.
		 *
		 * <p>
		 * Should only be called and overridden by {@link #resolve(TypeVariable)}.
		 */
		Type resolveInternal(TypeVariable<?> var, TypeTable forDependants) {
			...
			if (type == null) {
				 ...
				 * bounds. There's just one problem: Starting with JDK 7u51, the JDK
				 * TypeVariable's equals() method doesn't recognize instances of our
				 * TypeVariable implementation. This is a problem because users compare
				 * TypeVariables from the JDK against TypeVariables returned by
				 * TypeResolver. To work with all JDK versions, TypeResolver must return
				 * the appropriate TypeVariable implementation in each of the three
				 * possible cases:
				 *
				 * 1. Prior to JDK 7u51, the JDK TypeVariable implementation interoperates
				 * with ours. Therefore, we can always create our own TypeVariable.
				 *
				 * 2. Starting with JDK 7u51, the JDK TypeVariable implementations does
				 * not interoperate with ours. Therefore, we have to be careful about
				 * whether we create our own TypeVariable:
				 *
				 * 2a. If the resolved types are identical to the original types, then we
				 * can return the original, identical JDK TypeVariable. By doing so, we
				 * sidestep the problem entirely.
				 *
				 * 2b. If the resolved types are different from the original types, things
				 * are trickier. The only way to get a TypeVariable instance for the
				 * resolved types is to create our own. The created TypeVariable will not
				 * interoperate with any JDK TypeVariable. But this is OK: We don't _want_
				 * our new TypeVariable to be equal to the JDK TypeVariable because it has
				 * _different bounds_ than the JDK TypeVariable. And it wouldn't make
				 * sense for our new TypeVariable to be equal to any _other_ JDK
				 * TypeVariable, either, because any other JDK TypeVariable must have a
				 * different declaration or name. The only TypeVariable that our new
				 * TypeVariable _will_ be equal to is an equivalent TypeVariable that was
				 * also created by us. And that equality is guaranteed to hold because it
				 * doesn't involve the JDK TypeVariable implementation at all.
				 */
	...
	private static final class TypeMappingIntrospector extends TypeVisitor {
		...
		/**
		 * Returns type mappings using type parameters and type arguments found in the
		 * generic superclass and the super interfaces of {@code contextClass}.
		 */
	...
	// Instead, it should create a capture of the wildcard so that set() rejects any
	// List<T>.
	private static class WildcardCapturer {
		...
		final Type capture(Type type) {
			...
			if (type instanceof WildcardType) {
				...
				if (lowerBounds.length == 0) { // ? extends something changes to
												// capture-of
		...
		private WildcardCapturer forTypeVariable(TypeVariable<?> typeParam) {
			return new WildcardCapturer(id) {
				...
				TypeVariable<?> captureAsTypeVariable(Type[] upperBounds) {
					...
					// bother checking
					// subtyping between declared type bound and actual type bound. So
					...
					// There is no contract one way or another as long as isSubtypeOf()
					// works as expected.
					combined.addAll(asList(typeParam.getBounds()));
					if (combined.size() > 1) { // Object is implicit and only useful if
												// it's the only bound.
	...
	/**
	 * Wraps around {@code TypeVariable<?>} to ensure that any two type variables are
	 * equal as long as they are declared by the same
	 * {@link java.lang.reflect.GenericDeclaration} and have the same name, even if their
	 * bounds differ.
	 *
	 * <p>
	 * While resolving a type variable from a {@code var -> type} map, we don't care
	 * whether the type variable's bound has been partially resolved. As long as the type
	 * variable "identity" matches.
	 *
	 * <p>
	 * On the other hand, if for example we are resolving {@code List<A extends B>} to
	 * {@code
	 * List<A extends String>}, we need to compare that {@code <A extends B>} is unequal
	 * to {@code <A
	 * extends String>} in order to decide to use the transformed type instead of the
	 * original type.
	 */
	static final class TypeVariableKey {
		...
		/**
		 * Returns true if {@code type} is a {@code TypeVariable} with the same name and
		 * declared by the same {@code GenericDeclaration}.
		 */

 ...
 *
 * <p>
 * Operations that are otherwise only available in {@link Class} are implemented to
 * support {@code Type}, for example {@link #isSubtypeOf}, {@link #isArray} and
 * {@link #getComponentType}. It also provides additional utilities such as
 * {@link #getTypes}, {@link #resolveType}, etc.
 *
 * <p>
 ...
 * }</pre>
 * <li>Capture a generic type with a (usually anonymous) subclass and resolve it against a
 * context class that knows what the type parameters are. For example: <pre>{@code
 ...
 *
 * <p>
 * Note to Guice users: {@code} TypeToken is similar to Guice's {@code TypeLiteral} class
...
public abstract class TypeToken<T> extends org.magneton.core.reflect.TypeCapture<T> implements Serializable {

	// This happens to be the hash of the class as of now. So setting it makes a backward
	// compatible
	// change. Going forward, if any incompatible change is added, we can change the UID
	// back to 1.
	...
	/**
	 * Resolver for resolving parameter and field types with {@link #runtimeType} as
	 * context.
	 */
	@CheckForNull
	private transient org.magneton.core.reflect.TypeResolver invariantTypeResolver;

	/** Resolver for resolving covariant types with {@link #runtimeType} as context. */
	@CheckForNull
	private transient org.magneton.core.reflect.TypeResolver covariantTypeResolver;

	/**
	 * Constructs a new type token of {@code T}.
	 *
	 * <p>
	 * Clients create an empty anonymous subclass. Doing so embeds the type parameter in
	 * the anonymous class's type hierarchy so we can reconstitute it at runtime despite
	 * erasure.
	 *
	 * <p>
	 * For example:
	...
	/**
	 * Constructs a new type token of {@code T} while resolving free type variables in the
	 * context of {@code declaringClass}.
	 *
	 * <p>
	 * Clients create an empty anonymous subclass. Doing so embeds the type parameter in
	 * the anonymous class's type hierarchy so we can reconstitute it at runtime despite
	...
	protected TypeToken(Class<?> declaringClass) {
		...
		if (captured instanceof Class) {
			runtimeType = captured;
		}
		else {
			runtimeType = org.magneton.core.reflect.TypeResolver.covariantly(declaringClass).resolveType(captured);
		}
	}

	private TypeToken(Type type) {
		runtimeType = Preconditions.checkNotNull(type);
	}
	...
	/**
	 * In reflection, {@code Foo<?>.getUpperBounds()[0]} is always {@code Object.class},
	 * even when Foo is defined as {@code Foo<T extends String>}. Thus directly calling
	 * {@code <?>.is(String.class)} will return false. To mitigate, we canonicalize
	 * wildcards by enforcing the following invariants:
	 *
	 * <ol>
	 * <li>{@code canonicalize(t)} always produces the equal result for equivalent types.
	 * For example both {@code Enum<?>} and {@code Enum<? extends Enum<?>>} canonicalize
	 * to {@code
	 *       Enum<? extends Enum<E>}.
	 * <li>{@code canonicalize(t)} produces a "literal" supertype of t. For example:
	 * {@code Enum<?
	 *       extends Enum<?>>} canonicalizes to {@code Enum<?>}, which is a supertype (if
	 * we disregard the upper bound is implicitly an Enum too).
	 * <li>If {@code canonicalize(A) == canonicalize(B)}, then
	 * {@code Foo<A>.isSubtypeOf(Foo<B>)} and vice versa. i.e. {@code A.is(B)} and
	 * {@code B.is(A)}.
	...
	private static Type canonicalizeTypeArg(TypeVariable<?> declaration, Type typeArg) {
		return typeArg instanceof WildcardType ? canonicalizeWildcardType(declaration, ((WildcardType) typeArg))
	...
	private static Type canonicalizeWildcardsInType(Type type) {
		...
		if (type instanceof GenericArrayType) {
			return org.magneton.core.reflect.Types
					.newArrayType(canonicalizeWildcardsInType(((GenericArrayType) type).getGenericComponentType()));
		}
		return type;
	}

	// WARNING: the returned type may have empty upper bounds, which may violate common
	// expectations
	// by user code or even some of our own code. It's fine for the purpose of checking
	// subtypes.
	...
	private static WildcardType canonicalizeWildcardType(TypeVariable<?> declaration, WildcardType type) {
		...
		return new org.magneton.core.reflect.Types.WildcardTypeImpl(type.getLowerBounds(),
				upperBounds.toArray(new Type[0]));
	}
	...
	private static ParameterizedType canonicalizeWildcardsInParameterizedType(ParameterizedType type) {
		...
		return org.magneton.core.reflect.Types.newParameterizedTypeWithOwner(type.getOwnerType(), rawType, typeArgs);
	}
	 ...
	 * For example: {@code TypeToken.getGenericType(Iterable.class)} returns
	 * {@code Iterable<T>}.
	 *
	 * <p>
	 * If {@code cls} isn't parameterized and isn't a generic array, the type token of the
	 * class is returned.
	 */
	@VisibleForTesting
	static <T> TypeToken<? extends T> toGenericType(Class<T> cls) {
		if (cls.isArray()) {
			Type arrayOfGenericType = org.magneton.core.reflect.Types.newArrayType(
					// If we are passed with int[].class, don't turn it to
					// GenericArrayType
		...
		if ((typeParams.length > 0) || ((ownerType != null) && ownerType != cls.getEnclosingClass())) {
			...
			TypeToken<? extends T> type = (TypeToken<? extends T>) of(
					org.magneton.core.reflect.Types.newParameterizedTypeWithOwner(ownerType, cls, typeParams));
			return type;
		}
		else {
			return of(cls);
	...
	private static Type newArrayClassOrGenericArrayType(Type componentType) {
		return org.magneton.core.reflect.Types.JavaVersion.JAVA7.newArrayType(componentType);
	}
	 ...
	 * <li>If {@code T} is a {@link ParameterizedType}, the raw type of the parameterized
	 * type is returned.
	 * <li>If {@code T} is a {@link GenericArrayType}, the returned type is the
	 * corresponding array class. For example: {@code List<Integer>[] => List[]}.
	 * <li>If {@code T} is a type variable or a wildcard type, the raw type of the first
	 * upper bound is returned. For example: {@code <X extends Foo> => Foo}.
	 * </ul>
	 */
	...
	/**
	 * Returns a new {@code TypeToken} where type variables represented by
	 * {@code typeParam} are substituted by {@code typeArg}. For example, it can be used
	 * to construct {@code Map<K, V>} for any {@code K} and {@code V} type:
	 *
	 * <pre>{@code
	 ...
	 */
	/*
	 * TODO(cpovirk): Is there any way for us to support TypeParameter instances for type
	 * parameters that have nullable bounds? Unfortunately, if we change the parameter to
	 * TypeParameter<? extends
	 *
	 * @Nullable X>, then users might pass a TypeParameter<Y>, where Y is a subtype of X,
	 * while still passing a TypeToken<X>. This would be invalid. Maybe we could accept a
	 * TypeParameter<@PolyNull X> if we support such a thing? It would be weird or
	 * misleading for users to be able to pass `new TypeParameter<@Nullable T>() {}` and
	 * have it act as a plain `TypeParameter<T>`, but hopefully no one would do that,
	 * anyway. See also the comment on TypeParameter itself.
	 *
	 * TODO(cpovirk): Elaborate on this / merge with other comment?
	 */
	public final <X> TypeToken<T> where(org.magneton.core.reflect.TypeParameter<X> typeParam, TypeToken<X> typeArg) {
		org.magneton.core.reflect.TypeResolver resolver = new org.magneton.core.reflect.TypeResolver().where(
				ImmutableMap.of(new org.magneton.core.reflect.TypeResolver.TypeVariableKey(typeParam.typeVariable),
						typeArg.runtimeType));
	...
	/*
	 * TODO(cpovirk): Is there any way for us to support TypeParameter instances for type
	 * parameters that have nullable bounds? See discussion on the other overload of this
	 * method.
	 */
	...
	public final TypeToken<?> resolveType(Type type) {
		...
		// Being conservative here because the user could use resolveType() to resolve a
		// type in an
		// invariant context.
	 ...
	 * {@link Class#getGenericSuperclass}. For example, {@code new
	 * TypeToken<StringArrayList>() {}.getGenericSuperclass()} will return {@code new
	 * TypeToken<ArrayList<String>>() {}}; while
	 * {@code StringArrayList.class.getGenericSuperclass()} will return
	 * {@code ArrayList<E>}, where {@code E} is the type variable declared by class
	 * {@code ArrayList}.
	 *
	 * <p>
	 * If this type is a type variable or wildcard, its first upper bound is examined and
	 * returned if the bound is a class or extends from a class. This means that the
	 * returned type could be a type variable too.
	 */
	@CheckForNull
	...
	/**
	 * Returns the generic interfaces that this type directly {@code implements}. This
	 * method is similar but different from {@link Class#getGenericInterfaces()}. For
	 * example, {@code new TypeToken<List<String>>() {}.getGenericInterfaces()} will
	 * return a list that contains {@code new TypeToken<Iterable<String>>() {}}; while
	 * {@code List.class.getGenericInterfaces()} will return an array that contains
	 * {@code Iterable<T>}, where the {@code T} is the type variable declared by interface
	 * {@code Iterable}.
	 *
	 * <p>
	 * If this type is a type variable or wildcard, its upper bounds are examined and
	 * those that are either an interface or upper-bounded only by interfaces are
	 * returned. This means that the returned types could include type variables too.
	 */
	...
	/**
	 * Returns the set of interfaces and classes that this type is or is a subtype of. The
	 * returned types are parameterized with proper type arguments.
	 *
	 * <p>
	 * Subtypes are always listed before supertypes. But the reverse is not true. A type
	 * isn't necessarily a subtype of all the types following. Order between types without
	 * subtype relationship is arbitrary and not guaranteed.
	 *
	 * <p>
	 * If this type is a type variable or wildcard, upper bounds that are themselves type
	 * variables aren't included (their super interfaces and superclasses are).
	 */
	...
	public final TypeToken<? super T> getSupertype(Class<? super T> superclass) {
		Preconditions.checkArgument(someRawTypeIsSubclassOf(superclass), "%s is not a super class of %s", superclass,
				this);
		if (runtimeType instanceof TypeVariable) {
	...
	/**
	 * Returns subtype of {@code this} with {@code subclass} as the raw class. For
	 * example, if this is {@code Iterable<String>} and {@code subclass} is {@code List},
	 * {@code List<String>} is returned.
	 */
	public final TypeToken<? extends T> getSubtype(Class<?> subclass) {
		Preconditions.checkArgument(!(runtimeType instanceof TypeVariable), "Cannot get subtype of type variable <%s>",
				this);
		if (runtimeType instanceof WildcardType) {
		...
		Preconditions.checkArgument(getRawType().isAssignableFrom(subclass), "%s isn't a subclass of %s", subclass,
				this);
		Type resolvedTypeArgs = resolveTypeArgsForSubclass(subclass);
		// guarded by the isAssignableFrom() statement
		// above
		TypeToken<? extends T> subtype = (TypeToken<? extends T>) of(resolvedTypeArgs);
		Preconditions.checkArgument(subtype.isSubtypeOf(this), "%s does not appear to be a subtype of %s", subtype,
				this);
		return subtype;
	}

	/**
	 * Returns true if this type is a supertype of the given {@code type}. "Supertype" is
	 * defined according to
	 * <a href="http://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.5.1">the
	 * rules for type arguments</a> introduced with Java generics.
	 *
	 * @since 19.0
	 */
	...
	/**
	 * Returns true if this type is a subtype of the given {@code type}. "Subtype" is
	 * defined according to
	 * <a href="http://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.5.1">the
	 * rules for type arguments</a> introduced with Java generics.
	 *
	...
	/**
	 * Returns true if this type is known to be an array type, such as {@code int[]},
	 * {@code T[]}, {@code <? extends Map<String, Integer>[]>} etc.
	 */
	...
	/**
	 * Returns the corresponding wrapper type if this is a primitive type; otherwise
	 * returns {@code
	...
	public final TypeToken<T> wrap() {
		if (isPrimitive()) {
			...
			return of(org.magneton.core.primitives.Primitives.wrap(type));
		}
		return this;
	}

	private boolean isWrapper() {
		return org.magneton.core.primitives.Primitives.allWrapperTypes().contains(runtimeType);
	}

	/**
	 * Returns the corresponding primitive type if this is a wrapper type; otherwise
	 * returns {@code
	 * this} itself. Idempotent.
	 *
	 * @since 15.0
	 ...
	 * {@code T[]}, {@code <? extends Map<String, Integer>[]>} etc.), or else {@code null}
	 * is returned.
	 */
	@CheckForNull
	public final TypeToken<?> getComponentType() {
		Type componentType = org.magneton.core.reflect.Types.getComponentType(runtimeType);
	...
	/**
	 * Returns the {@link org.magneton.core.reflect.Invokable} for {@code method}, which
	 * must be a member of {@code T}.
	 *
	 * @since 14.0
	 */

	public final org.magneton.core.reflect.Invokable<T, Object> method(Method method) {
		Preconditions.checkArgument(someRawTypeIsSubclassOf(method.getDeclaringClass()), "%s not declared by %s",
				method, this);
		return new org.magneton.core.reflect.Invokable.MethodInvokable<T>(method) {
			@Override
	...
	/**
	 * Returns the {@link org.magneton.core.reflect.Invokable} for {@code constructor},
	 * which must be a member of {@code T}.
	 *
	 * @since 14.0
	 */

	public final org.magneton.core.reflect.Invokable<T, T> constructor(Constructor<?> constructor) {
		Preconditions.checkArgument(constructor.getDeclaringClass() == getRawType(), "%s not declared by %s",
	...
	/**
	 * Returns true if {@code o} is another {@code TypeToken} that represents the same
	 * {@link Type}.
	 */
	@Override
	...
	@Override
	public String toString() {
		return org.magneton.core.reflect.Types.toString(runtimeType);
	}
	...
	protected Object writeReplace() {
		...
		// except TypeVariable.
		return of(new org.magneton.core.reflect.TypeResolver().resolveType(runtimeType));
	}

	/**
	 * Ensures that this type token doesn't contain type variables, which can cause
	 * unchecked type errors for callers like {@link TypeToInstanceMap}.
	 */
	@CanIgnoreReturnValue
	final TypeToken<T> rejectTypeVariables() {
		new org.magneton.core.reflect.TypeVisitor() {
			@Override
	...
	private boolean isSubtypeOfParameterizedType(ParameterizedType supertype) {
		...
		for (int i = 0; i < typeVars.length; i++) {
			...
			// First step is to figure out StringArrayList "is-a" List<E> where <E> =
			// String.
			// String is then matched against <? extends CharSequence>, the
			// supertypeArgs[0].
		...
		// in which case we need to make sure the subclass's owner type is a subtype of
		// the
		// supertype's owner.
	 ...
	 * <li>{@code formalType} is {@code <? extends Foo>} and 'this' is a subtype of
	 * {@code Foo}.
	 * <li>{@code formalType} is {@code <? super Foo>} and 'this' is a supertype of
	 * {@code Foo}.
	 * </ol>
	 *
	 * Note that condition 2 isn't technically accurate under the context of a recursively
	 * bounded type variables. For example, {@code Enum<? extends Enum<E>>} canonicalizes
	 * to {@code Enum<?>} where {@code E} is the type variable declared on the
	 * {@code Enum} class declaration. It's technically <em>not</em> true that
	 * {@code Foo<Enum<? extends Enum<E>>>} is a subtype of {@code
	 * Foo<Enum<?>>} according to JLS. See testRecursiveWildcardSubtypeBug() for a real
	 * example.
	 *
	 * <p>
	 * It appears that properly handling recursive type bounds in the presence of implicit
	 * type bounds is not easy. For now we punt, hoping that this defect should rarely
	 * cause issues in real code.
	 * @param formalType is {@code Foo<formalType>} a supertype of {@code Foo<T>}?
	 * @param declaration The type variable in the context of a parameterized type. Used
	 * to infer type bound when {@code formalType} is a wildcard with implicit upper
	 * bound.
	 */
	...
	private ImmutableSet<Class<? super T>> getRawTypes() {
		...
		new org.magneton.core.reflect.TypeVisitor() {
			...
			void visitGenericArrayType(GenericArrayType t) {
				builder.add(
						org.magneton.core.reflect.Types.getArrayClass(of(t.getGenericComponentType()).getRawType()));
			}
		}.visit(runtimeType);
	...
	/**
	 * Returns the owner type of a {@link ParameterizedType} or enclosing class of a
	 * {@link Class}, or null otherwise.
	 */
	@CheckForNull
	...
	private org.magneton.core.reflect.TypeResolver getCovariantTypeResolver() {
		org.magneton.core.reflect.TypeResolver resolver = covariantTypeResolver;
		if (resolver == null) {
			resolver = (covariantTypeResolver = org.magneton.core.reflect.TypeResolver.covariantly(runtimeType));
		}
		return resolver;
	}

	private org.magneton.core.reflect.TypeResolver getInvariantTypeResolver() {
		org.magneton.core.reflect.TypeResolver resolver = invariantTypeResolver;
		if (resolver == null) {
			resolver = (invariantTypeResolver = org.magneton.core.reflect.TypeResolver.invariantly(runtimeType));
		}
		return resolver;
	}
	...
	private TypeToken<? super T> getArraySupertype(Class<? super T> supertype) {
		 ...
		 * supertype.isArray().
		 */
		TypeToken<?> componentSupertype = componentType.getSupertype(requireNonNull(supertype.getComponentType()));
	...
	private TypeToken<? extends T> getArraySubtype(Class<?> subclass) {
		...
		TypeToken<?> componentSubtype = requireNonNull(getComponentType()).getSubtype(subclassComponentType);
	...
	private Type resolveTypeArgsForSubclass(Class<?> subclass) {
		...
		// If runtimeType is not parameterized but subclass is, process subclass as a
		// parameterized type
		// If runtimeType is a raw type (i.e. is a parameterized type specified as a
		// Class<?>), we
	...
	private abstract static class TypeCollector<K> {
		...
		private static <K, V> ImmutableList<K> sortKeysByValue(Map<K, V> map, Comparator<? super V> valueComparator) {
			org.magneton.core.collect.Ordering<K> keyOrdering = new org.magneton.core.collect.Ordering<K>() {
				@Override
				public int compare(K left, K right) {
					// requireNonNull is safe because we are passing keys in the map.
					return valueComparator.compare(requireNonNull(map.get(left)), requireNonNull(map.get(right)));
				}
			};
		...
		ImmutableList<K> collectTypes(Iterable<? extends K> types) {
			// type -> order number. 1 for Object, 2 for anything directly below, so on so
			// forth.
		...
		private int collectTypes(K type, Map<? super K, Integer> map) {
			...
			/*
			 * TODO(benyu): should we include Object for interface? Also, CharSequence[]
			 * and Object[] for String[]?
			 *
			 */
	...
	/**
	 * The set of interfaces and classes that {@code T} is or is a subtype of.
	 * {@link Object} is not included in the set if this type is an interface.
	 *
	 * @since 13.0
	 */
	public class TypeSet extends ForwardingSet<TypeToken<? super T>> implements Serializable {
		...
		protected Set<TypeToken<? super T>> delegate() {
			...
			if (filteredTypes == null) {
				// Java has no way to express ? super T when we parameterize TypeToken vs.
				// Class.
	...
	private final class ClassSet extends TypeSet {
		...
		public Set<Class<? super T>> rawTypes() {
			// Java has no way to express ? super T when we parameterize TypeToken vs.
			// Class.
			ImmutableList<Class<? super T>> collectedTypes = (ImmutableList) TypeCollector.FOR_RAW_TYPE.classesOnly()
					.collectTypes(getRawTypes());
			return ImmutableSet.copyOf(collectedTypes);
		}

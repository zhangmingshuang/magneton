...
/**
 * Wrapper around either a {@link Method} or a {@link Constructor}. Convenience API is
 * provided to make common reflective operation easier to deal with, such as
 * {@link #isPublic}, {@link #getParameters} etc.
 *
 * <p>
 * In addition to convenience methods, {@link TypeToken#method} and
 * {@link TypeToken#constructor} will resolve the type parameters of the method or
 * constructor in the context of the owner type, which may be a subtype of the declaring
 * class. For example:
 *
 * <pre>{@code
 ...
 *
 * <p>
 * <b>Note:</b> earlier versions of this class inherited from {@link AccessibleObject
 * AccessibleObject} and {@link java.lang.reflect.GenericDeclaration GenericDeclaration}.
 * Since version 31.0 that is no longer the case. However, most methods from those types
 * are present with the same signature in this class.
 *
 * @param <T> the type that owns this method or constructor.
 * @param <R> the return type of (or supertype thereof) the method or the declaring type
 * of the constructor.
 * @author Ben Yu
 * @since 14.0 (no longer implements {@link AccessibleObject} or
 * {@code GenericDeclaration} since 31.0)
 */
...
public abstract class Invokable<T, R> implements AnnotatedElement, Member {
	...
	// We ought to be able to implement GenericDeclaration instead its parent
	// AnnotatedElement.
	...
	public final boolean trySetAccessible() {
		// We can't call accessibleObject.trySetAccessible since that was added in Java 9
		// and this code
	...
	/**
	 * Returns {@code true} if this method is final, per
	 * {@code Modifier.isFinal(getModifiers())}.
	 *
	 * <p>
	 * Note that a method may still be effectively "final", or non-overridable when it has
	 * no {@code final} keyword. For example, it could be private, or it could be declared
	 * by a final class. To tell whether a method is overridable, use
	...
	/**
	 * Returns {@code true} if this is an overridable method. Constructors, private,
	 * static or final methods, or methods declared by final classes are not overridable.
	 */
	...
	/**
	 * Invokes with {@code receiver} as 'this' and {@code args} passed to the underlying
	 * method and returns the return value; or calls the underlying constructor with
	 * {@code args} and returns the constructed instance.
	 * @throws IllegalAccessException if this {@code Constructor} object enforces Java
	 * language access control and the underlying method or constructor is inaccessible.
	 ...
	 * @throws InvocationTargetException if the underlying method or constructor throws an
	 * exception.
	 */
	...
	/**
	 * Returns all declared parameters of this {@code Invokable}. Note that if this is a
	 * constructor of a non-static inner class, unlike
	 * {@link Constructor#getParameterTypes}, the hidden {@code
	...
	// The declaring class is T's raw class, or one of its
	// supertypes.
	@Override
	...
	static class ConstructorInvokable<T> extends Invokable<T, T> {
		...
		/**
		 * If the class is parameterized, such as {@link java.util.ArrayList ArrayList},
		 * this returns {@code ArrayList<E>}.
		 */
		@Override
		Type getGenericReturnType() {
		 ...
		 *
		 * <p>
		 * {@code [<E>]} will be returned for ArrayList's constructor. When both the class
		 * and the constructor have type parameters, the class parameters are prepended
		 * before those of the constructor's. This is an arbitrary rule since no existing
		 * language spec mandates one way or the other. From the declaration syntax, the
		 * class type parameter appears first, but the call syntax may show up in opposite
		 * order such as {@code new <A>Foo<B>()}.
		 */
		@Override
		public final TypeVariable<?>[] getTypeParameters() {
		...
		private boolean mayNeedHiddenThis() {
			...
			if (enclosingMethod != null) {
				...
				// Strictly, this doesn't necessarily indicate a hidden 'this' in the case
				// of
				// static initializer. But there seems no way to tell in that case. :(
				// This may cause issues when an anonymous class is created inside a
				// static initializer,
				// and the class's constructor's first parameter happens to be the
				// enclosing class.
				// In such case, we may mistakenly think that the class is within a
				// non-static context

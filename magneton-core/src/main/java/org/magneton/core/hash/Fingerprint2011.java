 ...
 * {@link Hashing#fingerprint2011} for information on the behaviour of the algorithm.
 *
 * <p>
...
final class Fingerprint2011 extends AbstractNonStreamingHashFunction {
	...
	static long fingerprint(byte[] bytes, int offset, int length) {
		...
		long u = length >= 8 ? LittleEndianByteArray.load64(bytes, offset) : K0;
		long v = length >= 9 ? LittleEndianByteArray.load64(bytes, offset + length - 8) : K0;
	 ...
	 * are returned in the output array - this is 12% faster than allocating new arrays
	 * every time.
	 */
	...
	private static long hashLength33To64(byte[] bytes, int offset, int length) {
		...
		long a = LittleEndianByteArray.load64(bytes, offset)

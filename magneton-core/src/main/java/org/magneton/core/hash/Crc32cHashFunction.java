 ...
 * generator polynomial for this checksum is {@code 0x11EDC6F41}.
 *
...
final class Crc32cHashFunction extends AbstractHashFunction {
	...
	static final class Crc32cHasher extends AbstractStreamingHasher {
		...
		/*
		 * The striding algorithm works roughly as follows: it is universally the case
		 * that CRC(x ^ y) == CRC(x) ^ CRC(y). The approach we take is to break the
		 * message as follows, with each letter representing a 4-byte word:
		 * ABCDABCDABCDABCD... and to calculate CRC(A000A000A000...), CRC(0B000B000B...),
		 * CRC(00C000C000C...), CRC(000D000D000D...) and then to XOR them together. The
		 * STRIDE_TABLE enables us to hash an int followed by 12 zero bytes (3 ints),
		 * while the BYTE_TABLE is for advancing one byte at a time. This algorithm is due
		 * to the paper "Everything we know about CRC but [are] afraid to forget" by
		 * Kadatch and Jenkins, 2010.
		 */

		static final int[] BYTE_TABLE = { 0x00000000, 0xf26b8303, 0xe13b70f7, 0x1350f3f4, 0xc79a971f, 0x35f1141c,
		...
		static final int[][] STRIDE_TABLE = {
				{ 0x00000000, 0x30d23865, 0x61a470ca, 0x517648af, 0xc348e194, 0xf39ad9f1, 0xa2ec915e, 0x923ea93b,
						...
						0x92a00d08, 0xa272356d, 0xf3047dc2, 0xc3d645a7, 0x51e8ec9c, 0x613ad4f9, 0x304c9c56,
						0x009ea433, },
				{ 0x00000000, 0x54075546, 0xa80eaa8c, 0xfc09ffca, 0x55f123e9, 0x01f676af, 0xfdff8965, 0xa9f8dc23,
						...
						0x8a4bf8c2, 0xde4cad84, 0x2245524e, 0x76420708, 0xdfbadb2b, 0x8bbd8e6d, 0x77b471a7,
						0x23b324e1, },
				{ 0x00000000, 0x678efd01, 0xcf1dfa02, 0xa8930703, 0x9bd782f5, 0xfc597ff4, 0x54ca78f7, 0x334485f6,
						...
						0x223c1dcb, 0x45b2e0ca, 0xed21e7c9, 0x8aaf1ac8, 0xb9eb9f3e, 0xde65623f, 0x76f6653c,
						0x1178983d, },
				{ 0x00000000, 0xf20c0dfe, 0xe1f46d0d, 0x13f860f3, 0xc604aceb, 0x3408a115, 0x27f0c1e6, 0xd5fccc18,
						...
						0x5a26b1e2, 0xa82abc1c, 0xbbd2dcef, 0x49ded111, 0x9c221d09, 0x6e2e10f7, 0x7dd67004,
						0x8fda7dfa, }, };
		...
		/*
		 * This trick allows us to avoid having separate states for "first four ints" and
		 * "all other four int chunks." The state we want after the first four bytes is
		 *
		 * crc0 = ~int0 crc1 = int1 crc2 = int2 crc3 = int3
		 *
		 * ...so we set crc0 so that computeForWord(crc0) = -1 and xoring it with the
		 * first int gives us the desired result. computeForWord(0) == 0, so all the
		...
		static int computeForWord(int word) {
			return STRIDE_TABLE[3][word & 0xFF] ^ STRIDE_TABLE[2][(word >>> 8) & 0xFF]
					^ STRIDE_TABLE[1][(word >>> 16) & 0xFF] ^ STRIDE_TABLE[0][word >>> 24];
		}
		...
		protected org.magneton.core.hash.HashCode makeHash() {
			if (!finished) {
				// processRemaining does teardown we always want to do -- the folding
				// together of the four
				// rolling CRCs. So we call it on an empty ByteBuffer if we didn't
				// already.

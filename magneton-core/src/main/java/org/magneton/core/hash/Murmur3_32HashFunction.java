...
final class Murmur3_32HashFunction extends org.magneton.core.hash.AbstractHashFunction implements Serializable {

	static final org.magneton.core.hash.HashFunction MURMUR3_32 = new Murmur3_32HashFunction(0,
			/* supplementaryPlaneFix= */ false);
	static final org.magneton.core.hash.HashFunction MURMUR3_32_FIXED = new Murmur3_32HashFunction(0,
			/* supplementaryPlaneFix= */ true);

	// We can include the non-BMP fix here because Hashing.goodFastHash stresses that the
	// hash is a
	...
	private static org.magneton.core.hash.HashCode fmix(int h1, int length) {
		h1 ^= length;
		h1 ^= h1 >>> 16;
		h1 *= 0x85ebca6b;
		h1 ^= h1 >>> 13;
		h1 *= 0xc2b2ae35;
		h1 ^= h1 >>> 16;
		return org.magneton.core.hash.HashCode.fromInt(h1);
	}

	private static long codePointToFourUtf8Bytes(int codePoint) {
		// codePoint has at most 21 bits
		return ((0xFL << 4) | (codePoint >>> 18)) | ((0x80L | (0x3F & (codePoint >>> 12))) << 8)
				| ((0x80L | (0x3F & (codePoint >>> 6))) << 16) | ((0x80L | (0x3F & codePoint)) << 24);
	}

	private static long charToThreeUtf8Bytes(char c) {
		return ((0x7L << 5) | (c >>> 12)) | ((0x80 | (0x3F & (c >>> 6))) << 8) | ((0x80 | (0x3F & c)) << 16);
	}
		...
		if (Charsets.UTF_8.equals(charset)) {
			...
			for (; i < utf16Length; i++) {
				...
				if (c < 0x80) {
					...
					if (supplementaryPlaneFix) { // bug compatibility: earlier versions
													// did not have this add
						shift += 32;
					}
					len += 4;
				}
	...
	private static final class Murmur3_32Hasher extends org.magneton.core.hash.AbstractHasher {
		...
		@Override
		public org.magneton.core.hash.Hasher putBytes(ByteBuffer buffer) {
			...
			if (Charsets.UTF_8.equals(charset)) {
				...
				for (; i < utf16Length; i++) {
					...
					if (c < 0x80) {
						...
						int codePoint = Character.codePointAt(input, i);
						if (codePoint == c) {
							// fall back to JDK getBytes instead of trying to handle
							// invalid surrogates ourselves

...
final class LittleEndianByteArray {
	...
	/**
	 * The instance that actually does the work; delegates to Unsafe or a pure-Java
	 * fallback.
	 */
	...
	/**
	 * Load 8 bytes into long in a little endian manner, from the substring between
	 * position and position + 8. The array must have at least 8 bytes from offset
	 * (inclusive).
	...
	static long load64(byte[] input, int offset) {
		// We don't want this in production code as this is the most critical part of the
		// loop.
	...
	/**
	 * Similar to load64, but allows offset + 8 > input.length, padding the result with
	 * zeroes. This has to explicitly reverse the order of the bytes as it packs them into
	 * the result which makes it slower than the native version.
	 * @param input the input bytes
	...
	static long load64Safely(byte[] input, int offset, int length) {
		long result = 0;
		// Due to the way we shift, we can stop iterating once we've run out of data, the
		// rest
		// of the result already being filled with zeros.

		// This loop is critical to performance, so please check HashBenchmark if altering
		// it.
	...
	/**
	 * Store 8 bytes into the provided array at the indicated offset, using the value
	 * provided.
	...
	/**
	 * Indicates that the loading of Unsafe was successful and the load and store
	 * operations will be very efficient. May be useful for calling code to fall back on
	 * an alternative implementation that is slower than Unsafe.get/store but faster than
	 * the pure-Java mask-and-shift.
	 */
	 ...
	 * Unsafe.theUnsafe is inaccessible, the attempt to load the nested class fails, and
	 * the outer class's static initializer can fall back on a non-Unsafe version.
	 */
	private enum UnsafeByteArray implements LittleEndianBytes {
		...
		UNSAFE_BIG_ENDIAN {
			...
			public long getLongLittleEndian(byte[] array, int offset) {
				...
				// The hardware is big-endian, so we need to reverse the order of the
				// bytes.
			...
			@Override
			public void putLongLittleEndian(byte[] array, int offset, long value) {
				// Reverse the order of the bytes before storing, since we're on
				// big-endian hardware.
		...
		// Provides load and store operations that use native instructions to get better
		// performance.
	...
	/**
	 * Fallback implementation for when Unsafe is not available in our current
	 * environment.
	 */
	private enum JavaLittleEndianBytes implements LittleEndianBytes {
		...
		INSTANCE {
			...
			public long getLongLittleEndian(byte[] source, int offset) {
				return Longs.fromBytes(source[offset + 7], source[offset + 6], source[offset + 5], source[offset + 4],
						source[offset + 3], source[offset + 2], source[offset + 1], source[offset]);
			}
	 ...
	 *
	 * <p>
	 * This abstraction allows us to use single-instruction load and put when available,
	 * or fall back on the slower approach of using Longs.fromBytes(byte...).
	 */
	...
	static {
		...
		try {
			/*
			 * UnsafeByteArray uses Unsafe.getLong() in an unsupported way, which is known
			 * to cause crashes on Android when running in 32-bit mode. For maximum
			 * safety, we shouldn't use Unsafe.getLong() at all, but the performance
			 * benefit on x86_64 is too great to ignore, so as a compromise, we enable the
			 ...
			 * ByteBuffer.getLong(), which will have an efficient native implementation in
			 * JDK 9.
			 *
			 */
			...
			if ("amd64".equals(arch)) {
				theGetter = ByteOrder.nativeOrder().equals(ByteOrder.LITTLE_ENDIAN)
						? UnsafeByteArray.UNSAFE_LITTLE_ENDIAN : UnsafeByteArray.UNSAFE_BIG_ENDIAN;
			}
		}
		catch (Throwable t) {

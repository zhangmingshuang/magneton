...
/**
 * Implementation of FarmHash Fingerprint64, an open-source fingerprinting algorithm for
 * strings.
 *
 * <p>
 ...
 *
 * <p>
 * Note to maintainers: This implementation relies on signed arithmetic being bit-wise
 * equivalent to unsigned arithmetic in all cases except:
 *
 * <ul>
...
final class FarmHashFingerprint64 extends org.magneton.core.hash.AbstractNonStreamingHashFunction {
	...
	/**
	 * Computes intermediate hash of 32 bytes of byte array from the given offset. Results
	 * are returned in the output array because when we last measured, this was 12% faster
	 * than allocating new arrays every time.
	 */
	...
	private static long hashLength0to16(byte[] bytes, int offset, int length) {
		...
		if (length >= 4) {
			...
			return hashLength16(length + (a << 3),
					LittleEndianByteArray.load32(bytes, offset + length - 4) & 0xFFFFFFFFL, mul);
		}
		if (length > 0) {
			byte a = bytes[offset];
	...
	private static long hashLength65Plus(byte[] bytes, int offset, int length) {
		int seed = 81;
		// For strings over 64 bytes we loop. Internal state consists of 56 bytes: v, w,
		// x, y, and z.
		long x = seed;
		long y = seed * K1 + 113;
		...
		long[] v = new long[2];
		long[] w = new long[2];
		x = x * K2 + LittleEndianByteArray.load64(bytes, offset);

		// Set end so that after the loop we have 1 to 64 bytes left to process.
		int end = offset + ((length - 1) / 64) * 64;
		int last64offset = end + ((length - 1) & 63) - 63;
		do {
			x = rotateRight(x + y + v[0] + LittleEndianByteArray.load64(bytes, offset + 8), 37) * K1;
			...
			weakHashLength32WithSeeds(bytes, offset + 32, z + w[1],
					y + LittleEndianByteArray.load64(bytes, offset + 16), w);
			long tmp = x;
			x = z;
			z = tmp;
			offset += 64;
		...
		v[0] += w[0];
		w[0] += v[0];
		x = rotateRight(x + y + v[0] + LittleEndianByteArray.load64(bytes, offset + 8), 37) * mul;
		y = rotateRight(y + v[1] + LittleEndianByteArray.load64(bytes, offset + 48), 42) * mul;
		x ^= w[1] * 9;
		y += v[0] * 9 + LittleEndianByteArray.load64(bytes, offset + 40);
		z = rotateRight(z + w[0], 33) * mul;
		weakHashLength32WithSeeds(bytes, offset, v[1] * mul, x + w[0], v);
		weakHashLength32WithSeeds(bytes, offset + 32, z + w[1], y + LittleEndianByteArray.load64(bytes, offset + 16),
				w);
		return hashLength16(hashLength16(v[0], w[0], mul) + shiftMix(y) * K0 + x, hashLength16(v[1], w[1], mul) + z,
				mul);
	}

 ...
 * @since 3.0
 */
public class UCharSequence {

	static final int TO_STRING_LIMIT = 16;

	private static final int NOT_FOUND = -1;

	private UCharSequence() {
	...
	/**
	 * <p>
	 * Returns a new {@code CharSequence} that is a subsequence of this sequence starting
	 * with the {@code char} value at the specified index.
	 * </p>
	 *
	 * <p>
	 ...
	 * @throws IndexOutOfBoundsException if {@code start} is negative or if {@code start}
	 * is greater than {@code length()}
	 */
	public static CharSequence subSequence(@Nullable CharSequence cs, int start) {
	 ...
	 * represented by the {@code cs} object at an index no smaller than {@code start},
	 * then the index of the first such occurrence is returned. For values of
	 * {@code searchChar} in the range from 0 to 0xFFFF (inclusive), this is the smallest
	 * value <i>k</i> such that: <blockquote><pre>
	 * (this.charAt(<i>k</i>) == searchChar) &amp;&amp; (<i>k</i> &gt;= start)
	 * </pre></blockquote> is true. For other values of {@code searchChar}, it is the
	 * smallest value <i>k</i> such that: <blockquote><pre>
	 * (this.codePointAt(<i>k</i>) == searchChar) &amp;&amp; (<i>k</i> &gt;= start)
	 * </pre></blockquote> is true. In either case, if no such character occurs inm
	 * {@code cs} at or after position {@code start}, then {@code -1} is returned.
	 *
	 * <p>
	 * There is no restriction on the value of {@code start}. If it is negative, it has
	 * the same effect as if it were zero: the entire {@code CharSequence} may be
	 * searched. If it is greater than the length of {@code cs}, it has the same effect as
	 * if it were equal to the length of {@code cs}: {@code -1} is returned.
	 *
	 * <p>
	 ...
	 * {@code searchChar} in the range from 0 to 0xFFFF (inclusive), the index returned is
	 * the largest value <i>k</i> such that: <blockquote><pre>
	 * (this.charAt(<i>k</i>) == searchChar) &amp;&amp; (<i>k</i> &lt;= start)
	 * </pre></blockquote> is true. For other values of {@code searchChar}, it is the
	 * largest value <i>k</i> such that: <blockquote><pre>
	 * (this.codePointAt(<i>k</i>) == searchChar) &amp;&amp; (<i>k</i> &lt;= start)
	 * </pre></blockquote> is true. In either case, if no such character occurs in
	...
	public static char[] toCharArray(@Nullable CharSequence source) {
		final int len = UString.length(source);
		if (len == 0) {
			return CharPool.EMPTY_CHAR_ARRAY;
		}

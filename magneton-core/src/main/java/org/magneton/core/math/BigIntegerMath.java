...
public final class BigIntegerMath {
	...
	/*
	 * The maximum number of bits in a square root for which we'll precompute an explicit
	 * half power of two. This can be any value, but higher values incur more class load
	 * time and linearly increasing memory consumption.
	 */
	@VisibleForTesting
	 ...
	 * equivalent to {@code BigInteger.valueOf(2).pow(log2(x, CEILING))}.
	...
	public static int log2(BigInteger x, RoundingMode mode) {
		MathPreconditions.checkPositive("x", checkNotNull(x));
	...
	public static int log10(BigInteger x, RoundingMode mode) {
		...
		if (approxCmp > 0) {
			 ...
			 * still yield the correct answer eventually, but in practice this branch
			 * should almost never be entered, and even then the loop should not run more
			 * than once.
			 */
			do {
				approxLog10--;
	...
	public static BigInteger sqrt(BigInteger x, RoundingMode mode) {
		...
		case UP:
			...
			boolean sqrtFloorIsExact = (sqrtFloorInt * sqrtFloorInt == x.intValue()) // fast
																						// check
																						// mod
																						// 2^32
	...
	private static BigInteger sqrtFloor(BigInteger x) {
		 ...
		 *
		 * Using DoubleUtils.bigToDouble, getting a double approximation of x is extremely
		 * fast, and then we can get a double approximation of the square root. Then, we
		 * iteratively improve this guess with an application of Newton's method, which
		 * sets guess := (guess + (x / guess)) / 2. This iteration has the following two
		 * properties:
		 *
		 * a) every iteration (except potentially the first) has guess >= floor(sqrt(x)).
		 * This is because guess' is the arithmetic mean of guess and x / guess, sqrt(x)
		 * is the geometric mean, and the arithmetic mean is always higher than the
		 * geometric mean.
		 *
		 ...
		 * lower than the true value. Therefore, we perform at least one Newton iteration
		 * to get a guess that's definitely >= floor(sqrt(x)), and then continue the
	 ...
	 *
	 * <p>
	 * For the case of {@link RoundingMode#HALF_DOWN}, {@code HALF_UP}, and
	 * {@code HALF_EVEN}, infinite {@code double} values are considered infinitely far
	 * away. For example, 2^2000 is not representable as a double, but
	 * {@code roundToDouble(BigInteger.valueOf(2).pow(2000), HALF_UP)} will return
	 * {@code Double.MAX_VALUE}, not {@code Double.POSITIVE_INFINITY}.
	 *
	 * <p>
	 ...
	 * or {@code 1} if {@code n == 0}.
	 *
	 * <p>
	 * <b>Warning:</b> the result takes <i>O(n log n)</i> space, so use cautiously.
	 *
	 * <p>
	 * This uses an efficient binary recursive algorithm to compute the factorial with
	 * balanced multiplies. It also removes all the 2s from the intermediate products
	 * (shifting them back in at the end).
	...
	public static BigInteger binomial(int n, int k) {
		...
		for (int i = 1; i < k; i++) {
			...
			if (numeratorBits + bits >= Long.SIZE - 1) {
				...
				// We can definitely multiply into the long accumulators without
				// overflowing them.
				numeratorAccum *= p;
				denominatorAccum *= q;
				numeratorBits += bits;

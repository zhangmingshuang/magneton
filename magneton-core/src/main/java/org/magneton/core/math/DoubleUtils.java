...
final class DoubleUtils {
	...
	/*
	 * Returns x scaled by a power of 2 such that it is in the range [1, 2). Assumes x is
	 * positive, normal, and finite.
	 */
	...
	static double bigToDouble(BigInteger x) {
		// This is an extremely fast implementation of BigInteger.doubleValue(). JDK patch
		// pending.
		...
		/*
		 * We need the top SIGNIFICAND_BITS + 1 bits, including the "implicit" one bit. To
		 * make rounding easier, we pick out the top SIGNIFICAND_BITS + 2 bits, so we have
		 * one to help us round up or down. twiceSignifFloor will contain the top
		 ...
		 * and add 1 to the exponent. This is exactly the behavior we get from just adding
		 * signifRounded to bits directly. If the exponent is MAX_DOUBLE_EXPONENT, we

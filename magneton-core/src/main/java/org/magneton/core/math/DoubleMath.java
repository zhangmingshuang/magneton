...
import static org.magneton.core.math.MathPreconditions.checkInRangeForRoundingInputs;
...
public final class DoubleMath {
	...
	/*
	 * We cannot store Long.MAX_VALUE as a double without losing precision. Instead, we
	 * store Long.MAX_VALUE + 1 == -Long.MIN_VALUE, and then offset all comparisons by 1.
	 */
	...
	/*
	 * This method returns a value y such that rounding y DOWN (towards zero) gives the
	 * same result as rounding x according to the specified mode.
	 */
	...
	/**
	 * Returns the {@code BigInteger} value that is equal to {@code x} rounded with the
	 * specified rounding mode, if possible.
	 * @throws ArithmeticException if
	 * <ul>
	 * <li>{@code x} is infinite or NaN
	...
	/**
	 * Returns the base 2 logarithm of a double value, rounded with the specified rounding
	 * mode to an {@code int}.
	 *
	 * <p>
	...
	public static int log2(double x, RoundingMode mode) {
		checkArgument(x > 0.0 && DoubleUtils.isFinite(x), "x must be positive and finite");
		int exponent = getExponent(x);
	...
	public static boolean isMathematicalInteger(double x) {
		return DoubleUtils.isFinite(x) && (x == 0.0 || DoubleUtils.SIGNIFICAND_BITS
				- Long.numberOfTrailingZeros(DoubleUtils.getSignificand(x)) <= getExponent(x));
	}

	/**
	 * Returns {@code n!}, that is, the product of the first {@code n} positive integers,
	 * {@code 1} if {@code n == 0}, or {@code n!}, or {@link Double#POSITIVE_INFINITY} if
	 * {@code n! >
	 * Double.MAX_VALUE}.
	 *
	 * <p>
	...
	public static double factorial(int n) {
		...
		if (n > MAX_FACTORIAL) {
			...
			// Multiplying the last (n & 0xf) values into their own accumulator gives a
			// more accurate
	...
	/**
	 * Returns {@code true} if {@code a} and {@code b} are within {@code tolerance} of
	 * each other.
	 *
	 * <p>
	 ...
	 * <li>With {@link Double#POSITIVE_INFINITY} tolerance, all non-NaN values are fuzzily
	 * equal.
	 ...
	 *
	 * <p>
	 * This is reflexive and symmetric, but <em>not</em> transitive, so it is <em>not</em>
	 * an equivalence relation and <em>not</em> suitable for use in {@link Object#equals}
	 * implementations.
	 * @throws IllegalArgumentException if {@code tolerance} is {@code < 0} or NaN
	 * @since 13.0
	 */
	public static boolean fuzzyEquals(double a, double b, double tolerance) {
		org.magneton.core.math.MathPreconditions.checkNonNegative("tolerance", tolerance);
		return Math.copySign(a - b, 1.0) <= tolerance
				// copySign(x, 1.0) is a branch-free version of abs(x), but with different
				// NaN semantics
	 ...
	 * {@code fuzzyEquals(a, b, tolerance) ? 0 : Double.compare(a,
	 * b)}. In particular, like {@link Double#compare(double, double)}, it treats all NaN
	 ...
	 * @throws IllegalArgumentException if {@code values} is empty or contains any
	 * non-finite value
	 * @deprecated Use {@link org.magneton.core.math.Stats#meanOf} instead, noting the
	 * less strict handling of non-finite values.
	 */
	@Deprecated
	...
	public static double mean(int... values) {
		checkArgument(values.length > 0, "Cannot take mean of 0 values");
		// The upper bound on the length of an array and the bounds on the int values mean
		// that, in
		// this case only, we can compute the sum as a long without risking overflow or
		// loss of
	 ...
	 * If these values are a sample drawn from a population, this is also an unbiased
	 * estimator of the arithmetic mean of the population.
	 * @param values a nonempty series of values, which will be converted to
	 * {@code double} values (this may cause loss of precision for longs of magnitude over
	 * 2^53 (slightly over 9e15))
	 * @throws IllegalArgumentException if {@code values} is empty
	 * @deprecated Use {@link org.magneton.core.math.Stats#meanOf} instead, noting the
	 * less strict handling of non-finite values.
	 */
	 ...
	 * estimator of the arithmetic mean of the population.
	 * @param values a nonempty series of values, which will be converted to
	 * {@code double} values (this may cause loss of precision)
	 * @throws IllegalArgumentException if {@code values} is empty or contains any
	 * non-finite value
	 * @deprecated Use {@link org.magneton.core.math.Stats#meanOf} instead, noting the
	 * less strict handling of non-finite values.
	 ...
	 * @throws IllegalArgumentException if {@code values} is empty or contains any
	 * non-finite value
	 * @deprecated Use {@link Stats#meanOf} instead, noting the less strict handling of
	 * non-finite values.
	 */
	@Deprecated
	// com.google.common.math.DoubleUtils
	...
	private static double checkFinite(double argument) {
		checkArgument(DoubleUtils.isFinite(argument));
		return argument;
	}

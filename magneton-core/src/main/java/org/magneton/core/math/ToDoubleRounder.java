...
/**
 * Helper type to implement rounding {@code X} to a representable {@code double} value
 * according to a {@link RoundingMode}.
 */
...
abstract class ToDoubleRounder<X extends Number & Comparable<X>> {
	...
	/**
	 * Returns x rounded to either the greatest double less than or equal to the precise
	 * value of x, or the least double greater than or equal to the precise value of x.
	 */
	...
	final double roundToDouble(X x, RoundingMode mode) {
		...
		case HALF_EVEN: {
			...
			case HALF_EVEN:
				...
				return ((Double.doubleToRawLongBits(roundFloorAsDouble) & 1L) == 0) ? roundFloorAsDouble

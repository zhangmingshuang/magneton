 ...
 * {@link org.magneton.core.util.concurrent.Futures#getChecked(Future, Class)}.
 */

@ElementTypesAreNonnullByDefault
final class FuturesGetChecked {

	private static final org.magneton.core.collect.Ordering<Constructor<?>> WITH_STRING_PARAM_FIRST = Ordering.natural()
	...
	@ParametricNullness
	static <V extends Object, X extends Exception> V getChecked(Future<V> future, Class<X> exceptionClass) throws X {
	...
	@VisibleForTesting
	@ParametricNullness
	static <V extends Object, X extends Exception> V getChecked(GetCheckedTypeValidator validator, Future<V> future,
	...
	@CanIgnoreReturnValue
	@ParametricNullness
	static <V extends Object, X extends Exception> V getChecked(Future<V> future, Class<X> exceptionClass, long timeout,
			TimeUnit unit) throws X {
		// TODO(cpovirk): benchmark a version of this method that accepts a
	...
	/*
	 * TODO(user): FutureChecker interface for these to be static methods on? If so, refer
	 * to it in the (static-method) Futures.getChecked documentation
	 */
	 ...
	 *
	 * <p>
	 * Uses reflection to gracefully fall back to when certain implementations aren't
	 * available.
	 */
	@VisibleForTesting
	static class GetCheckedTypeValidatorHolder {

		static final String CLASS_VALUE_VALIDATOR_NAME = GetCheckedTypeValidatorHolder.class.getName()
				+ "$ClassValueValidator";

		static final GetCheckedTypeValidator BEST_VALIDATOR = getBestValidator();

		/**
		 * Returns the ClassValue-using validator, or falls back to the "weak Set"
		...
		enum ClassValueValidator implements GetCheckedTypeValidator {
			...
			/*
			 * Static final fields are presumed to be fastest, based on our experience
			 * with UnsignedBytesBenchmark. TODO(cpovirk): benchmark this
			 */
			...
			public void validateClass(Class<? extends Exception> exceptionClass) {
				isValidClass.get(exceptionClass); // throws if invalid; returns safely
													// (and caches) if valid
			}
		...
		enum WeakSetValidator implements GetCheckedTypeValidator {
			 ...
			 * MapMaker map with weakKeys() and concurrencyLevel(1), even up to at least
			 * 12 cached exception types.
			 */
			...
			public void validateClass(Class<? extends Exception> exceptionClass) {
				...
				/*
				 * It's very unlikely that any loaded Futures class will see getChecked
				 * called with more than a handful of exceptions. But it seems prudent to
				 * set a cap on how many we'll cache. This avoids out-of-control memory
				 * consumption, and it keeps the cache from growing so large that doing
				 * the lookup is noticeably slower than redoing the work would be.
				 *
				 * Ideally we'd have a real eviction policy, but until we see a problem in
				 * practice, I hope that this will suffice. I have not even benchmarked
				 * with different size limits.
				 */

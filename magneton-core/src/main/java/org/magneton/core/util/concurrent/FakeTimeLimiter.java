...
/**
 * A TimeLimiter implementation which actually does not attempt to limit time at all. This
 * may be desirable to use in some unit tests. More importantly, attempting to debug a
 * call which is time-limited would be extremely annoying, so this gives you a
 * time-limiter you can easily swap in for your real time-limiter while you're debugging.
 *
...
public final class FakeTimeLimiter implements TimeLimiter {
	...
	public <T> T callWithTimeout(Callable<T> callable, long timeoutDuration, TimeUnit timeoutUnit)
		...
		try {
			...
			throw new org.magneton.core.util.concurrent.ExecutionError(e);
		}
		catch (Throwable e) {
			// It's a non-Error, non-Exception Throwable. Such classes are usually
			// intended to extend

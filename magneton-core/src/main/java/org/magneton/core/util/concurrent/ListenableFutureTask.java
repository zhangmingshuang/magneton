...
/**
 * A {@link FutureTask} that also implements the
 * {@link org.magneton.core.util.concurrent.ListenableFuture} interface. Unlike {@code
 * FutureTask}, {@code ListenableFutureTask} does not provide an overrideable
 * {@link FutureTask#done() done()} method. For similar functionality, call
 * {@link #addListener}.
 *
 * <p>
 * Few users should use this class. It is intended primarily for those who are
 * implementing an {@code ExecutorService}. Most users should call
 * {@link ListeningExecutorService#submit(Callable) ListeningExecutorService.submit} on a
 * service obtained from {@link MoreExecutors#listeningDecorator}.
 *
 * @author Sven Mawson
 * @since 1.0
 */
...
public class ListenableFutureTask<V extends Object> extends FutureTask<V> implements ListenableFuture<V> {
	...
	// TODO(cpovirk): explore ways of making ListenableFutureTask final. There are some
	// valid reasons
	// such as BoundedQueueExecutorService to allow extends but it would be nice to make
	// it final to
	 ...
	 * @since 10.0
	 */
	public static <V extends Object> ListenableFutureTask<V> create(Runnable runnable, @ParametricNullness V result) {

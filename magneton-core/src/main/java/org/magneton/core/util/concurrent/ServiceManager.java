...
import org.magneton.core.util.concurrent.Service.State;
...
import static org.magneton.core.util.concurrent.Service.State.FAILED;
import static org.magneton.core.util.concurrent.Service.State.NEW;
import static org.magneton.core.util.concurrent.Service.State.RUNNING;
import static org.magneton.core.util.concurrent.Service.State.STARTING;
import static org.magneton.core.util.concurrent.Service.State.STOPPING;
import static org.magneton.core.util.concurrent.Service.State.TERMINATED;

/**
 * A manager for monitoring and controlling a set of
 * {@linkplain org.magneton.core.util.concurrent.Service services}. This class provides
 * methods for {@linkplain #startAsync() starting}, {@linkplain #stopAsync() stopping} and
 * {@linkplain #servicesByState inspecting} a collection of
 * {@linkplain org.magneton.core.util.concurrent.Service services}. Additionally, users
 * can monitor state transitions with the {@linkplain Listener listener} mechanism.
 *
 * <p>
...
public final class ServiceManager implements ServiceManagerBridge {
	...
	private final ImmutableList<org.magneton.core.util.concurrent.Service> services;
	 ...
	 */
	public ServiceManager(Iterable<? extends org.magneton.core.util.concurrent.Service> services) {
		ImmutableList<org.magneton.core.util.concurrent.Service> copy = ImmutableList.copyOf(services);
		if (copy.isEmpty()) {
			...
			copy = ImmutableList.<org.magneton.core.util.concurrent.Service>of(new NoOpService());
		}
		state = new ServiceManagerState(copy);
		this.services = copy;
		WeakReference<ServiceManagerState> stateReference = new WeakReference<>(state);
		for (org.magneton.core.util.concurrent.Service service : copy) {
	 ...
	 * {@linkplain org.magneton.core.util.concurrent.Service#startAsync started}.
	 *
	 * <p>
	...
	/**
	 * Initiates service {@linkplain org.magneton.core.util.concurrent.Service#startAsync
	 * startup} on all the services being managed. It is only valid to call this method if
	 * all of the services are {@linkplain State#NEW new}.
	 * @return this
	...
	public ServiceManager startAsync() {
		for (org.magneton.core.util.concurrent.Service service : services) {
	...
	/**
	 * Initiates service {@linkplain org.magneton.core.util.concurrent.Service#stopAsync
	 ...
	 */
	@Override
	public ImmutableSetMultimap<State, org.magneton.core.util.concurrent.Service> servicesByState() {
	 ...
	 */
	public ImmutableMap<org.magneton.core.util.concurrent.Service, Long> startupTimes() {
	 ...
	 * each particular {@linkplain org.magneton.core.util.concurrent.Service service}
	 * starts, or terminates), should attach
	 * {@linkplain org.magneton.core.util.concurrent.Service.Listener service listeners}
	...
	public abstract static class Listener {
		 ...
		 */
		public void failure(org.magneton.core.util.concurrent.Service service) {
		}
	...
	private static final class ServiceManagerState {
		...
		final org.magneton.core.util.concurrent.Monitor monitor = new org.magneton.core.util.concurrent.Monitor();

		@GuardedBy("monitor")
		final SetMultimap<State, org.magneton.core.util.concurrent.Service> servicesByState = MultimapBuilder
				.enumKeys(State.class).linkedHashSetValues().build();

		@GuardedBy("monitor")
		final Multiset<State> states = servicesByState.keys();

		@GuardedBy("monitor")
		final Map<org.magneton.core.util.concurrent.Service, Stopwatch> startupTimers = org.magneton.core.collect.Maps
				.newIdentityHashMap();
		 ...
		 */
		final org.magneton.core.util.concurrent.Monitor.Guard awaitHealthGuard = new AwaitHealthGuard();

		/** Controls how long to wait for all services to reach a terminal state. */
		final org.magneton.core.util.concurrent.Monitor.Guard stoppedGuard = new StoppedGuard();
		 ...
		 */
		ServiceManagerState(ImmutableCollection<org.magneton.core.util.concurrent.Service> services) {
		 ...
		 * {@link org.magneton.core.util.concurrent.Service#startAsync()}.
		 */
		void tryStartTiming(org.magneton.core.util.concurrent.Service service) {
			monitor.enter();
			try {
		...
		void markReady() {
			...
			try {
				if (!transitioned) {
					...
					List<org.magneton.core.util.concurrent.Service> servicesInBadStates = org.magneton.core.collect.Lists
							.newArrayList();
					for (org.magneton.core.util.concurrent.Service service : servicesByState().values()) {
		...
		ImmutableSetMultimap<State, org.magneton.core.util.concurrent.Service> servicesByState() {
			ImmutableSetMultimap.Builder<State, org.magneton.core.util.concurrent.Service> builder = ImmutableSetMultimap
					.builder();
			monitor.enter();
			try {
				for (Entry<State, org.magneton.core.util.concurrent.Service> entry : servicesByState.entries()) {
		...
		ImmutableMap<org.magneton.core.util.concurrent.Service, Long> startupTimes() {
			List<Entry<org.magneton.core.util.concurrent.Service, Long>> loadTimes;
			monitor.enter();
			try {
				...
				for (Entry<org.magneton.core.util.concurrent.Service, Stopwatch> entry : startupTimers.entrySet()) {
					org.magneton.core.util.concurrent.Service service = entry.getKey();
			...
			Collections.sort(loadTimes, Ordering.natural()
					.onResultOf(new Function<Entry<org.magneton.core.util.concurrent.Service, Long>, Long>() {
						@Override
						public Long apply(Entry<org.magneton.core.util.concurrent.Service, Long> input) {
		 ...
		 * </ol>
		 */
		void transitionService(org.magneton.core.util.concurrent.Service service, State from, State to) {
			...
			try {
				...
				checkState(servicesByState.put(to, service), "Service %s in the state map unexpectedly at %s", service,
						to);
				// Update the timer
				Stopwatch stopwatch = startupTimers.get(service);
				if (stopwatch == null) {
		...
		void enqueueFailedEvent(org.magneton.core.util.concurrent.Service service) {
			listeners.enqueue(new ListenerCallQueue.Event<Listener>() {
				@Override
				public void call(Listener listener) {
		...
		void checkHealthy() {
			if (states.count(RUNNING) != numberOfServices) {
				...
				for (org.magneton.core.util.concurrent.Service service : servicesByState.get(State.FAILED)) {
	...
	/**
	 * A {@link org.magneton.core.util.concurrent.Service} that wraps another service and
	 * times how long it takes for it to start and also calls the
	 * {@link ServiceManagerState#transitionService(org.magneton.core.util.concurrent.Service, State, State)},
	 * to record the state transitions.
	 */
	private static final class ServiceListener extends org.magneton.core.util.concurrent.Service.Listener {

		final org.magneton.core.util.concurrent.Service service;
		...
		ServiceListener(org.magneton.core.util.concurrent.Service service, WeakReference<ServiceManagerState> state) {
	...
	/**
	 * A {@link org.magneton.core.util.concurrent.Service} instance that does nothing.
	 * This is only useful as a placeholder to ensure that the {@link ServiceManager}

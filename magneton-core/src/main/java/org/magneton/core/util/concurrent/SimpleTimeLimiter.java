...
import org.magneton.core.collect.ObjectArrays;
 ...
 * {@link ExecutorService}. If the time limit expires for a given method call, the thread
 * running the call will be interrupted.
 *
...
public final class SimpleTimeLimiter implements TimeLimiter {
	...
	/**
	 * Creates a TimeLimiter instance using the given executor service to execute method
	 * calls.
	 *
	 * <p>
	 * <b>Warning:</b> using a bounded executor may be counterproductive! If the thread
	 * pool fills up, any time callers spend waiting for a thread may count toward their
	 * time limit, and in this case the call may even time out before the target method is
	 * ever invoked.
	 * @param executor the ExecutorService that will execute the method calls on the
	 * target objects; for example, a {@link Executors#newCachedThreadPool()}.
	 * @since 22.0
	 */
	...
	private <T> T callWithTimeout(Callable<T> callable, long timeoutDuration, TimeUnit timeoutUnit,
		...
		try {
			if (amInterruptible) {
				...
				return org.magneton.core.util.concurrent.Uninterruptibles.getUninterruptibly(future, timeoutDuration,
						timeoutUnit);
			}
		}
	...
	private void wrapAndThrowExecutionExceptionOrError(Throwable cause) throws ExecutionException {
		if (cause instanceof Error) {
			throw new org.magneton.core.util.concurrent.ExecutionError((Error) cause);
		}

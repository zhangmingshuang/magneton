...
public interface ListOps {
	 ...
	 */
	default <V> long add(String list, V... values) {
	 ...
	 * @return 操作完成之后当前列表集合中的长度
	 */
	<V> long add(String list, List<V> values);
	 ...
	 * @return 操作完成之后当前列表集合中的长度
	 */
	default <V> long addAtHead(String list, V... values) {
	 ...
	 * @return 操作完成之后当前列表集合中的长度
	 */
	<V> long addAtHead(String list, List<V> values);
	 ...
	 * <b>索引范围说明：</b>如果有一个100长度的列表，则查找范围0，10的数据，即返回11个数据。
	 * @param list 列表名称
	 * @param start 开始索引。从0开始。即0表示第一个元素。支持负数，-1表示最后一个元素，-2表示倒数第二个。
	 * 如果开始索引超过列表大小，则直接返回一个空列表。
	 * @param end 结束索引。该位置会被包括在内。如果超过列表长度，则表示直到当前列表的最后一个元素，超出的被忽略。
	 * @param clazz 将数据转换成该类型
	 * @return 选择范围内的列表数据。
	 */
	<V> List<V> range(String list, long start, long end, Class<V> clazz);
	 ...
	 * {@code null}.
	 */
	<V> V get(String list, long index, Class<V> clazz);
	 ...
	 * @return 删除的数量
	 */
	<V> long remove(String list, V value);
